...
describe('experiment worker client', () => {
  ...
  beforeEach(() => {
    jest.resetModules();
    process.env.SQUIDRUN_EXPERIMENT_WORKER_REQUEST_TIMEOUT_MS = '20';
    stallNextRequest = false;
    workers = [];
    forkMock = jest.fn(() => {
  ...
  afterEach(async () => {
    ...
    delete process.env.SQUIDRUN_EXPERIMENT_WORKER_REQUEST_TIMEOUT_MS;
  });
  ...
  test('executeOperation quarantines a timed-out connected worker before reuse', async () => {
    ...
    await expect(client.executeOperation('health', {}, {})).rejects.toMatchObject({
      code: 'EXPERIMENT_WORKER_TIMEOUT',
    });
    expect(forkMock).toHaveBeenCalledTimes(1);
