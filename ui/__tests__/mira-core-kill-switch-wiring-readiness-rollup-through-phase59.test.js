const killSwitchWiringReadinessRollupThroughPhase59Contract = require('./fixtures/mira-core-kill-switch-wiring-readiness-rollup-through-phase59-contract.json');
const {
  BASELINE_COMMIT,
...
} = require('../modules/mira-core/kill-switch-wiring-readiness-rollup-through-phase59');
const {
  main,
  parseArgs,
} = require('../scripts/hm-mira-core-kill-switch-wiring-readiness-rollup-through-phase59');
...
function build(inputSignals = {}) {
  return buildMiraCoreKillSwitchWiringReadinessRollupThroughPhase59({
    contract: killSwitchWiringReadinessRollupThroughPhase59Contract,
    inputSignals,
...
function rollup(output) {
  return output.kill_switch_wiring_readiness_rollup_through_phase59;
}
...
function recomputeRollupKey(output) {
  rollup(output).idempotency_key =
    killSwitchWiringReadinessRollupThroughPhase59IdempotencyKey(rollup(output));
}

function expectValidatorFails(output, checkId) {
  const validation = validateMiraCoreKillSwitchWiringReadinessRollupThroughPhase59Output(
    output,
    killSwitchWiringReadinessRollupThroughPhase59Contract,
  );
  expect(validation.ok).toBe(false);
  expect(validation.checks.some((entry) => entry.id === checkId && entry.ok === false)).toBe(true);
}

describe('mira core kill-switch wiring readiness rollup through Phase59 v0', () => {
  afterEach(() => {
  ...
  test('satisfies Oracle output, manifest, and validation report shapes', () => {
    ...
    expectRequiredFields(output, killSwitchWiringReadinessRollupThroughPhase59Contract.expectedOutputShape.requiredTopLevelFields);
    expect(killSwitchWiringReadinessRollupThroughPhase59Contract.expectedOutputShape.requiredTopLevelFields)
      .toEqual(REQUIRED_OUTPUT_FIELDS);
    expect(rollup(output).schema).toBe(KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE59_SCHEMA_VERSION);
    expect(rollup(output).version).toBe(0);
    expect(report(output).schema).toBe(VALIDATION_REPORT_SCHEMA_VERSION);
    expectRequiredFields(rollup(output), killSwitchWiringReadinessRollupThroughPhase59Contract.expectedManifestShape.requiredFields);
    expect(killSwitchWiringReadinessRollupThroughPhase59Contract.expectedManifestShape.requiredFields)
      .toEqual(REQUIRED_DRY_CHECK_FIELDS);
    expectRequiredFields(report(output), killSwitchWiringReadinessRollupThroughPhase59Contract.expectedValidationReportShape.requiredFields);
    expect(killSwitchWiringReadinessRollupThroughPhase59Contract.expectedValidationReportShape.requiredFields)
      .toEqual(REQUIRED_VALIDATION_REPORT_FIELDS);
    expect(report(output)).toEqual(expect.objectContaining({
      schema: killSwitchWiringReadinessRollupThroughPhase59Contract.expectedOutputShape.reportSchema,
      fixture_path: 'ui/__tests__/fixtures/mira-core-kill-switch-wiring-readiness-rollup-through-phase59-contract.json',
      baseline_commit: BASELINE_COMMIT,
      decision: 'accepted_validation_only',
      status: killSwitchWiringReadinessRollupThroughPhase59Contract.expectedValidationReportShape.requiredLiteralValues.status,
      caveats: [],
    }));
    expect(validateMiraCoreKillSwitchWiringReadinessRollupThroughPhase59Output(output, killSwitchWiringReadinessRollupThroughPhase59Contract))
      .toEqual(expect.objectContaining({ ok: true }));
    expect(() => assertNoForbiddenOutput(output, killSwitchWiringReadinessRollupThroughPhase59Contract.forbiddenOutputSubstrings))
      .not.toThrow();
  });

  test('pins baseline ea795c8, registries through Phase 59, and exact commit chain', () => {
    const current = rollup(build());
    const expected = killSwitchWiringReadinessRollupThroughPhase59Contract.phaseRegistryExpected;
    const counts = killSwitchWiringReadinessRollupThroughPhase59Contract.expectedCounts;

    expect(BASELINE_COMMIT).toBe(killSwitchWiringReadinessRollupThroughPhase59Contract.baseline.commit);
    ...
    expect(current.phase_registry).toEqual(expect.objectContaining({
      ...
      schema_registry_count:
        killSwitchWiringReadinessRollupThroughPhase59Contract.expectedManifestShape
    ...
    expect(current.commit_chain).toEqual(killSwitchWiringReadinessRollupThroughPhase59Contract.commitChainExpected);
  ...
  test('preserves Phase59 source truth, stale truth, closure chain, and source refs', () => {
    const current = rollup(build());

    expect(current.source_recommendation).toEqual(expect.objectContaining(
      killSwitchWiringReadinessRollupThroughPhase59Contract.sourceRecommendation,
    ));
    expect(current.satisfied_prior_recommendations).toEqual(
      killSwitchWiringReadinessRollupThroughPhase59Contract.satisfiedPriorRecommendations,
    );
    expect(current.current_truth).toEqual(expect.objectContaining(
      killSwitchWiringReadinessRollupThroughPhase59Contract.currentTruthExpected,
    ));
    expect(current.stale_readiness).toEqual(expect.objectContaining(
      killSwitchWiringReadinessRollupThroughPhase59Contract.staleReadinessExpected,
    ));
    expect(current.phase34_prior_recommendations).toEqual(expect.objectContaining(
      killSwitchWiringReadinessRollupThroughPhase59Contract.phase34PriorRecommendationsExpected,
    ));
    expect(current.closure_summary).toEqual(expect.objectContaining(
      killSwitchWiringReadinessRollupThroughPhase59Contract.closureSummaryExpected,
    ));
    ...
    expect(current.source_refs).toEqual(killSwitchWiringReadinessRollupThroughPhase59Contract.sourceRefsExpected);
    expect(current.source_refs).toHaveLength(killSwitchWiringReadinessRollupThroughPhase59Contract.expectedCounts.source_ref_count);
  ...
  test('binds the Phase59 readiness rollup plus Phase49/Phase50/Phase51/Phase52/Phase53/Phase54/Phase55/Phase56/Phase57/Phase58/Phase59 evidence summaries', () => {
    const current = rollup(build());

    expect(current.readiness_rollup_summary).toEqual(expect.objectContaining(
      killSwitchWiringReadinessRollupThroughPhase59Contract.readinessRollupSummaryExpected,
    ));
    expect(current.phase49_boundary_refresh_summary).toEqual(expect.objectContaining(
      killSwitchWiringReadinessRollupThroughPhase59Contract.phase49BoundaryRefreshSummaryExpected,
    ));
    expect(current.phase50_readiness_rollup_summary).toEqual(expect.objectContaining(
      killSwitchWiringReadinessRollupThroughPhase59Contract.phase50ReadinessRollupSummaryExpected,
    ));
    expect(current.phase51_readiness_rollup_summary).toEqual(expect.objectContaining(
      killSwitchWiringReadinessRollupThroughPhase59Contract.phase51ReadinessRollupSummaryExpected,
    ));
    expect(current.phase52_readiness_rollup_summary).toEqual(expect.objectContaining(
      killSwitchWiringReadinessRollupThroughPhase59Contract.phase52ReadinessRollupSummaryExpected,
    ));
    expect(current.phase53_readiness_rollup_summary).toEqual(expect.objectContaining(
      killSwitchWiringReadinessRollupThroughPhase59Contract.phase53ReadinessRollupSummaryExpected,
    ));
    expect(current.phase54_readiness_rollup_summary).toEqual(expect.objectContaining(
      killSwitchWiringReadinessRollupThroughPhase59Contract.phase54ReadinessRollupSummaryExpected,
    ));
    expect(current.phase55_readiness_rollup_summary).toEqual(expect.objectContaining(
      killSwitchWiringReadinessRollupThroughPhase59Contract.phase55ReadinessRollupSummaryExpected,
    ));
    expect(current.phase56_readiness_rollup_summary).toEqual(expect.objectContaining(
      killSwitchWiringReadinessRollupThroughPhase59Contract.phase56ReadinessRollupSummaryExpected,
    ));
    expect(current.phase57_readiness_rollup_summary).toEqual(expect.objectContaining(
      killSwitchWiringReadinessRollupThroughPhase59Contract.phase57ReadinessRollupSummaryExpected,
    ));
    expect(current.phase58_readiness_rollup_summary).toEqual(expect.objectContaining(
      killSwitchWiringReadinessRollupThroughPhase59Contract.phase58ReadinessRollupSummaryExpected,
    ));
    expect(current.phase59_readiness_rollup_summary).toEqual(expect.objectContaining(
      killSwitchWiringReadinessRollupThroughPhase59Contract.phase59ReadinessRollupSummaryExpected,
    ));
    ...
    expect(current.noop_boundary_refresh_summary).toEqual(expect.objectContaining(
      killSwitchWiringReadinessRollupThroughPhase59Contract.noopBoundaryRefreshSummaryExpected,
    ));
    expect(current.noop_boundary_status).toEqual(expect.objectContaining(
      killSwitchWiringReadinessRollupThroughPhase59Contract.noopBoundaryStatusExpected,
    ));
    expect(current.readiness_rollup_summary).toEqual(expect.objectContaining({
      decision: 'remain_kill_switch_wiring_readiness_rollup_contract_only',
      current_through_phase: 59,
      selected_source_recommendation: 'phase60-kill-switch-wiring-readiness-rollup-through-phase59-contract',
      satisfied_prior_recommendation: 'phase60-kill-switch-wiring-readiness-rollup-through-phase58-validator',
  ...
  test('preserves disabled runtime, no-op, env/config, kill-switch, gap, and blocked-slice boundaries', () => {
    ...
    for (const [field, shape] of [
      ['runtime_mode_boundary', killSwitchWiringReadinessRollupThroughPhase59Contract.runtimeModeBoundaryShapeExpected],
      ['flag_reader_boundary', killSwitchWiringReadinessRollupThroughPhase59Contract.flagReaderBoundaryShapeExpected],
      ['env_config_read_boundary', killSwitchWiringReadinessRollupThroughPhase59Contract.envConfigReadBoundaryShapeExpected],
      ['kill_switch_boundary', killSwitchWiringReadinessRollupThroughPhase59Contract.killSwitchBoundaryShapeExpected],
      ['kill_switch_wiring_boundary', killSwitchWiringReadinessRollupThroughPhase59Contract.killSwitchWiringBoundaryShapeExpected],
    ]) {
    ...
    expect(current.noop_dry_check_request_contract).toEqual(expect.objectContaining(
      killSwitchWiringReadinessRollupThroughPhase59Contract.noopDryCheckRequestContractExpected,
    ));
    expect(current.noop_dry_check_result_contract).toEqual(expect.objectContaining(
      killSwitchWiringReadinessRollupThroughPhase59Contract.noopDryCheckResultContractExpected,
    ));
  ...
  test('preserves capability, proof, redaction, boundary, and no-side-effect truth', () => {
    ...
    expect(current.capability_matrix).toEqual(expect.objectContaining(
      killSwitchWiringReadinessRollupThroughPhase59Contract.capabilityMatrixExpected,
    ));
    expect(current.proof_boundaries).toEqual(expect.objectContaining(
      killSwitchWiringReadinessRollupThroughPhase59Contract.proofBoundaryExpected,
    ));
    ...
    for (const field of Object.keys(killSwitchWiringReadinessRollupThroughPhase59Contract.sideEffectTruthExpected)) {
  ...
  test('next recommendations are fixture-owned Tier0/Tier1 and non-authorizing', () => {
    ...
    expect(current.next_phase_recommendations).toEqual(expect.arrayContaining(
      killSwitchWiringReadinessRollupThroughPhase59Contract.nextRecommendationExpectedCandidates
        .map((candidate) => expect.objectContaining(candidate)),
    ));
    expect(current.next_phase_recommendations).toHaveLength(
      killSwitchWiringReadinessRollupThroughPhase59Contract.nextRecommendationExpectedCandidates.length,
    );
  ...
  test('validation report binds static rules, acceptance checks, tamper cases, literals, and paths', () => {
    ...
    expect(report(output).static_rule_results.map((entry) => entry.id))
      .toEqual(killSwitchWiringReadinessRollupThroughPhase59Contract.staticValidationRules.map((entry) => entry.id));
    expect(report(output).acceptance_check_results.map((entry) => entry.id))
      .toEqual(killSwitchWiringReadinessRollupThroughPhase59Contract.acceptanceChecks.map((entry) => entry.id));
    expect(report(output).static_rule_results.length).toBeGreaterThanOrEqual(
      killSwitchWiringReadinessRollupThroughPhase59Contract.expectedCounts.static_rule_min_count,
    );
    expect(report(output).acceptance_check_results.length).toBeGreaterThanOrEqual(
      killSwitchWiringReadinessRollupThroughPhase59Contract.expectedCounts.acceptance_check_min_count,
    );
    expect(report(output).tamper_case_results).toHaveLength(killSwitchWiringReadinessRollupThroughPhase59Contract.tamperCases.length);
    expect(report(output).tamper_case_results.length).toBeGreaterThanOrEqual(
      killSwitchWiringReadinessRollupThroughPhase59Contract.expectedCounts.tamper_case_min_count,
    );
    expect(report(output).required_literal_results.length).toBeGreaterThanOrEqual(
      killSwitchWiringReadinessRollupThroughPhase59Contract.expectedCounts.required_literal_min_count,
    );
    ...
    expect(report(output).referenced_path_results.length).toBeGreaterThanOrEqual(
      killSwitchWiringReadinessRollupThroughPhase59Contract.expectedCounts.source_ref_count,
    );
    ...
    expect(report(output).scoped_status).toEqual(expect.objectContaining({
      ...
      scoped_phase60_through_phase59_files_only: true,
  ...
  test('validator rejects baseline, registry, source, stale, closure, summary, and path drift', () => {
    const cases = [
      ...
      ['accept-readiness-rollup-through-phase59-contract-only', (output) => { rollup(output).readiness_rollup_summary.ready_for_dry_run_now = true; }],
      ['accept-readiness-rollup-through-phase59-non-authorizing', (output) => { rollup(output).readiness_rollup_summary.authorizes_output_file = true; }],
  ...
  test('validator rejects unsafe action drift and Tier2+ recommendations after idempotency recompute', () => {
    for (const phrase of killSwitchWiringReadinessRollupThroughPhase59Contract.unsafeActionPhrases) {
      const output = clone(build());
  ...
  test('validator rejects validation-report lies, missing tamper coverage, and literal result lies', () => {
    ...
    expect(report(stalePhase58ScopedStatus).scoped_status.scoped_phase60_through_phase59_files_only).toBe(true);
    ...
    expect(report(stalePhase59ScopedStatus).scoped_status.scoped_phase60_through_phase59_files_only).toBe(true);
    ...
    delete report(missingCurrentScopedStatus).scoped_status.scoped_phase60_through_phase59_files_only;
  ...
  test('CLI is stdout-only, consumes the Oracle fixture, and ignores output-file flags', () => {
    ...
    expect(parsed.fixturePath).toContain('mira-core-kill-switch-wiring-readiness-rollup-through-phase59-contract.json');
    ...
    expect(rollup(output).phase_registry.current_through_phase)
      .toBe(killSwitchWiringReadinessRollupThroughPhase59Contract.expectedCounts.phase_inventory_count);
    expect(rollup(output).schema_registry.count)
      .toBe(killSwitchWiringReadinessRollupThroughPhase59Contract.expectedCounts.schema_registry_count);
    expect(rollup(output).cli_registry.count)
      .toBe(killSwitchWiringReadinessRollupThroughPhase59Contract.expectedCounts.cli_registry_count);
    expect(rollup(output).commit_chain)
      .toHaveLength(killSwitchWiringReadinessRollupThroughPhase59Contract.expectedCounts.commit_chain_count);
    expect(rollup(output).source_refs)
      .toHaveLength(killSwitchWiringReadinessRollupThroughPhase59Contract.expectedCounts.source_ref_count);
    ...
    expect(validateMiraCoreKillSwitchWiringReadinessRollupThroughPhase59Output(
      output,
      killSwitchWiringReadinessRollupThroughPhase59Contract,
    )).toEqual(expect.objectContaining({ ok: true }));
  });
