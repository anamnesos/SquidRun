const killSwitchWiringReadinessRollupThroughPhase61Contract = require('./fixtures/mira-core-kill-switch-wiring-readiness-rollup-through-phase60-contract.json');
const {
  BASELINE_COMMIT,
...
} = require('../modules/mira-core/kill-switch-wiring-readiness-rollup-through-phase60');
const {
  main,
  parseArgs,
} = require('../scripts/hm-mira-core-kill-switch-wiring-readiness-rollup-through-phase60');
...
function build(inputSignals = {}) {
  return buildMiraCoreKillSwitchWiringReadinessRollupThroughPhase61({
    contract: killSwitchWiringReadinessRollupThroughPhase61Contract,
    inputSignals,
...
function rollup(output) {
  return output.kill_switch_wiring_readiness_rollup_through_phase60;
}
...
function recomputeRollupKey(output) {
  rollup(output).idempotency_key =
    killSwitchWiringReadinessRollupThroughPhase61IdempotencyKey(rollup(output));
}

function expectValidatorFails(output, checkId) {
  const validation = validateMiraCoreKillSwitchWiringReadinessRollupThroughPhase61Output(
    output,
    killSwitchWiringReadinessRollupThroughPhase61Contract,
  );
  expect(validation.ok).toBe(false);
  expect(validation.checks.some((entry) => entry.id === checkId && entry.ok === false)).toBe(true);
}

describe('mira core kill-switch wiring readiness rollup through Phase60 v0', () => {
  afterEach(() => {
  ...
  test('satisfies Oracle output, manifest, and validation report shapes', () => {
    ...
    expectRequiredFields(output, killSwitchWiringReadinessRollupThroughPhase61Contract.expectedOutputShape.requiredTopLevelFields);
    expect(killSwitchWiringReadinessRollupThroughPhase61Contract.expectedOutputShape.requiredTopLevelFields)
      .toEqual(REQUIRED_OUTPUT_FIELDS);
    expect(rollup(output).schema).toBe(KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE61_SCHEMA_VERSION);
    expect(rollup(output).version).toBe(0);
    expect(report(output).schema).toBe(VALIDATION_REPORT_SCHEMA_VERSION);
    expectRequiredFields(rollup(output), killSwitchWiringReadinessRollupThroughPhase61Contract.expectedManifestShape.requiredFields);
    expect(killSwitchWiringReadinessRollupThroughPhase61Contract.expectedManifestShape.requiredFields)
      .toEqual(REQUIRED_DRY_CHECK_FIELDS);
    expectRequiredFields(report(output), killSwitchWiringReadinessRollupThroughPhase61Contract.expectedValidationReportShape.requiredFields);
    expect(killSwitchWiringReadinessRollupThroughPhase61Contract.expectedValidationReportShape.requiredFields)
      .toEqual(REQUIRED_VALIDATION_REPORT_FIELDS);
    expect(report(output)).toEqual(expect.objectContaining({
      schema: killSwitchWiringReadinessRollupThroughPhase61Contract.expectedOutputShape.reportSchema,
      fixture_path: 'ui/__tests__/fixtures/mira-core-kill-switch-wiring-readiness-rollup-through-phase60-contract.json',
      baseline_commit: BASELINE_COMMIT,
      decision: 'accepted_validation_only',
      status: killSwitchWiringReadinessRollupThroughPhase61Contract.expectedValidationReportShape.requiredLiteralValues.status,
      caveats: [],
    }));
    expect(validateMiraCoreKillSwitchWiringReadinessRollupThroughPhase61Output(output, killSwitchWiringReadinessRollupThroughPhase61Contract))
      .toEqual(expect.objectContaining({ ok: true }));
    expect(() => assertNoForbiddenOutput(output, killSwitchWiringReadinessRollupThroughPhase61Contract.forbiddenOutputSubstrings))
      .not.toThrow();
  });

  test('pins baseline 7ee9ea4, registries through Phase 60, and exact commit chain', () => {
    const current = rollup(build());
    const expected = killSwitchWiringReadinessRollupThroughPhase61Contract.phaseRegistryExpected;
    const counts = killSwitchWiringReadinessRollupThroughPhase61Contract.expectedCounts;

    expect(BASELINE_COMMIT).toBe(killSwitchWiringReadinessRollupThroughPhase61Contract.baseline.commit);
    ...
    expect(current.phase_registry).toEqual(expect.objectContaining({
      ...
      schema_registry_count:
        killSwitchWiringReadinessRollupThroughPhase61Contract.expectedManifestShape
    ...
    expect(current.commit_chain).toEqual(killSwitchWiringReadinessRollupThroughPhase61Contract.commitChainExpected);
  ...
  test('preserves Phase60 source truth, stale truth, closure chain, and source refs', () => {
    const current = rollup(build());

    expect(current.source_recommendation).toEqual(expect.objectContaining(
      killSwitchWiringReadinessRollupThroughPhase61Contract.sourceRecommendation,
    ));
    expect(current.satisfied_prior_recommendations).toEqual(
      killSwitchWiringReadinessRollupThroughPhase61Contract.satisfiedPriorRecommendations,
    );
    expect(current.current_truth).toEqual(expect.objectContaining(
      killSwitchWiringReadinessRollupThroughPhase61Contract.currentTruthExpected,
    ));
    expect(current.stale_readiness).toEqual(expect.objectContaining(
      killSwitchWiringReadinessRollupThroughPhase61Contract.staleReadinessExpected,
    ));
    expect(current.phase34_prior_recommendations).toEqual(expect.objectContaining(
      killSwitchWiringReadinessRollupThroughPhase61Contract.phase34PriorRecommendationsExpected,
    ));
    expect(current.closure_summary).toEqual(expect.objectContaining(
      killSwitchWiringReadinessRollupThroughPhase61Contract.closureSummaryExpected,
    ));
    ...
    expect(current.source_refs).toEqual(killSwitchWiringReadinessRollupThroughPhase61Contract.sourceRefsExpected);
    expect(current.source_refs).toHaveLength(killSwitchWiringReadinessRollupThroughPhase61Contract.expectedCounts.source_ref_count);
  ...
  test('binds the Phase60 readiness rollup plus Phase49/Phase50/Phase51/Phase52/Phase53/Phase54/Phase55/Phase56/Phase57/Phase58/Phase59/Phase60 evidence summaries', () => {
    const current = rollup(build());

    expect(current.readiness_rollup_summary).toEqual(expect.objectContaining(
      killSwitchWiringReadinessRollupThroughPhase61Contract.readinessRollupSummaryExpected,
    ));
    expect(current.phase49_boundary_refresh_summary).toEqual(expect.objectContaining(
      killSwitchWiringReadinessRollupThroughPhase61Contract.phase49BoundaryRefreshSummaryExpected,
    ));
    expect(current.phase50_readiness_rollup_summary).toEqual(expect.objectContaining(
      killSwitchWiringReadinessRollupThroughPhase61Contract.phase50ReadinessRollupSummaryExpected,
    ));
    expect(current.phase51_readiness_rollup_summary).toEqual(expect.objectContaining(
      killSwitchWiringReadinessRollupThroughPhase61Contract.phase51ReadinessRollupSummaryExpected,
    ));
    expect(current.phase52_readiness_rollup_summary).toEqual(expect.objectContaining(
      killSwitchWiringReadinessRollupThroughPhase61Contract.phase52ReadinessRollupSummaryExpected,
    ));
    expect(current.phase53_readiness_rollup_summary).toEqual(expect.objectContaining(
      killSwitchWiringReadinessRollupThroughPhase61Contract.phase53ReadinessRollupSummaryExpected,
    ));
    expect(current.phase54_readiness_rollup_summary).toEqual(expect.objectContaining(
      killSwitchWiringReadinessRollupThroughPhase61Contract.phase54ReadinessRollupSummaryExpected,
    ));
    expect(current.phase55_readiness_rollup_summary).toEqual(expect.objectContaining(
      killSwitchWiringReadinessRollupThroughPhase61Contract.phase55ReadinessRollupSummaryExpected,
    ));
    expect(current.phase56_readiness_rollup_summary).toEqual(expect.objectContaining(
      killSwitchWiringReadinessRollupThroughPhase61Contract.phase56ReadinessRollupSummaryExpected,
    ));
    expect(current.phase57_readiness_rollup_summary).toEqual(expect.objectContaining(
      killSwitchWiringReadinessRollupThroughPhase61Contract.phase57ReadinessRollupSummaryExpected,
    ));
    expect(current.phase58_readiness_rollup_summary).toEqual(expect.objectContaining(
      killSwitchWiringReadinessRollupThroughPhase61Contract.phase58ReadinessRollupSummaryExpected,
    ));
    expect(current.phase59_readiness_rollup_summary).toEqual(expect.objectContaining(
      killSwitchWiringReadinessRollupThroughPhase61Contract.phase59ReadinessRollupSummaryExpected,
    ));
    expect(current.phase60_readiness_rollup_summary).toEqual(expect.objectContaining(
      killSwitchWiringReadinessRollupThroughPhase61Contract.phase60ReadinessRollupSummaryExpected,
    ));
    ...
    expect(current.noop_boundary_refresh_summary).toEqual(expect.objectContaining(
      killSwitchWiringReadinessRollupThroughPhase61Contract.noopBoundaryRefreshSummaryExpected,
    ));
    expect(current.noop_boundary_status).toEqual(expect.objectContaining(
      killSwitchWiringReadinessRollupThroughPhase61Contract.noopBoundaryStatusExpected,
    ));
    expect(current.readiness_rollup_summary).toEqual(expect.objectContaining({
      decision: 'remain_kill_switch_wiring_readiness_rollup_contract_only',
      current_through_phase: 60,
      selected_source_recommendation: 'phase61-kill-switch-wiring-readiness-rollup-through-phase60-contract',
      satisfied_prior_recommendation: 'phase61-kill-switch-wiring-readiness-rollup-through-phase59-validator',
  ...
  test('preserves disabled runtime, no-op, env/config, kill-switch, gap, and blocked-slice boundaries', () => {
    ...
    for (const [field, shape] of [
      ['runtime_mode_boundary', killSwitchWiringReadinessRollupThroughPhase61Contract.runtimeModeBoundaryShapeExpected],
      ['flag_reader_boundary', killSwitchWiringReadinessRollupThroughPhase61Contract.flagReaderBoundaryShapeExpected],
      ['env_config_read_boundary', killSwitchWiringReadinessRollupThroughPhase61Contract.envConfigReadBoundaryShapeExpected],
      ['kill_switch_boundary', killSwitchWiringReadinessRollupThroughPhase61Contract.killSwitchBoundaryShapeExpected],
      ['kill_switch_wiring_boundary', killSwitchWiringReadinessRollupThroughPhase61Contract.killSwitchWiringBoundaryShapeExpected],
    ]) {
    ...
    expect(current.noop_dry_check_request_contract).toEqual(expect.objectContaining(
      killSwitchWiringReadinessRollupThroughPhase61Contract.noopDryCheckRequestContractExpected,
    ));
    expect(current.noop_dry_check_result_contract).toEqual(expect.objectContaining(
      killSwitchWiringReadinessRollupThroughPhase61Contract.noopDryCheckResultContractExpected,
    ));
  ...
  test('preserves capability, proof, redaction, boundary, and no-side-effect truth', () => {
    ...
    expect(current.capability_matrix).toEqual(expect.objectContaining(
      killSwitchWiringReadinessRollupThroughPhase61Contract.capabilityMatrixExpected,
    ));
    expect(current.proof_boundaries).toEqual(expect.objectContaining(
      killSwitchWiringReadinessRollupThroughPhase61Contract.proofBoundaryExpected,
    ));
    ...
    for (const field of Object.keys(killSwitchWiringReadinessRollupThroughPhase61Contract.sideEffectTruthExpected)) {
  ...
  test('next recommendations are fixture-owned Tier0/Tier1 and non-authorizing', () => {
    ...
    expect(current.next_phase_recommendations).toEqual(expect.arrayContaining(
      killSwitchWiringReadinessRollupThroughPhase61Contract.nextRecommendationExpectedCandidates
        .map((candidate) => expect.objectContaining(candidate)),
    ));
    expect(current.next_phase_recommendations).toHaveLength(
      killSwitchWiringReadinessRollupThroughPhase61Contract.nextRecommendationExpectedCandidates.length,
    );
  ...
  test('validation report binds static rules, acceptance checks, tamper cases, literals, and paths', () => {
    ...
    expect(report(output).static_rule_results.map((entry) => entry.id))
      .toEqual(killSwitchWiringReadinessRollupThroughPhase61Contract.staticValidationRules.map((entry) => entry.id));
    expect(report(output).acceptance_check_results.map((entry) => entry.id))
      .toEqual(killSwitchWiringReadinessRollupThroughPhase61Contract.acceptanceChecks.map((entry) => entry.id));
    expect(report(output).static_rule_results.length).toBeGreaterThanOrEqual(
      killSwitchWiringReadinessRollupThroughPhase61Contract.expectedCounts.static_rule_min_count,
    );
    expect(report(output).acceptance_check_results.length).toBeGreaterThanOrEqual(
      killSwitchWiringReadinessRollupThroughPhase61Contract.expectedCounts.acceptance_check_min_count,
    );
    expect(report(output).tamper_case_results).toHaveLength(killSwitchWiringReadinessRollupThroughPhase61Contract.tamperCases.length);
    expect(report(output).tamper_case_results.length).toBeGreaterThanOrEqual(
      killSwitchWiringReadinessRollupThroughPhase61Contract.expectedCounts.tamper_case_min_count,
    );
    expect(report(output).required_literal_results.length).toBeGreaterThanOrEqual(
      killSwitchWiringReadinessRollupThroughPhase61Contract.expectedCounts.required_literal_min_count,
    );
    ...
    expect(report(output).referenced_path_results.length).toBeGreaterThanOrEqual(
      killSwitchWiringReadinessRollupThroughPhase61Contract.expectedCounts.source_ref_count,
    );
    ...
    expect(report(output).scoped_status).toEqual(expect.objectContaining({
      ...
      scoped_phase61_through_phase60_files_only: true,
  ...
  test('validator rejects baseline, registry, source, stale, closure, summary, and path drift', () => {
    const cases = [
      ...
      ['accept-readiness-rollup-through-phase60-contract-only', (output) => { rollup(output).readiness_rollup_summary.ready_for_dry_run_now = true; }],
      ['accept-readiness-rollup-through-phase60-non-authorizing', (output) => { rollup(output).readiness_rollup_summary.authorizes_output_file = true; }],
  ...
  test('validator rejects unsafe action drift and Tier2+ recommendations after idempotency recompute', () => {
    for (const phrase of killSwitchWiringReadinessRollupThroughPhase61Contract.unsafeActionPhrases) {
      const output = clone(build());
  ...
  test('validator rejects validation-report lies, missing tamper coverage, and literal result lies', () => {
    ...
    expect(report(stalePhase58ScopedStatus).scoped_status.scoped_phase61_through_phase60_files_only).toBe(true);
    ...
    expect(report(stalePhase59ScopedStatus).scoped_status.scoped_phase61_through_phase60_files_only).toBe(true);
    ...
    expect(report(stalePhase60ScopedStatus).scoped_status.scoped_phase61_through_phase60_files_only).toBe(true);
    ...
    delete report(missingCurrentScopedStatus).scoped_status.scoped_phase61_through_phase60_files_only;
  ...
  test('CLI is stdout-only, consumes the Oracle fixture, and ignores output-file flags', () => {
    ...
    expect(parsed.fixturePath).toContain('mira-core-kill-switch-wiring-readiness-rollup-through-phase60-contract.json');
    ...
    expect(rollup(output).phase_registry.current_through_phase)
      .toBe(killSwitchWiringReadinessRollupThroughPhase61Contract.expectedCounts.phase_inventory_count);
    expect(rollup(output).schema_registry.count)
      .toBe(killSwitchWiringReadinessRollupThroughPhase61Contract.expectedCounts.schema_registry_count);
    expect(rollup(output).cli_registry.count)
      .toBe(killSwitchWiringReadinessRollupThroughPhase61Contract.expectedCounts.cli_registry_count);
    expect(rollup(output).commit_chain)
      .toHaveLength(killSwitchWiringReadinessRollupThroughPhase61Contract.expectedCounts.commit_chain_count);
    expect(rollup(output).source_refs)
      .toHaveLength(killSwitchWiringReadinessRollupThroughPhase61Contract.expectedCounts.source_ref_count);
    ...
    expect(validateMiraCoreKillSwitchWiringReadinessRollupThroughPhase61Output(
      output,
      killSwitchWiringReadinessRollupThroughPhase61Contract,
    )).toEqual(expect.objectContaining({ ok: true }));
  });
