...
describe('Mira runtime bridge manual-plan API', () => {
  ...
  test('promotes saved Mission Control route previews into internal route requests only', async () => {
    ...
    const postActivationDesignPipelineStatusResponse = await fetch(`${baseUrl}/mission-control/activation-pipeline-status?includeInternal=1`);
    const postActivationDesignPipelineStatusPayload = await postActivationDesignPipelineStatusResponse.json();
    const activationRequestStatusDir = path.join(tempStateRoot, 'mission-control', 'internal-send-activation-requests');
    const activationRequestFilesAfterStatusRefresh = fs.existsSync(activationRequestStatusDir)
      ? fs.readdirSync(activationRequestStatusDir).filter((file) => file.endsWith('.json'))
      : [];
    expect(postActivationDesignPipelineStatusResponse.status).toBe(200);
    expect(postActivationDesignPipelineStatusPayload.currentStage).toEqual(expect.objectContaining({
      id: 'activation_design',
      label: 'Activation design',
      latestToken: activationDesignPayload.design.actionToken,
      relativePath: activationDesignPayload.relativePath,
    ...
    expect(postActivationDesignPipelineStatusPayload.lastSavedArtifact).toEqual(expect.objectContaining({
      id: 'activation_design',
      label: 'Activation design',
      latestToken: activationDesignPayload.design.actionToken,
      relativePath: activationDesignPayload.relativePath,
    }));
    expect(postActivationDesignPipelineStatusPayload.currentStageTrace.entries).toEqual(expect.arrayContaining([
      expect.objectContaining({
        stageId: 'internal_send_dry_run',
        token: internalSendDryRunPayload.dryRun.actionToken,
        sourceStageId: 'dispatch_readiness',
    ...
    expect(postActivationDesignPipelineStatusPayload.advanceSelection).toEqual(expect.objectContaining({
      status: 'advance_available',
      selectedStageId: 'activation_design',
      selectedStageLabel: 'Activation design',
      selectedArtifactToken: activationDesignPayload.design.actionToken,
      selectedRelativePath: activationDesignPayload.relativePath,
      selectedSourceStageId: 'internal_send_dry_run',
      selectedSourceToken: internalSendDryRunPayload.dryRun.actionToken,
      selectedBodySha256: expectedDeliveryBodySha256,
      selectedAdapterPacketSha256: expectedAdapterPacketSha256,
      nextStageId: 'activation_request',
      nextStageLabel: 'Activation request',
    }));
    expect(postActivationDesignPipelineStatusPayload.manualActionPreflight).toEqual(expect.objectContaining({
      status: 'ready',
      selectedStageId: 'activation_design',
      selectedStageLabel: 'Activation design',
      manualActionLabel: 'Preview activation request',
      tokenField: 'internalSendActivationDesignToken',
      tokenValue: activationDesignPayload.design.actionToken,
    }));
    expect(postActivationDesignPipelineStatusPayload.payloadPreview).toEqual(expect.objectContaining({
      status: 'ready',
      actionLabel: 'Preview activation request',
      method: 'POST',
      endpoint: '/mission-control/internal-send-activation-requests',
      payload: {
        internalSendActivationDesignToken: activationDesignPayload.design.actionToken,
      },
      requiredManualInputs: [],
    }));
    expect(postActivationDesignPipelineStatusPayload.payloadPreview.handlerDriftCheck).toEqual(expect.objectContaining({
      status: 'matched',
      handlerName: 'createInternalSendActivationRequest',
      expectedEndpoint: '/mission-control/internal-send-activation-requests',
      expectedTokenField: 'internalSendActivationDesignToken',
      previewTokenField: 'internalSendActivationDesignToken',
    }));
    expect(activationRequestFilesAfterStatusRefresh).toHaveLength(0);
    ...
    const postDecisionAuditPipelineStatusResponse = await fetch(`${baseUrl}/mission-control/activation-pipeline-status?includeInternal=1`);
    const postDecisionAuditPipelineStatusPayload = await postDecisionAuditPipelineStatusResponse.json();
    const implementationReadinessStatusDir = path.join(tempStateRoot, 'mission-control', 'internal-send-activation-implementation-readiness');
    const implementationReadinessFilesAfterStatusRefresh = fs.existsSync(implementationReadinessStatusDir)
      ? fs.readdirSync(implementationReadinessStatusDir).filter((file) => file.endsWith('.json'))
      : [];
    expect(postDecisionAuditPipelineStatusResponse.status).toBe(200);
    expect(postDecisionAuditPipelineStatusPayload.currentStage).toEqual(expect.objectContaining({
      id: 'activation_decision_audit',
      label: 'Decision audit',
      latestToken: decisionAuditPayload.audit.actionToken,
      relativePath: decisionAuditPayload.relativePath,
    ...
    expect(postDecisionAuditPipelineStatusPayload.lastSavedArtifact).toEqual(expect.objectContaining({
      id: 'activation_decision_audit',
      label: 'Decision audit',
      latestToken: decisionAuditPayload.audit.actionToken,
      relativePath: decisionAuditPayload.relativePath,
    }));
    expect(postDecisionAuditPipelineStatusPayload.currentStageTrace.entries).toEqual(expect.arrayContaining([
      expect.objectContaining({
        stageId: 'activation_request',
        token: activationRequestPayload.request.actionToken,
        relativePath: activationRequestPayload.relativePath,
    ...
    expect(postDecisionAuditPipelineStatusPayload.advanceSelection).toEqual(expect.objectContaining({
      status: 'advance_available',
      selectedStageId: 'activation_decision_audit',
      selectedStageLabel: 'Decision audit',
      selectedArtifactToken: decisionAuditPayload.audit.actionToken,
      selectedRelativePath: decisionAuditPayload.relativePath,
      selectedSourceStageId: 'activation_request',
      selectedSourceToken: activationRequestPayload.request.actionToken,
      selectedBodySha256: expectedDeliveryBodySha256,
      selectedAdapterPacketSha256: expectedAdapterPacketSha256,
      nextStageId: 'activation_implementation_readiness',
      nextStageLabel: 'Implementation readiness',
    }));
    expect(postDecisionAuditPipelineStatusPayload.manualActionPreflight).toEqual(expect.objectContaining({
      status: 'ready',
      selectedStageId: 'activation_decision_audit',
      selectedStageLabel: 'Decision audit',
      manualActionLabel: 'Check implementation readiness',
      tokenField: 'internalSendActivationDecisionAuditToken',
      tokenValue: decisionAuditPayload.audit.actionToken,
    }));
    expect(postDecisionAuditPipelineStatusPayload.payloadPreview).toEqual(expect.objectContaining({
      status: 'ready',
      actionLabel: 'Check implementation readiness',
      method: 'POST',
      endpoint: '/mission-control/internal-send-activation-implementation-readiness',
      payload: {
        internalSendActivationDecisionAuditToken: decisionAuditPayload.audit.actionToken,
      },
      requiredManualInputs: [],
    }));
    expect(postDecisionAuditPipelineStatusPayload.payloadPreview.handlerDriftCheck).toEqual(expect.objectContaining({
      status: 'matched',
      handlerName: 'createInternalSendActivationImplementationReadiness',
      expectedEndpoint: '/mission-control/internal-send-activation-implementation-readiness',
      expectedTokenField: 'internalSendActivationDecisionAuditToken',
      previewTokenField: 'internalSendActivationDecisionAuditToken',
    }));
    expect(implementationReadinessFilesAfterStatusRefresh).toHaveLength(0);
    ...
    const walkthrough = pipelineStatusPayload.endToEndReadout.demoPath.walkthrough;
    expect(walkthrough).toEqual(expect.objectContaining({
      protocol: 'mira.mission_control_demo_walkthrough.v0',
      title: 'Mission Control completed-chain hard-stop walkthrough',
      narrativeSteps: expect.arrayContaining([
      ...
      whyUseful: expect.stringContaining('inspectable next-state explanation instead of generic chat'),
      stillManual: expect.stringContaining('does not click, submit, send, execute'),
      nextBoundary: expect.stringContaining('hard-stop contract'),
    }));
    expect(walkthrough.sourceEvidence).toEqual(expect.arrayContaining([
