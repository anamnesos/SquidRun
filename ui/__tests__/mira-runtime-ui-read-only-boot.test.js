...
function collectMissionControlText(elements) {
  ...
  return [
    ...
    elements.internalRouteAuditPromotionDecisionGateProofSummary.textContent,
    draftText,
...
describe('Mira runtime UI boot', () => {
  ...
  test('renders internal-route promotion review plan from mocked SquidRun context without turn POST', async () => {
    const appJsPath = path.join(__dirname, '..', '..', 'mira', 'ui', 'app.js');
    const appJs = fs.readFileSync(appJsPath, 'utf8');
    const commandCardId = 'mission-control-v0-command-card-acceptance';
    const proofId = 'mission-control-command-card-route-plan-follow-through-v0';
    const planId = 'mission-control-internal-route-promotion-review-plan-v0';
    const planMessage = 'Review the internal-route promotion/review plan from the visible mission-control-command-card-route-plan-follow-through-v0 proof for source evidence, manual review boundary, no-send, no-promotion, no-route-flip, no-execution, and exactly one James-action line; do not POST, route, send, promote, execute, start runtime, open browser/workbench, perform UI/status actions, call provider/model, touch credentials, deploy, move money, or touch trading.';
    const controlPoint = 'James can inspect this local plan before any future promotion, route, send, runtime, browser, workbench, UI/status, provider/model, credential, deploy, money, or trading action is proposed.';
    const harness = createRuntimeBootHarness({
      customizeSquidRunContext(context) {
        context.lane.sourceRef = 'architect#282';
        context.git.dirtyCount = 0;
        context.git.statusPreview = [];
        context.dirtyWork.summary = 'Worktree is clean.';
        context.dirtyWork.files = [];
        context.missionControl.answer = [
          'Project/lane: squidrun / architect#282. Display the completed internal-route promotion/review plan from local context only.',
          `Next team move: Oracle reviews ${planId}; Builder holds it as local context.`,
          'JAMES ACTION: NONE - Local display-only Mission Control work; no runtime, route, send, provider/model, credential, deploy, money, or external action is needed.',
        ].join('\n');
        context.missionControl.nextTeamMove = `Oracle reviews ${planId}; Builder holds it as local context.`;
        context.missionControl.commandCardAcceptance = {
          id: commandCardId,
          status: 'acceptance_planning_only',
          owner: 'Builder',
          ...
          cardFields: {
            currentLaneWhyItMatters: 'Current lane architect#282 matters because James needs the internal-route plan visible without terminal logs.',
            whatChangedRecently: 'Committed b01f00b8 internal-route promotion plan makes the plan visible context.',
            builderNextMove: 'Render the completed internal-route promotion/review plan from already-loaded /squidrun/context only.',
            oracleNextMove: 'Review the display-only plan surface for no live effects.',
            contextCardStatus: 'Context-card/current dirty-context status: Worktree is clean.',
            jamesActionLine: 'JAMES ACTION: NONE',
            dryRunRoutePlan: {
              target: 'oracle',
        ...
        context.missionControl.commandCardRoutePlanFollowThroughProof = {
          ...
          preconditions: ['Worktree is clean.'],
          refusalNoGoConditions: ['Any POST, /turn call, route, send, promotion, runtime/browser/workbench action, provider/model call, credential access, deploy, money, or trading effect.'],
          audit: {
            proofOnly: true,
            planningOnly: true,
            runtimeStarted: false,
        ...
        context.missionControl.internalRoutePromotionReviewPlan = {
          ...
          sourceEvidence: [
            {
              kind: 'completed_context',
              summary: `Completed context retained: ${commandCardId} remains acceptance_planning_only.`,
            },
            {
              kind: 'completed_context',
              summary: `Completed context retained: ${proofId} remains proof_ready_for_oracle_review.`,
            },
            {
              kind: 'comms',
          ...
          completedContext: {
            commandCardAcceptanceId: commandCardId,
            commandCardRoutePlanFollowThroughProofId: proofId,
          },
          jamesControlPoint: controlPoint,
          preconditions: [
            'Worktree is clean.',
            ...
            'The plan is read from loaded /squidrun/context only.',
          ],
          refusalNoGoConditions: [
            'Missing completed visible proof context.',
            'Any POST, /turn call, route, send, promotion, runtime/browser/workbench action, provider/model call, credential access, deploy, money, or trading effect.',
          ],
          audit: {
            planningOnly: true,
            manualOnly: true,
        ...
        context.summary.happening = 'Working in squidrun on architect#282: display-only internal-route promotion/review plan surface.';
        context.summary.nextStep = `Oracle reviews ${planId}; Builder holds it as local context.`;
        context.summary.commandCardAcceptance = `${commandCardId}: completed visible command-card context.`;
        context.summary.commandCardRoutePlanFollowThroughProof = `${proofId}: proof_ready_for_oracle_review.`;
        context.summary.internalRoutePromotionReviewPlan = `${planId}: planning_only_ready_for_oracle_review.`;
        context.summary.jamesAction = 'NONE';
        context.summary.jamesActionReason = 'Local display-only Mission Control work; no runtime, route, send, provider/model, credential, deploy, money, or external action is needed.';
        return context;
      },
    ...
    expect(planText).toContain(`Internal-route promotion/review plan: ${planId}`);
    expect(planText).toContain('Status: planning_only_ready_for_oracle_review');
    expect(planText).toContain('Owner: Builder');
    expect(planText).toContain(`Completed contexts: ${commandCardId} + ${proofId}`);
    expect(planText).toContain('Target: oracle');
    expect(planText).toContain('Purpose: internal-route promotion no-send review');
    expect(planText).toContain(`Message/body: ${planMessage}`);
    expect(planText).toContain('architect#270 0cb27b6b');
    expect(planText).toContain('oracle#88 0cb27b6b');
    expect(planText).toContain(`Control point: ${controlPoint}`);
    expect(planText).toContain('Preconditions: Worktree is clean. | commandCardAcceptance is present as completed visible context. | mission-control-command-card-route-plan-follow-through-v0 is present as completed visible context. | The plan is read from loaded /squidrun/context only.');
    expect(planText).toContain('No-go: Missing completed visible proof context. | Any POST, /turn call, route, send, promotion, runtime/browser/workbench action, provider/model call, credential access, deploy, money, or trading effect.');
    expect(planText).toContain('Audit flags: planningOnly=true, manualOnly=true');
    expect(planText).toContain('sendPerformed=false');
    expect(planText).toContain('promotionPerformed=false');
    expect(planText).toContain('routeFlip=false');
    expect(planText).toContain('runtimeExecutes=false');
    expect(planText).toContain('runtimeStarted=false');
    expect(planText).toContain('browserOpened=false');
    expect(planText).toContain('workbenchOpened=false');
    expect(planText).toContain('uiActionPerformed=false');
    expect(planText).toContain('fetched=false');
    expect(planText).toContain('posted=false');
    expect(planText).toContain('routed=false');
    expect(planText).toContain('sent=false');
    expect(planText).toContain('providerInvoked=false');
    expect(planText).toContain('modelInvoked=false');
    expect(planText).toContain('accountAccessed=false');
    expect(planText).toContain('tokenAccessed=false');
    expect(planText).toContain('credentialAccessed=false');
    expect(planText).toContain('deviceTouched=false');
    expect(planText).toContain('userTargeted=false');
    expect(planText).toContain('externalTargeted=false');
    expect(planText).toContain('deployed=false');
    expect(planText).toContain('moneyMovement=false');
    expect(planText).toContain('tradingTouched=false');
    expect(missionSurfaceText).toContain(commandCardId);
    expect(missionSurfaceText).toContain(proofId);
  ...
  test('renders internal-route audit promotion decision-gate proof from mocked SquidRun context without turn POST', async () => {
    const appJsPath = path.join(__dirname, '..', '..', 'mira', 'ui', 'app.js');
    const appJs = fs.readFileSync(appJsPath, 'utf8');
    const commandCardId = 'mission-control-v0-command-card-acceptance';
    const routePlanProofId = 'mission-control-command-card-route-plan-follow-through-v0';
    const promotionPlanId = 'mission-control-internal-route-promotion-review-plan-v0';
    const auditReviewProofId = 'mission-control-internal-route-audit-review-lane-proof-v0';
    const decisionGateProofId = 'mission-control-internal-route-audit-promotion-decision-gate-proof-v0';
    const proofBody = 'Review mission-control-internal-route-audit-promotion-decision-gate-proof-v0 from the visible mission-control-internal-route-audit-review-lane-proof-v0 only for owner Builder, target Oracle, source evidence, James control point before real promotion, preconditions/no-go conditions, manual-only/no-send/no-promotion/no-route-flip/no-execution audit flags, and exactly one James-action line; do not POST, route, send, promote, flip routes, execute, start runtime, open browser/workbench, perform UI/status actions, call provider/model, touch credentials, deploy, move money, or touch trading.';
    const controlPoint = 'James must explicitly review and approve a separate future route/audit promotion before any real promotion, route flip, send, runtime, browser, workbench, UI/status, provider/model, credential, deploy, money, or trading action is proposed.';
    const falseAuditFlags = {
      sendPerformed: false,
      promotionPerformed: false,
      routeFlip: false,
      runtimeExecutes: false,
    ...
    const harness = createRuntimeBootHarness({
      customizeSquidRunContext(context) {
        context.lane.sourceRef = 'architect#400';
        context.git.dirtyCount = 0;
        context.git.statusPreview = [];
        context.dirtyWork.summary = 'Worktree is clean.';
        context.dirtyWork.files = [];
        context.missionControl.answer = [
          'Project/lane: squidrun / architect#400. Display the completed internal route/audit promotion decision-gate proof from local context only.',
          `Next team move: Oracle reviews ${decisionGateProofId}; Builder holds it as local context.`,
          'JAMES ACTION: NONE - Local display-only Mission Control work; no runtime, route, send, provider/model, credential, deploy, money, or external action is needed.',
        ].join('\n');
        context.missionControl.nextTeamMove = `Oracle reviews ${decisionGateProofId}; Builder holds it as local context.`;
        context.missionControl.commandCardAcceptance = {
          id: commandCardId,
          status: 'acceptance_planning_only',
          owner: 'Builder',
          ...
          cardFields: {
            currentLaneWhyItMatters: 'Current lane architect#400 matters because James needs the promotion decision-gate proof visible without terminal logs.',
            whatChangedRecently: 'Committed adb1e649 exposes the internal route/audit promotion decision-gate proof as local planning-only context.',
            builderNextMove: 'Render the completed internal route/audit promotion decision-gate proof from already-loaded /squidrun/context only.',
            oracleNextMove: 'Review the display-only proof surface for no live effects.',
            contextCardStatus: 'Context-card/current dirty-context status: Worktree is clean.',
            jamesActionLine: 'JAMES ACTION: NONE',
            dryRunRoutePlan: {
              target: 'oracle',
              purpose: 'internal route/audit promotion decision-gate no-effect review',
              manualExecutionRequired: true,
              sendPerformed: false,
              summary: 'Dry-run route plan asks Oracle to review the local proof display only; no /turn, POST, route, send, provider/model call, or external send is performed.',
            },
        ...
        context.missionControl.internalRoutePromotionReviewPlan = {
          ...
          sourceEvidence: [{ kind: 'completed_context', summary: `${promotionPlanId} remains completed visible context.` }],
          jamesControlPoint: controlPoint,
          preconditions: ['Worktree is clean.'],
          refusalNoGoConditions: ['Any live route, send, POST, promotion, execution, credential, deploy, money, or trading effect.'],
          audit: {
        ...
        context.missionControl.internalRouteAuditReviewLaneProof = {
          id: auditReviewProofId,
          status: 'planning_only_ready_for_oracle_review',
          owner: 'Builder',
          review: {
            target: 'oracle',
            purpose: 'internal route/audit planning review',
            message: 'Completed internal route/audit review-lane proof remains visible context for the decision-gate proof.',
            body: 'Completed internal route/audit review-lane proof remains visible context for the decision-gate proof.',
            manualExecutionRequired: true,
            runtimeExecutes: false,
            sendPerformed: false,
            promotionPerformed: false,
          ...
          sourceEvidence: [{ kind: 'completed_context', summary: `${auditReviewProofId} remains completed visible context.` }],
          jamesControlPoint: controlPoint,
          preconditions: ['Worktree is clean.'],
          refusalNoGoConditions: ['Any live route, send, POST, promotion, execution, credential, deploy, money, or trading effect.'],
          audit: {
        ...
        context.missionControl.internalRouteAuditPromotionDecisionGateProof = {
          id: decisionGateProofId,
          status: 'planning_only_ready_for_oracle_review',
          owner: 'Builder',
          decisionGate: {
            target: 'oracle',
            ...
            message: proofBody,
            body: proofBody,
            sourceProofId: auditReviewProofId,
            manualExecutionRequired: true,
            runtimeExecutes: false,
            sendPerformed: false,
            promotionPerformed: false,
          ...
          sourceEvidence: [
            {
              kind: 'completed_context',
              summary: `Completed context retained: ${auditReviewProofId} remains planning_only_ready_for_oracle_review.`,
            },
            {
              kind: 'comms',
          ...
          completedContext: {
            ...
            commandCardRoutePlanFollowThroughProofId: routePlanProofId,
            internalRoutePromotionReviewPlanId: promotionPlanId,
            internalRouteAuditReviewLaneProofId: auditReviewProofId,
          },
          jamesControlPoint: controlPoint,
          preconditions: [
            'Worktree is clean.',
        ...
        context.summary.happening = 'Working in squidrun on architect#400: display-only internal route/audit promotion decision-gate proof surface.';
        context.summary.nextStep = `Oracle reviews ${decisionGateProofId}; Builder holds it as local context.`;
        context.summary.commandCardAcceptance = `${commandCardId}: completed visible command-card context.`;
        context.summary.commandCardRoutePlanFollowThroughProof = `${routePlanProofId}: proof_ready_for_oracle_review.`;
        context.summary.internalRoutePromotionReviewPlan = `${promotionPlanId}: planning_only_ready_for_oracle_review.`;
        context.summary.internalRouteAuditReviewLaneProof = `${auditReviewProofId}: planning_only_ready_for_oracle_review.`;
        context.summary.internalRouteAuditPromotionDecisionGateProof = `${decisionGateProofId}: planning_only_ready_for_oracle_review.`;
        context.summary.jamesAction = 'NONE';
        context.summary.jamesActionReason = 'Local display-only Mission Control work; no runtime, route, send, provider/model, credential, deploy, money, or external action is needed.';
        return context;
      },
    ...
    const proofText = harness.elements.internalRouteAuditPromotionDecisionGateProofSummary.textContent;
    const auditReviewText = harness.elements.internalRouteAuditReviewLaneProofSummary.textContent;
    const missionSurfaceText = collectMissionControlText(harness.elements);
    expect(harness.calls.filter((call) => call.method === 'POST')).toHaveLength(0);
    expect(harness.calls.some((call) => call.url === '/turn')).toBe(false);
    expect(harness.calls.filter((call) => call.url === '/squidrun/context')).toEqual([
    ...
    expect(auditReviewText).toContain(`Internal-route audit review-lane proof: ${auditReviewProofId}`);
    expect(proofText).toContain(`Internal-route audit promotion decision-gate proof: ${decisionGateProofId}`);
    expect(proofText).toContain('Status: planning_only_ready_for_oracle_review');
    expect(proofText).toContain('Owner: Builder');
    expect(proofText).toContain(`Completed contexts: ${commandCardId} + ${routePlanProofId} + ${promotionPlanId} + ${auditReviewProofId}`);
    expect(proofText).toContain('Target: oracle');
    expect(proofText).toContain('Purpose: internal route/audit promotion decision-gate review');
    expect(proofText).toContain(`Decision object: ${auditReviewProofId} -> internal route/audit promotion decision-gate review`);
    expect(proofText).toContain(`Message/body: ${proofBody}`);
    expect(proofText).toContain('architect#384 cbeb0d2f');
    expect(proofText).toContain('oracle#15 cbeb0d2f');
    expect(proofText).toContain(`Control point: ${controlPoint}`);
    expect(proofText).toContain('Preconditions: Worktree is clean. | mission-control-internal-route-audit-review-lane-proof-v0 is present as completed visible context. | The proof is read from loaded /squidrun/context only.');
    expect(proofText).toContain('No-go: Dirty worktree or missing completed internal route/audit review-lane proof context. | Any request to start runtime, open browser/workbench, perform UI/status actions, fetch, POST, route, send, promote, flip routes, call provider/model, touch accounts/tokens/credentials/devices/users/external targets, deploy, move money, or touch trading.');
    expect(proofText).toContain('Audit flags: planningOnly=true, manualOnly=true');
    for (const key of Object.keys(falseAuditFlags)) {
      expect(proofText).toContain(`${key}=false`);
    }
    expect(missionSurfaceText).toContain(auditReviewProofId);
    expect(missionSurfaceText).toContain(decisionGateProofId);
    expect((missionSurfaceText.match(/JAMES ACTION:/g) || [])).toHaveLength(1);
    expect(harness.elements.missionAnswer.textContent).not.toContain('JAMES ACTION:');
  });
  ...
  test('answers local-preview availability questions from existing Mission Control context without a turn POST', async () => {
    ...
    for (const question of ['what can I ask Mission Control locally?', 'what local previews are available?']) {
      ...
      expect(availableReply).toContain('Mission answer continuity');
  ...
  test('answers manual-only boundary questions from existing Mission Control status without a turn POST', async () => {
    const appJsPath = path.join(__dirname, '..', '..', 'mira', 'ui', 'app.js');
    const appJs = fs.readFileSync(appJsPath, 'utf8');
    const harness = createRuntimeBootHarness();
    ...
    const postCountBeforeQuestion = harness.calls.filter((call) => call.method === 'POST').length;
    for (const question of ['what remains manual-only in Mission Control?', 'why is Mission Control manual-only?']) {
      harness.elements.turnText.value = question;
      const submitEvent = { preventDefault: jest.fn() };
      await harness.elements.turnForm.listeners.submit(submitEvent);
      ...
      expect(harness.elements.thread.children.slice(-2).map((node) => node.children[0].textContent)).toEqual([
        question,
        expect.stringContaining('Manual-only status: blocked'),
      ]);
      const manualOnlyReply = harness.elements.thread.children[harness.elements.thread.children.length - 1].children[0].textContent;
      expect(manualOnlyReply).toContain('Manual-only summary: No manual advancement is ready from this readout.');
      expect(manualOnlyReply).toContain('Selector summary: blocked: no existing manual action is ready to select.');
      expect(manualOnlyReply).toContain('Manual preflight: blocked no source · No manual action is ready because there is no selected saved artifact with a token and next missing stage.');
      expect(manualOnlyReply).toContain('Manual action: No manual action is ready.');
      expect(manualOnlyReply).toContain('Payload preview: blocked; no method no endpoint; No payload preview is available because the manual action preflight is blocked.');
      expect(manualOnlyReply).toContain('Handler drift: blocked · No workbench handler drift check is available because the manual action preflight is blocked.');
      expect(manualOnlyReply).toContain('Next boundary: Next inspectable step is local review; live send is still unavailable.');
      expect(manualOnlyReply).toContain('Source: already-loaded Mission Control manual-only readout, selector, preflight, payload, and handler-drift status from local SquidRun context.');
      expect(manualOnlyReply).toContain('Boundary: local inspection only; no /turn, fetch, POST, persistence, make manual, manualize, perform, do, click, submit, save, endpoint call, handler call, workbench action, artifact creation, context-carry artifact/stage, Telegram, hm-send, route flip, provider/model call, account/token access, runtime execution, or external send.');
      expect(manualOnlyReply).toContain('JAMES ACTION: NONE - Local dry-run Mission Control work; no account setup needed.');
      expect((manualOnlyReply.match(/JAMES ACTION:/g) || [])).toHaveLength(1);
      expect(harness.elements.lastTurn.textContent).toBe('mission control local');
      expect(harness.elements.sendButton.disabled).toBe(false);
      expect(harness.elements.sendButton.textContent).toBe('Send');
    }
  ...
  test('answers checksum-evidence questions from existing Mission Control status without a turn POST', async () => {
    const appJsPath = path.join(__dirname, '..', '..', 'mira', 'ui', 'app.js');
    const appJs = fs.readFileSync(appJsPath, 'utf8');
    const harness = createRuntimeBootHarness();
    ...
    const postCountBeforeQuestion = harness.calls.filter((call) => call.method === 'POST').length;
    for (const question of ['what checksum backs the Mission Control artifact?', 'what is the Mission Control evidence checksum?']) {
      harness.elements.turnText.value = question;
      const submitEvent = { preventDefault: jest.fn() };
      await harness.elements.turnForm.listeners.submit(submitEvent);
      ...
      expect(harness.elements.thread.children.slice(-2).map((node) => node.children[0].textContent)).toEqual([
        question,
        expect.stringContaining('Checksum evidence ready: no'),
      ]);
      const checksumReply = harness.elements.thread.children[harness.elements.thread.children.length - 1].children[0].textContent;
      expect(checksumReply).toContain('Current stage: No saved Mission Control send chain yet');
      expect(checksumReply).toContain('Artifact token: not available');
      expect(checksumReply).toContain('Artifact path: No saved artifact path is available.');
      expect(checksumReply).toContain('Body checksum: not available');
      expect(checksumReply).toContain('Adapter checksum: not available');
      expect(checksumReply).toContain('Selected checksum snapshot: body not available; adapter not available');
      expect(checksumReply).toContain('Trace checksum trail: none loaded');
      expect(checksumReply).toContain('Body preview: No body preview recorded.');
      expect(checksumReply).toContain('Next boundary: Next inspectable step is local review; live send is still unavailable.');
      expect(checksumReply).toContain('Source: already-loaded Mission Control activation pipeline current-stage trace and advance-selection checksum fields from local SquidRun context.');
  ...
  test('answers Mission-answer continuity questions from existing Mission Control status without a turn POST', async () => {
    const appJsPath = path.join(__dirname, '..', '..', 'mira', 'ui', 'app.js');
    const appJs = fs.readFileSync(appJsPath, 'utf8');
    const harness = createRuntimeBootHarness();
    ...
    const postCountBeforeQuestion = harness.calls.filter((call) => call.method === 'POST').length;
    for (const question of ['does the Mission Control answer carry through?', 'what is the Mission Control answer continuity?']) {
      harness.elements.turnText.value = question;
      const submitEvent = { preventDefault: jest.fn() };
      await harness.elements.turnForm.listeners.submit(submitEvent);
      ...
      expect(harness.elements.thread.children.slice(-2).map((node) => node.children[0].textContent)).toEqual([
        question,
        expect.stringContaining('Answer continuity status: empty'),
      ]);
      const continuityReply = harness.elements.thread.children[harness.elements.thread.children.length - 1].children[0].textContent;
      expect(continuityReply).toContain('Origin answer: No originating Mission Control answer is loaded.');
      expect(continuityReply).toContain('Current answer: No current Mission Control answer is loaded.');
      expect(continuityReply).toContain('Matching stages: 0/0 available; 0 carried.');
      expect(continuityReply).toContain('Missing answer stages: none');
      expect(continuityReply).toContain('Mismatched answer stages: none');
      expect(continuityReply).toContain('Continuity trail: none loaded');
      expect(continuityReply).toContain('Summary: No originating Mission Control answer is attached to the saved activation pipeline yet.');
      expect(continuityReply).toContain('No-effect: Read-only Mission answer continuity proof only; it derives from existing saved Mission Control artifacts/status and does not persist, submit, execute, send, deliver, call a provider/model, access accounts/tokens, flip routes, or start runtime work.');
      expect(continuityReply).toContain('Next boundary: Next inspectable step is local review; live send is still unavailable.');
      expect(continuityReply).toContain('Source: already-loaded Mission Control end-to-end readout Mission answer continuity from local SquidRun context.');
      expect(continuityReply).toContain('Boundary: local inspection only; no /turn, fetch, POST, persistence, carry, copy, fix, update, change, submit, endpoint call, handler call, click, artifact creation, context-carry artifact/stage, Telegram, hm-send, route flip, provider/model call, account/token access, runtime execution, or external send.');
      expect(continuityReply).toContain('JAMES ACTION: NONE - Local dry-run Mission Control work; no account setup needed.');
      expect((continuityReply.match(/JAMES ACTION:/g) || [])).toHaveLength(1);
      expect(harness.elements.lastTurn.textContent).toBe('mission control local');
      expect(harness.elements.sendButton.disabled).toBe(false);
      expect(harness.elements.sendButton.textContent).toBe('Send');
    }
  ...
  test('answers missing-stage questions from existing Mission Control status without a turn POST', async () => {
    const appJsPath = path.join(__dirname, '..', '..', 'mira', 'ui', 'app.js');
    const appJs = fs.readFileSync(appJsPath, 'utf8');
    const harness = createRuntimeBootHarness();
    ...
    const postCountBeforeQuestion = harness.calls.filter((call) => call.method === 'POST').length;
    for (const question of ['what is missing in Mission Control?', 'which Mission Control stage is blocked?']) {
      harness.elements.turnText.value = question;
      const submitEvent = { preventDefault: jest.fn() };
      await harness.elements.turnForm.listeners.submit(submitEvent);
      ...
      expect(harness.elements.thread.children.slice(-2).map((node) => node.children[0].textContent)).toEqual([
        question,
        expect.stringContaining('Missing stages: Route preview / Review item / Owned-work continuation'),
      ]);
      const missingReply = harness.elements.thread.children[harness.elements.thread.children.length - 1].children[0].textContent;
      expect(missingReply).toContain('Available stages: 0/12');
      expect(missingReply).toContain('Advance status: no chain');
      expect(missingReply).toContain('Next missing stage: Route preview');
      expect(missingReply).toContain('Current source stage: No saved artifact');
      expect(missingReply).toContain('Manual preflight: blocked no source · No manual action is ready because there is no selected saved artifact with a token and next missing stage.');
      expect(missingReply).toContain('Next boundary: Next inspectable step is local review; live send is still unavailable.');
      expect(missingReply).toContain('Source: already-loaded Mission Control activation pipeline status/selection from local SquidRun context.');
      expect(missingReply).toContain('Boundary: local inspection only; no /turn, fetch, POST, persistence, click, artifact creation, Telegram, hm-send, route flip, provider/model call, account/token access, runtime execution, or external send.');
      expect(missingReply).toContain('JAMES ACTION: NONE - Local dry-run Mission Control work; no account setup needed.');
      expect((missingReply.match(/JAMES ACTION:/g) || [])).toHaveLength(1);
      expect(harness.elements.lastTurn.textContent).toBe('mission control local');
      expect(harness.elements.sendButton.disabled).toBe(false);
      expect(harness.elements.sendButton.textContent).toBe('Send');
    }
  ...
  test('answers current-stage questions from existing Mission Control status without a turn POST', async () => {
    const appJsPath = path.join(__dirname, '..', '..', 'mira', 'ui', 'app.js');
    const appJs = fs.readFileSync(appJsPath, 'utf8');
    const harness = createRuntimeBootHarness();
    ...
    const postCountBeforeQuestion = harness.calls.filter((call) => call.method === 'POST').length;
    for (const question of ['what stage is Mission Control on?', 'where is Mission Control in the chain?']) {
      harness.elements.turnText.value = question;
      const submitEvent = { preventDefault: jest.fn() };
      await harness.elements.turnForm.listeners.submit(submitEvent);
      ...
      expect(harness.elements.thread.children.slice(-2).map((node) => node.children[0].textContent)).toEqual([
        question,
        expect.stringContaining('Current stage: No Mission Control send chain yet'),
      ]);
      const currentStageReply = harness.elements.thread.children[harness.elements.thread.children.length - 1].children[0].textContent;
      expect(currentStageReply).toContain('Pipeline status: empty');
      expect(currentStageReply).toContain('Stage state: missing');
      expect(currentStageReply).toContain('Artifact token: not available');
      expect(currentStageReply).toContain('Artifact path: No saved artifact path is available.');
      expect(currentStageReply).toContain('Saved stages: 0/12');
      expect(currentStageReply).toContain('Next missing stage: Route preview');
      expect(currentStageReply).toContain('Advance status: no chain');
      expect(currentStageReply).toContain('Hard stop recorded: no');
      expect(currentStageReply).toContain('Next boundary: Next inspectable step is local review; live send is still unavailable.');
      expect(currentStageReply).toContain('Source: already-loaded Mission Control activation pipeline status/current-stage trace from local SquidRun context.');
  ...
  test('answers payload-preview questions from existing Mission Control status without a turn POST', async () => {
    const appJsPath = path.join(__dirname, '..', '..', 'mira', 'ui', 'app.js');
    const appJs = fs.readFileSync(appJsPath, 'utf8');
    const harness = createRuntimeBootHarness();
    ...
    const postCountBeforeQuestion = harness.calls.filter((call) => call.method === 'POST').length;
    for (const question of ['what payload would Mission Control use?', 'which endpoint would Mission Control action call?']) {
      harness.elements.turnText.value = question;
      const submitEvent = { preventDefault: jest.fn() };
      await harness.elements.turnForm.listeners.submit(submitEvent);
      ...
      expect(harness.elements.thread.children.slice(-2).map((node) => node.children[0].textContent)).toEqual([
        question,
        expect.stringContaining('Payload preview status: blocked'),
      ]);
      const payloadReply = harness.elements.thread.children[harness.elements.thread.children.length - 1].children[0].textContent;
      expect(payloadReply).toContain('Action label: No manual action payload is ready.');
      expect(payloadReply).toContain('Method/endpoint: no method no endpoint');
      expect(payloadReply).toContain('Token field/value: no token=not ready');
      expect(payloadReply).toContain('Selected artifact: none · not available');
      expect(payloadReply).toContain('Payload body: not available');
      expect(payloadReply).toContain('Handler drift: blocked · no handler');
      expect(payloadReply).toContain('Validation checks: manual_preflight_ready:blocked / endpoint_known:blocked / selected_token_present:blocked / payload_preview_read_only:ok');
      expect(payloadReply).toContain('Next boundary: Next inspectable step is local review; live send is still unavailable.');
      expect(payloadReply).toContain('Source: already-loaded Mission Control activation pipeline payload preview/manual preflight/handler drift from local SquidRun context.');
      expect(payloadReply).toContain('Boundary: local inspection only; no /turn, fetch, POST, persistence, submit, click, artifact creation, Telegram, hm-send, route flip, provider/model call, account/token access, runtime execution, or external send.');
      expect(payloadReply).toContain('JAMES ACTION: NONE - Local dry-run Mission Control work; no account setup needed.');
      expect((payloadReply.match(/JAMES ACTION:/g) || [])).toHaveLength(1);
      expect(harness.elements.lastTurn.textContent).toBe('mission control local');
      expect(harness.elements.sendButton.disabled).toBe(false);
      expect(harness.elements.sendButton.textContent).toBe('Send');
    }
  ...
  test('answers blocked-reason questions from existing Mission Control status without a turn POST', async () => {
    ...
    const postCountBeforeQuestion = harness.calls.filter((call) => call.method === 'POST').length;
    for (const question of ['why is Mission Control blocked?', "why can't Mission Control advance?"]) {
      harness.elements.turnText.value = question;
      const submitEvent = { preventDefault: jest.fn() };
      await harness.elements.turnForm.listeners.submit(submitEvent);
      ...
      expect(harness.elements.thread.children.slice(-2).map((node) => node.children[0].textContent)).toEqual([
        question,
        expect.stringContaining('Blocked reason: No saved Mission Control route preview exists yet, so there is no artifact to advance.'),
      ]);
      const blockedReply = harness.elements.thread.children[harness.elements.thread.children.length - 1].children[0].textContent;
      expect(blockedReply).toContain('Advance status: no chain');
      expect(blockedReply).toContain('Manual preflight: blocked no source · No manual action is ready because there is no selected saved artifact with a token and next missing stage.');
      expect(blockedReply).toContain('Payload preview: blocked · No payload preview is available because the manual action preflight is blocked.');
      expect(blockedReply).toContain('Handler drift: blocked · No workbench handler drift check is available because the manual action preflight is blocked.');
      expect(blockedReply).toContain('Hard stop: recorded no; live send available no.');
      expect(blockedReply).toContain('Next boundary: Next inspectable step is local review; live send is still unavailable.');
      expect(blockedReply).toContain('Source: already-loaded Mission Control activation pipeline selection/preflight/payload/drift status from local SquidRun context.');
      expect(blockedReply).toContain('Boundary: local inspection only; no /turn, fetch, POST, persistence, submit, click, artifact creation, Telegram, hm-send, route flip, provider/model call, account/token access, runtime execution, or external send.');
      expect(blockedReply).toContain('JAMES ACTION: NONE - Local dry-run Mission Control work; no account setup needed.');
      expect((blockedReply.match(/JAMES ACTION:/g) || [])).toHaveLength(1);
      expect(harness.elements.lastTurn.textContent).toBe('mission control local');
      expect(harness.elements.sendButton.disabled).toBe(false);
      expect(harness.elements.sendButton.textContent).toBe('Send');
    }
  ...
  test('does not hijack generic manual-only action instructions from explicit user submit', async () => {
    const appJsPath = path.join(__dirname, '..', '..', 'mira', 'ui', 'app.js');
    const appJs = fs.readFileSync(appJsPath, 'utf8');
    const harness = createRuntimeBootHarness({ allowTurn: true });
    ...
    harness.elements.useModel.checked = false;
    for (const question of [
      'make Mission Control manual-only with the runtime fixture',
      'manualize the Mission Control action with the runtime fixture',
      'perform the Mission Control manual action with the runtime fixture',
      'do the Mission Control manual action with the runtime fixture',
      'click the Mission Control manual action with the runtime fixture',
      'submit the Mission Control manual action with the runtime fixture',
      'save the New Mira manual boundary with the runtime fixture',
    ]) {
      harness.elements.turnText.value = question;
      const submitEvent = { preventDefault: jest.fn() };
      await harness.elements.turnForm.listeners.submit(submitEvent);
  ...
  test('does not hijack generic checksum or hash instructions from explicit user submit', async () => {
    const appJsPath = path.join(__dirname, '..', '..', 'mira', 'ui', 'app.js');
    const appJs = fs.readFileSync(appJsPath, 'utf8');
    const harness = createRuntimeBootHarness({ allowTurn: true });
    ...
    harness.elements.useModel.checked = false;
    for (const question of [
      'verify the Mission Control checksum with the runtime fixture',
      'read the Mission Control checksum file with the runtime fixture',
  ...
  test('does not hijack generic Mission-answer continuity instructions from explicit user submit', async () => {
    const appJsPath = path.join(__dirname, '..', '..', 'mira', 'ui', 'app.js');
    const appJs = fs.readFileSync(appJsPath, 'utf8');
    const harness = createRuntimeBootHarness({ allowTurn: true });
    ...
    harness.elements.useModel.checked = false;
    for (const question of [
      'carry the Mission Control answer with the runtime fixture',
      'copy the Mission Control answer continuity with the runtime fixture',
      'fix the Mission Control answer continuity with the runtime fixture',
      'update the New Mira answer continuity with the runtime fixture',
      'change the Mission Control answer trail with the runtime fixture',
    ]) {
      harness.elements.turnText.value = question;
      const submitEvent = { preventDefault: jest.fn() };
      await harness.elements.turnForm.listeners.submit(submitEvent);
  ...
  test('does not hijack generic missing-stage instructions from explicit user submit', async () => {
    const appJsPath = path.join(__dirname, '..', '..', 'mira', 'ui', 'app.js');
    const appJs = fs.readFileSync(appJsPath, 'utf8');
    const harness = createRuntimeBootHarness({ allowTurn: true });
    ...
    harness.elements.useModel.checked = false;
    for (const question of [
      'please add the missing Mission Control stage with the runtime fixture',
      'fix the blocked Mission Control stage with the runtime fixture',
    ]) {
      harness.elements.turnText.value = question;
      const submitEvent = { preventDefault: jest.fn() };
      await harness.elements.turnForm.listeners.submit(submitEvent);
  ...
  test('does not hijack generic current-stage instructions from explicit user submit', async () => {
    const appJsPath = path.join(__dirname, '..', '..', 'mira', 'ui', 'app.js');
    const appJs = fs.readFileSync(appJsPath, 'utf8');
    const harness = createRuntimeBootHarness({ allowTurn: true });
    ...
    harness.elements.useModel.checked = false;
    for (const question of [
      'please set the current Mission Control stage with the runtime fixture',
      'update the Mission Control status with the runtime fixture',
  ...
  test('does not hijack generic payload or endpoint instructions from explicit user submit', async () => {
    const appJsPath = path.join(__dirname, '..', '..', 'mira', 'ui', 'app.js');
    const appJs = fs.readFileSync(appJsPath, 'utf8');
    const harness = createRuntimeBootHarness({ allowTurn: true });
    ...
    harness.elements.useModel.checked = false;
    for (const question of [
      'post the Mission Control payload with the runtime fixture',
  ...
  test('does not hijack generic unblock or advance instructions from explicit user submit', async () => {
    ...
    harness.elements.useModel.checked = false;
    for (const question of [
      'unblock Mission Control with the runtime fixture',
