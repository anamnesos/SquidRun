...
describe('hm-send retry behavior', () => {
  ...
  test('target-profile route failure does not fall back to main trigger', async () => {
    ...
    try {
      const result = await runHmSend(
        [
          'builder',
          '(ARCHITECT): should not hit main trigger',
          '--role',
          'architect',
          '--target-profile',
          'trustquote',
  ...
  test('auto-chunks oversized inline agent messages instead of risking silent drop', async () => {
    const sendAttempts = [];
    const { server, port } = await startAckServer(sendAttempts);
    const message = `(TEST #long): ${'A'.repeat(1800)}\n${'B'.repeat(800)}`;

    try {
      const result = await runHmSend(
        ['builder', message, '--role', 'builder', '--timeout', '80', '--retries', '0', '--no-fallback'],
        {
          HM_SEND_PORT: String(port),
          HM_SEND_LONG_MESSAGE_INLINE_LIMIT_BYTES: '500',
          HM_SEND_LONG_MESSAGE_CHUNK_BYTES: '256',
        }
      );

      expect(result.code).toBe(0);
      expect(result.stdout).toContain('Delivered to builder in');
      expect(sendAttempts.length).toBeGreaterThan(1);
      expect(sendAttempts[0].content).toContain('[hm-send chunk 1/');
      expect(sendAttempts[0].messageId).toMatch(/-part01$/);
      expect(sendAttempts.every((attempt) => Buffer.byteLength(attempt.content, 'utf8') < 500)).toBe(true);
      expect(sendAttempts.some((attempt) => attempt.content.includes('BBBB'))).toBe(true);
    } finally {
      await new Promise((resolve) => server.close(resolve));
    }
  });

  test('fails loudly when auto-chunked delivery fails and fallback is disabled', async () => {
    const sendAttempts = [];
    let server;
    ...
    try {
      const result = await runHmSend(
        ['builder', `(TEST #longfail): ${'C'.repeat(1200)}`, '--role', 'builder', '--timeout', '60', '--retries', '0', '--no-fallback'],
        {
          HM_SEND_PORT: String(port),
          HM_SEND_LONG_MESSAGE_INLINE_LIMIT_BYTES: '500',
          HM_SEND_LONG_MESSAGE_CHUNK_BYTES: '256',
      ...
      expect(result.code).toBe(1);
      expect(sendAttempts).toHaveLength(1);
      expect(result.stderr).toContain('Auto-chunked hm-send failed at chunk 1/');
      expect(result.stderr).toContain('No fallback was used');
    } finally {
      await new Promise((resolve) => server.close(resolve));
    }
  });
