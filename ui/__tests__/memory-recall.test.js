...
describe('memory recall broker', () => {
  ...
  test('boosts James direct terminal input with trading and architecture memory terms', () => {
    const query = buildRecallQueryFromMessage('Should we manage the open Hyperliquid position now?', {
      channel: 'user_prompt',
      userIdentity: 'james',
      assumeJamesDirectInput: true,
    });

    expect(query.toLowerCase()).toContain('hyperliquid');
  ...
  test('boosts Korean case entities for Eunbyeol Telegram chat', () => {
  ...
  test('persists audit and merges stubbed backends', async () => {
    ...
    const result = await recall({
      query: 'LINK short Eunbyeol customs',
      auditPath,
      feedbackOps: {
      ...
      backends: {
        queryEvidenceLedger: () => [{
          ...
          excerpt: 'Eunbyeol asked about customs evidence and LINK.',
        ...
        queryTeamMemoryClaims: () => [{
          ...
          title: 'Claim',
          excerpt: 'James wants real runtime proof, not theory.',
  ...
  test('extends recall with existing time-awareness context for session, consultation, and monitor', async () => {
    ...
    expect(block).toContain('Last Hyperliquid check: 8m 16s ago');
  });
  ...
  test('can render a compact injection block with time awareness and only 3 short recall items', () => {
    const compact = formatRecallForInjection({
      ...
      items: [
        ...
        { store: 'memory_search', excerpt: 'Second long excerpt that should also be shortened in compact mode for James.' },
  ...
  test('adds Eunbyeol-specific elapsed-time context when Korean-case recall is active', () => {
    const nowMs = Date.parse('2026-04-03T23:40:00.000Z');
    const timeAwareness = buildTimeAwareness({
      nowMs,
      message: 'Please review Eunbyeol customs evidence.',
      channel: 'telegram',
      chatId: '8754356993',
      commsRows: [{
