...
maybeDescribe('cognitive-memory sleep consolidation', () => {
  ...
  test('filters operational contradiction noise down to substantive user or agent claims', () => {
    ...
    expect(looksLikeOperationalNoiseStatement('The seller sent the package to Gini Qwan as James\'s test-buy alias.')).toBe(false);

    expect(isSubstantiveTeamMemoryClaim({
      owner: 'builder',
      claimType: 'fact',
      status: 'confirmed',
      statement: 'The seller sent the package to Gini Qwan as James\'s test-buy alias.',
    })).toBe(true);
    expect(isSubstantiveTeamMemoryClaim({
      owner: 'system',
      claimType: 'fact',
  ...
  test('queues only substantive team-memory contradictions into the antibody pipeline', async () => {
    ...
    teamMemoryClaims.createClaim({
      statement: 'James said the shipping label, invoice, and Messenger thread are all the same Gini Qwan test-buy trail.',
      owner: 'builder',
      claimType: 'fact',
      session: 'tm-real',
    ...
    teamMemoryClaims.createClaim({
      statement: 'The shipping label belongs to a different buyer and not to the Gini Qwan test buy.',
      owner: 'builder',
      claimType: 'negative',
      session: 'tm-real',
      scopes: ['korean-fraud.purchase-trail'],
