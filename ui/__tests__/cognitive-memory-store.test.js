...
describe('cognitive-memory store and extraction', () => {
  ...
  test('defaults cognitive memory DB to the profile-scoped runtime path', () => {
    ...
    const eunbyeolPath = resolveDefaultCognitiveMemoryDbPath({
      projectRoot: tempDir,
      profileName: 'eunbyeol',
    });

    expect(mainPath).toBe(path.join(tempDir, '.squidrun', 'runtime', 'cognitive-memory.db'));
    expect(eunbyeolPath).toBe(path.join(tempDir, '.squidrun', 'runtime-eunbyeol', 'cognitive-memory.db'));
    expect(resolveDefaultPendingPrPath({
      projectRoot: tempDir,
      profileName: 'eunbyeol',
    })).toBe(path.join(tempDir, '.squidrun', 'memory-eunbyeol', 'pending-pr.json'));
  });
  ...
  test('keeps main and Eunbyeol cognitive writes in separate runtime DBs', () => {
    const mainStore = new CognitiveMemoryStore({
      projectRoot: tempDir,
      profileName: 'main',
    });
    const eunbyeolStore = new CognitiveMemoryStore({
      projectRoot: tempDir,
      profileName: 'eunbyeol',
    });
    ...
    eunbyeolStore.stageMemoryPRs([
      { category: 'fact', statement: 'eunbyeol memory stays in eunbyeol runtime.' },
    ]);

    expect(mainStore.dbPath).toBe(path.join(tempDir, '.squidrun', 'runtime', 'cognitive-memory.db'));
    expect(eunbyeolStore.dbPath).toBe(path.join(tempDir, '.squidrun', 'runtime-eunbyeol', 'cognitive-memory.db'));
    expect(mainStore.pendingPrPath).toBe(path.join(tempDir, '.squidrun', 'memory', 'pending-pr.json'));
    expect(eunbyeolStore.pendingPrPath).toBe(path.join(tempDir, '.squidrun', 'memory-eunbyeol', 'pending-pr.json'));
    expect(mainStore.listPendingPRs({ limit: 10 }).map((row) => row.statement)).toEqual([
      'main memory stays in main runtime.',
    ]);
    expect(eunbyeolStore.listPendingPRs({ limit: 10 }).map((row) => row.statement)).toEqual([
      'eunbyeol memory stays in eunbyeol runtime.',
    ]);
