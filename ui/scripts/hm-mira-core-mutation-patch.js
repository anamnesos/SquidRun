...
const { buildMiraCoreMutationPatch } = require('../modules/mira-core/mutation-patch');
...
function parseArgs(argv = []) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (token === '--pretty') {
      ...
      // Phase 10 is stdout-only. Output-file flags are deliberately ignored.
    }
  }
  return parsed;
}
...
function main(argv = process.argv.slice(2), stdinText = null) {
  ...
  const output = buildMiraCoreMutationPatch({ inputSignals });
