...
const { buildMiraCoreMilestoneReadiness } = require('../modules/mira-core/milestone-readiness');

function parseArgs(argv = []) {
  const parsed = {
    fixturePath: path.resolve(__dirname, '..', '__tests__', 'fixtures', 'mira-core-milestone-readiness-contract.json'),
    pretty: false,
  };
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (token === '--pretty') {
      ...
      // Phase 23 is stdout-only. Output-file flags are deliberately ignored.
...
function main(argv = process.argv.slice(2), stdinText = null) {
  ...
  const output = buildMiraCoreMilestoneReadiness({ inputSignals, contract });
