...
const { buildMiraCorePerception } = require('../modules/mira-core/perception');
...
function parseArgs(argv = []) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (token === '--pretty') {
      ...
      // Phase 11 is stdout-only. Output-file flags are deliberately ignored.
    }
  }
  return parsed;
}
...
function main(argv = process.argv.slice(2), stdinText = null) {
  ...
  const output = buildMiraCorePerception({ inputSignals });
