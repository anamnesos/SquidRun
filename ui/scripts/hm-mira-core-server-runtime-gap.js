...
const { buildMiraCoreServerRuntimeGap } = require('../modules/mira-core/server-runtime-gap');

function parseArgs(argv = []) {
  const parsed = {
    fixturePath: path.resolve(__dirname, '..', '__tests__', 'fixtures', 'mira-core-server-runtime-gap-contract.json'),
    pretty: false,
  };
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (token === '--pretty') {
      ...
      // Phase 14 is stdout-only. Output-file flags are deliberately ignored.
...
function main(argv = process.argv.slice(2), stdinText = null) {
  ...
  const output = buildMiraCoreServerRuntimeGap({ inputSignals, contract });
