...
const { buildMiraCoreServerUpload } = require('../modules/mira-core/server-upload');
...
function parseArgs(argv = []) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (token === '--pretty') {
      ...
      // Phase 7 is stdout-only. Output-file flags are deliberately ignored.
    }
  }
  return parsed;
}
...
function main(argv = process.argv.slice(2), stdinText = null) {
  ...
  const output = buildMiraCoreServerUpload({ inputSignals });
