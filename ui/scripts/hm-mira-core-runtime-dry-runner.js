...
const { buildMiraCoreRuntimeDryRunner } = require('../modules/mira-core/runtime-dry-runner');

function parseArgs(argv = []) {
  const parsed = {
    fixturePath: path.resolve(__dirname, '..', '__tests__', 'fixtures', 'mira-core-runtime-dry-runner-contract.json'),
    pretty: false,
  };
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (token === '--pretty') {
      ...
      // Phase 32 is stdout-only. Output-file flags are deliberately ignored.
...
function main(argv = process.argv.slice(2), stdinText = null) {
  ...
  const output = buildMiraCoreRuntimeDryRunner({ inputSignals, contract });
