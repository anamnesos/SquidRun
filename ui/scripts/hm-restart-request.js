...
function captureHyperliquidSnapshot(projectRoot, enabled = true) {
  if (!enabled) {
    return {
      ok: false,
      skipped: true,
  ...
  const result = spawnSync(process.execPath, [scriptPath, '--json'], {
    ...
    env: {
      ...
      SQUIDRUN_HYPERLIQUID_CALLER: 'restart-request',
    },
    encoding: 'utf8',
    timeout: 45_000,
  });
  ...
  const cached = readCachedHyperliquidSnapshot(projectRoot);
  if (cached) {
    return {
      ...cached,
...
function readCachedHyperliquidSnapshot(projectRoot) {
...
function buildRequest(projectRoot, args) {
  ...
  const hlSnapshot = captureHyperliquidSnapshot(projectRoot, args.captureHl);
