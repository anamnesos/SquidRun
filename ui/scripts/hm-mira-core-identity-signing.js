...
const { buildMiraCoreIdentitySigning } = require('../modules/mira-core/identity-signing');

function parseArgs(argv = []) {
  const parsed = {
    fixturePath: path.resolve(__dirname, '..', '__tests__', 'fixtures', 'mira-core-identity-signing-contract.json'),
    pretty: false,
  };
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (token === '--pretty') {
      ...
      // Phase 15 is stdout-only. Output-file flags are deliberately ignored.
...
function main(argv = process.argv.slice(2), stdinText = null) {
  ...
  const output = buildMiraCoreIdentitySigning({ inputSignals, contract });
