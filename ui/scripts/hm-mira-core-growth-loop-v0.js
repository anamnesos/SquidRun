...
function parseArgs(argv = []) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    } else if (token === '--proposal-id' || token === '--event-id') {
      parsed.inputSignals.proposalId = argv[index + 1] || null;
      index += 1;
    } else if (token.startsWith('--proposal-id=')) {
      parsed.inputSignals.proposalId = token.slice('--proposal-id='.length);
    } else if (token.startsWith('--event-id=')) {
      parsed.inputSignals.proposalId = token.slice('--event-id='.length);
    } else if (token === '--out') {
      index += 1;
