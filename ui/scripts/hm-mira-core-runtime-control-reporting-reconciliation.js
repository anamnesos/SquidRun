...
function parseArgs(argv = []) {
  const parsed = {
    fixturePath: path.resolve(
      ...
      '__tests__',
      'fixtures',
      'mira-core-runtime-control-reporting-reconciliation-contract.json',
    ),
    pretty: false,
  };
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (token === '--pretty') {
      ...
      // Phase 37 is stdout-only. Output-file flags are deliberately ignored.
...
function main(argv = process.argv.slice(2), stdinText = null) {
  ...
  const output = buildMiraCoreRuntimeControlReportingReconciliation({ inputSignals, contract });
