...
function parseArgs(argv = []) {
  const parsed = {
    fixturePath: path.resolve(
      ...
      '__tests__',
      'fixtures',
      'mira-core-kill-switch-wiring-readiness-rollup-through-phase63-contract.json',
    ),
    pretty: false,
  };
...
function main(argv = process.argv.slice(2), stdinText = null) {
  ...
  const output = buildMiraCoreKillSwitchWiringReadinessRollupThroughPhase64({ inputSignals, contract });
