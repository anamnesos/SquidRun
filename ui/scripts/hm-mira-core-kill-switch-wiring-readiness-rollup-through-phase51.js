...
function parseArgs(argv = []) {
  const parsed = {
    fixturePath: path.resolve(
      ...
      '__tests__',
      'fixtures',
      'mira-core-kill-switch-wiring-readiness-rollup-through-phase51-contract.json',
    ),
    pretty: false,
  };
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (token === '--pretty') {
      ...
      // Phase 51 is stdout-only. Output-file flags are deliberately ignored.
...
function main(argv = process.argv.slice(2), stdinText = null) {
  ...
  const output = buildMiraCoreKillSwitchWiringReadinessRollupThroughPhase51({ inputSignals, contract });
