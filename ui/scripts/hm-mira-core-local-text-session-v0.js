...
function parseArgs(argv = []) {
  const parsed = {
    fixturePath: path.resolve(
      ...
      '__tests__',
      'fixtures',
      'mira-core-local-text-session-v0-contract.json',
    ),
    pretty: false,
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    } else if (token === '--source-scope') {
      parsed.inputSignals.sourceScope = argv[index + 1] || null;
      index += 1;
    } else if (token.startsWith('--source-scope=')) {
      parsed.inputSignals.sourceScope = token.slice('--source-scope='.length);
    ...
    } else if (token === '--text' || token === '--message') {
      parsed.inputSignals.text = argv[index + 1] || '';
      index += 1;
    } else if (token.startsWith('--text=')) {
      parsed.inputSignals.text = token.slice('--text='.length);
    } else if (token.startsWith('--message=')) {
      parsed.inputSignals.text = token.slice('--message='.length);
    } else if (token === '--now') {
...
function parseStdinSignals(inputText = '') {
...
function main(argv = process.argv.slice(2), stdinText = null) {
  ...
  const stdinSignals = parseStdinSignals(inputText);
  const contract = JSON.parse(fs.readFileSync(path.resolve(args.fixturePath), 'utf8'));
  const output = buildMiraCoreLocalTextSessionV0({
    contract,
