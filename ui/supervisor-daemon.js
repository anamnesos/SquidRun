  ...
  async runLiveOpsPhase(event) {
    ...
    try {
      if (phaseKey === 'polymarket_scan') {
        const markets = await this.getLiveOpsCandidateMarkets(event, { forceRefresh: true });
        const result = {
          ok: true,
          phase: phaseKey,
          marketDate: event.marketDate || '',
      ...
      if (phaseKey === 'polymarket_consensus') {
        const consensusPayload = await this.buildLiveOpsConsensus(event, { forceRefresh: true });
        const result = {
          ok: true,
          phase: phaseKey,
          marketDate: event.marketDate || '',
      ...
      if (phaseKey === 'polymarket_execute') {
        const consensusPayload = await this.buildLiveOpsConsensus(event);
        const bankrollSnapshot = await this.getLiveOpsBankrollSnapshot(new Date(scheduledAt));
