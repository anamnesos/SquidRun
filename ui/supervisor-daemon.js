...
// Eunbyeol profile MUST NOT spawn wallet-touching trading lanes. The dotenv.config
// call above uses override:true and would clobber any pre-set process.env values
// that ui/profile.js applyProfileEnv tried to inject. So we re-apply the eunbyeol
// disable AFTER the .env load. Belt + suspenders: kill all SQUIDRUN_* trading flags
// AND blank the HL credentials so even if a lane slipped through, hasHyperliquidCredentials returns false.
if (String(process.env.SQUIDRUN_PROFILE || '').toLowerCase() === 'eunbyeol') {
  ...
  process.env.SQUIDRUN_HYPERLIQUID_MONITOR = '0';
...
const DEFAULT_HYPERLIQUID_SQUEEZE_DETECTOR_INTERVAL_MS = Math.max(
  30_000,
...
const DEFAULT_HYPERLIQUID_MONITOR_POLL_MS = Math.max(
  60_000,
  Number.parseInt(process.env.SQUIDRUN_HYPERLIQUID_MONITOR_POLL_MS || '300000', 10) || 300_000
);
...
const MARKET_SCANNER_PHASES = Object.freeze([
  { key: 'market_scanner', label: 'Hyperliquid market scanner' },
]);
const EUNBYEOL_CHECKIN_PHASES = Object.freeze([
  { key: 'eunbyeol_checkin', label: 'Eunbyeol check-in review' },
]);
const DEFAULT_PENDING_FOLLOWUP_INTERVAL_MINUTES = 4 * 60;
const DEFAULT_MARKET_RESEARCH_INTERVAL_MINUTES = 4 * 60;
const DEFAULT_TOKENOMIST_INTERVAL_MINUTES = 6 * 60;
...
function resolveHyperliquidWalletAddress(env = process.env) {
  return String(
...
function normalizeHyperliquidKillSwitchAction(value, fallback = 'block_new_entries') {
...
function hasHyperliquidCredentials(env = process.env) {
  return Boolean(
    ...
    && resolveHyperliquidWalletAddress(env)
  );
}
...
function formatHyperliquidRetainedRatio(value) {
  const numeric = Number(value);
  if (!Number.isFinite(numeric)) return 'n/a';
  return `${(numeric * 100).toFixed(0)}%`;
}

function buildHyperliquidPaneAlert(position = {}, level = 'warning') {
  const label = String(level || 'warning').trim().toUpperCase();
  const side = String(position?.side || '').trim().toUpperCase() || 'POSITION';
  return `[${label}] Hyperliquid ${position.coin} ${side} unrealized P&L is now $${toNumber(position?.unrealizedPnl, 0).toFixed(2)} vs peak $${toNumber(position?.peakUnrealizedPnl, 0).toFixed(2)} (${formatHyperliquidRetainedRatio(position?.retainedPeakRatio)} retained). Review the live position now.`;
}
...
function createHyperliquidExecutor(options = {}) {
  ...
  return {
    async getAccountState() {
      const walletAddress = resolveHyperliquidWalletAddress(env);
      if (!walletAddress) {
      ...
      return {
        ...
        positions: (Array.isArray(positions) ? positions : [])
          .map((position) => normalizeHyperliquidPosition({
            ...(position?.raw || {}),
...
function defaultEunbyeolCheckInState() {
  return {
    lastProcessedAt: null,
    lastResult: null,
    nextEvent: null,
...
function buildTokenomistDailySchedule(referenceDate = new Date(), options = {}) {
  return buildUtcIntervalSchedule(referenceDate, [{ key: 'tokenomist_scan', label: 'Token unlock scan' }], options.intervalMinutes || DEFAULT_TOKENOMIST_INTERVAL_MINUTES);
}

function getNextTokenomistEvent(referenceDate = new Date(), options = {}) {
  const now = referenceDate instanceof Date ? referenceDate : new Date(referenceDate);
  for (let offset = 0; offset < 3; offset += 1) {
    const candidateDate = new Date(now.getTime() + (offset * 24 * 60 * 60 * 1000));
    const day = buildTokenomistDailySchedule(candidateDate, options);
    const nextEvent = day.schedule.find((event) => new Date(event.scheduledAt).getTime() > now.getTime());
    if (nextEvent) {
      return {
        ...nextEvent,
...
function getLatestEunbyeolMessageTimestamp(rows = []) {
  for (const row of Array.isArray(rows) ? rows : []) {
    ...
    if (
      chatId === '8754356993'
      || /은별|eunbyeol/i.test(sender)
      || /은별|eunbyeol|8754356993/i.test(rawBody)
    ) {
...
function isEunbyeolOwnedPendingItem(item = {}) {
  const blockedOn = String(item?.blockedOn || '');
  const label = `${item?.item || ''} ${item?.caseLabel || ''} ${item?.status || ''}`;
  return /은별 input|은별 action|은별|eunbyeol/i.test(blockedOn)
    || /은별|eunbyeol/i.test(label);
}

function isJamesOwnedPendingItem(item = {}) {
  const blockedOn = String(item?.blockedOn || '');
  const label = `${item?.item || ''} ${item?.caseLabel || ''} ${item?.status || ''}`;
  return /james\b|james action|builder|architect|oracle/i.test(blockedOn)
    || /\bJames\b|미국 filings|US filings|test buy/i.test(label);
}
...
function buildEunbyeolCheckInDailySchedule(referenceDate = new Date(), options = {}) {
...
function getNextEunbyeolCheckInEvent(referenceDate = new Date(), options = {}) {
...
class SupervisorDaemon {
  constructor(options = {}) {
    ...
    this.hyperliquidManualActivityPath = options.hyperliquidManualActivityPath || DEFAULT_HYPERLIQUID_MANUAL_ACTIVITY_PATH;
    ...
    this.hmHyperliquidSqueezeDetectorScriptPath = path.resolve(String(options.hyperliquidSqueezeDetectorScriptPath || resolveProjectUiScriptPath('hm-hyperliquid-squeeze-detector.js', this.projectRoot)));
    ...
    // Previously hardcoded to true → produced manualOnly=true, hyperliquidExecution.enabled=false,
    ...
    this.manualTradingOnly = options.manualTradingOnly === false
      ...
      ? false
      : true;
    this.hyperliquidScalpModeArmed = options.hyperliquidScalpModeArmed === true
      || this.runtimeEnv.SQUIDRUN_HYPERLIQUID_SCALP_MODE === '1';
    ...
    this.hyperliquidSqueezeDetectorEnabled = options.hyperliquidSqueezeDetectorEnabled !== false
      && this.runtimeEnv.SQUIDRUN_HYPERLIQUID_SQUEEZE_DETECTOR !== '0';
    this.hyperliquidSqueezeDetectorIntervalMs = Math.max(
      30_000,
      Number.parseInt(
        options.hyperliquidSqueezeDetectorIntervalMs || DEFAULT_HYPERLIQUID_SQUEEZE_DETECTOR_INTERVAL_MS,
        10
      ) || DEFAULT_HYPERLIQUID_SQUEEZE_DETECTOR_INTERVAL_MS
    );
    this.lastHyperliquidSqueezeDetectorRunAtMs = 0;
    this.hyperliquidSqueezeDetectorPromise = null;
    this.lastHyperliquidSqueezeDetectorSummary = this.hyperliquidSqueezeDetectorEnabled
      ? {
        enabled: true,
        status: 'idle',
        intervalMs: this.hyperliquidSqueezeDetectorIntervalMs,
        lastRunAt: null,
        lastSummary: null,
      }
      : {
    ...
    this.hyperliquidMonitorPollMs = Math.max(
      60_000,
      Number.parseInt(options.hyperliquidMonitorPollMs || DEFAULT_HYPERLIQUID_MONITOR_POLL_MS, 10) || DEFAULT_HYPERLIQUID_MONITOR_POLL_MS
    );
    this.hyperliquidMonitorEnabled = options.hyperliquidMonitorEnabled !== false && hyperliquidConfigured;
    this.hyperliquidMonitorTimer = null;
    this.hyperliquidMonitorPromise = null;
    this.defiPeakPnlPath = options.defiPeakPnlPath || DEFAULT_DEFI_PEAK_PNL_PATH;
    this.hyperliquidMonitorOrchestrator = this.hyperliquidMonitorEnabled
      ? (options.hyperliquidMonitorOrchestrator || tradingOrchestrator.createOrchestrator({
        defiMonitorAutoStart: false,
        defiPeakPnlPath: this.defiPeakPnlPath,
        defiStatusScriptPath: this.hmDefiStatusScriptPath,
      }))
      : null;
    this.lastHyperliquidMonitorSummary = this.hyperliquidMonitorEnabled
      ? {
        enabled: true,
        status: 'idle',
        pollMs: this.hyperliquidMonitorPollMs,
        checkedAt: null,
        warnings: [],
        telegramAlerts: [],
      }
      : {
        enabled: false,
        status: hyperliquidConfigured ? 'manual_opt_out' : 'credentials_unavailable',
        pollMs: this.hyperliquidMonitorPollMs,
        checkedAt: null,
        warnings: [],
        telegramAlerts: [],
    ...
    this.positionAttributionReconciliationEnabled = positionAttributionReconciliationRequested
      && (Boolean(resolveHyperliquidWalletAddress(this.runtimeEnv)) || Boolean(this.positionAttributionSnapshotProvider));
    this.lastPositionAttributionReconciliationSummary = this.positionAttributionReconciliationEnabled
      ...
      : {
        enabled: false,
        status: resolveHyperliquidWalletAddress(this.runtimeEnv) ? 'manual_opt_out' : 'wallet_unavailable',
        statePath: this.positionAttributionStatePath,
        checkedAt: null,
        liveCount: 0,
        updatedCount: 0,
    ...
    this.hyperliquidExecutionLeverage = Math.max(
      1,
      Number.parseInt(
        options.hyperliquidExecutionLeverage
        || this.runtimeEnv.SQUIDRUN_HYPERLIQUID_DEFAULT_LEVERAGE
        || (this.hyperliquidScalpModeArmed ? '20' : '5'),
        10
      ) || (this.hyperliquidScalpModeArmed ? 20 : 5)
    );
    this.hyperliquidKillSwitchAction = normalizeHyperliquidKillSwitchAction(
      options.hyperliquidKillSwitchAction || this.runtimeEnv.SQUIDRUN_HYPERLIQUID_KILL_SWITCH_ACTION,
      'block_new_entries'
    );
    this.hyperliquidExecutor = this.hyperliquidExecutionEnabled
      ...
      : null;
    this.lastHyperliquidExecutionSummary = (this.manualTradingOnly && !this.hyperliquidScalpModeArmed)
      ? {
        enabled: false,
        status: 'manual_only',
        dryRun: this.hyperliquidExecutionDryRun,
      ...
      : this.hyperliquidExecutionEnabled
        ? {
          enabled: true,
          status: 'idle',
          armed: this.hyperliquidScalpModeArmed,
          dryRun: this.hyperliquidExecutionDryRun,
          accountValue: null,
          position: null,
          action: null,
    ...
    this.tokenomistEnabled = options.tokenomistEnabled === true
      || (
        options.tokenomistEnabled !== false
        && RUNNING_SUPERVISOR_MAIN
        && process.env.SQUIDRUN_TOKENOMIST_AUTOMATION !== '0'
      );
    this.tokenomistIntervalMinutes = Math.max(
      60,
      Math.floor(Number(options.tokenomistIntervalMinutes) || DEFAULT_TOKENOMIST_INTERVAL_MINUTES)
    );
    this.tokenomistStatePath = options.tokenomistStatePath || DEFAULT_TOKENOMIST_STATE_PATH;
    this.tokenomistState = {
      ...defaultTokenomistState(),
      ...(readJsonFile(this.tokenomistStatePath, defaultTokenomistState()) || {}),
    };
    this.tokenomistPhasePromise = null;
    this.lastTokenomistSummary = this.tokenomistEnabled
      ? {
        enabled: true,
        status: 'idle',
        intervalMinutes: this.tokenomistIntervalMinutes,
        lastProcessedAt: this.tokenomistState.lastProcessedAt || null,
        nextEvent: this.tokenomistState.nextEvent || null,
      }
      : {
        enabled: false,
        status: 'disabled',
    ...
    this.eunbyeolCheckInEnabled = options.eunbyeolCheckInEnabled !== false
      && process.env.SQUIDRUN_EUNBYEOL_CHECKIN_AUTOMATION !== '0';
    this.eunbyeolCheckInIntervalMinutes = Math.max(
      30,
      Math.floor(Number(options.eunbyeolCheckInIntervalMinutes) || DEFAULT_EUNBYEOL_CHECKIN_INTERVAL_MINUTES)
    );
    this.eunbyeolCheckInSilenceMs = Math.max(
      60_000,
      Number.parseInt(options.eunbyeolCheckInSilenceMs || `${DEFAULT_EUNBYEOL_CHECKIN_SILENCE_MS}`, 10) || DEFAULT_EUNBYEOL_CHECKIN_SILENCE_MS
    );
    this.eunbyeolCheckInStatePath = options.eunbyeolCheckInStatePath || DEFAULT_EUNBYEOL_CHECKIN_STATE_PATH;
    this.eunbyeolCheckInState = {
      ...defaultEunbyeolCheckInState(),
      ...(readJsonFile(this.eunbyeolCheckInStatePath, defaultEunbyeolCheckInState()) || {}),
    };
    this.eunbyeolCheckInPhasePromise = null;
    this.lastEunbyeolCheckInSummary = this.eunbyeolCheckInEnabled
      ? {
        enabled: true,
        status: 'idle',
        intervalMinutes: this.eunbyeolCheckInIntervalMinutes,
        lastProcessedAt: this.eunbyeolCheckInState.lastProcessedAt || null,
        nextEvent: this.eunbyeolCheckInState.nextEvent || null,
      }
      : {
        enabled: false,
        status: 'disabled',
  ...
  start() {
    ...
    this.startHyperliquidPositionMonitor();
  ...
  persistTokenomistState() {
    this.tokenomistState.updatedAt = new Date().toISOString();
    writeJsonFile(this.tokenomistStatePath, this.tokenomistState);
  }
  ...
  buildPendingFollowupTelegramAlert(summary = {}) {
    return [
      ...
      `James-actionable items: ${Number(summary.jamesActionCount || 0)}`,
  ...
  buildPendingFollowupArchitectAlert(summary = {}) {
    return [
      ...
      summary.jamesTopItems?.length ? `James items: ${summary.jamesTopItems.join(' | ')}` : null,
      summary.eunbyeolTopItems?.length ? `Internal items: ${summary.eunbyeolTopItems.join(' | ')}` : null,
    ].filter(Boolean).join('\n');
  }

  buildEunbyeolCheckInDraft(summary = {}) {
  ...
  buildMarketScannerArchitectAlert(summary = {}) {
    ...
    return [
      '[PROACTIVE][MARKET] Hyperliquid full-universe scan found movers.',
  ...
  async maybeRunHyperliquidSqueezeDetector(nowMs = Date.now()) {
    if (!this.hyperliquidSqueezeDetectorEnabled || this.stopping) {
      return { ok: false, skipped: true, reason: 'hyperliquid_squeeze_detector_disabled' };
    }
    const manualActivity = this.getActiveHyperliquidManualActivity(nowMs);
    if (manualActivity) {
      this.lastHyperliquidSqueezeDetectorSummary = {
        enabled: true,
    ...
    if (this.hyperliquidSqueezeDetectorPromise) {
      return this.hyperliquidSqueezeDetectorPromise;
    }
    if ((nowMs - this.lastHyperliquidSqueezeDetectorRunAtMs) < this.hyperliquidSqueezeDetectorIntervalMs) {
      return {
        ok: false,
        skipped: true,
        reason: 'hyperliquid_squeeze_detector_cooldown',
        nextEligibleAtMs: this.lastHyperliquidSqueezeDetectorRunAtMs + this.hyperliquidSqueezeDetectorIntervalMs,
      };
    }

    this.hyperliquidSqueezeDetectorPromise = executeNodeScript(
      this.hmHyperliquidSqueezeDetectorScriptPath,
      ['--json'],
      {
        cwd: this.projectRoot,
        env: this.runtimeEnv,
      ...
      const summary = this.parseWatcherScriptSummary(result, 'hyperliquid_squeeze_detector');
      this.lastHyperliquidSqueezeDetectorRunAtMs = Date.now();
      this.lastHyperliquidSqueezeDetectorSummary = {
        enabled: true,
        status: summary?.ok ? (summary?.alerted ? 'alert_sent' : 'ok') : 'failed',
        intervalMs: this.hyperliquidSqueezeDetectorIntervalMs,
        lastRunAt: new Date(this.lastHyperliquidSqueezeDetectorRunAtMs).toISOString(),
        lastSummary: summary,
      };
      if (summary?.ok !== true) {
        this.logger.warn(`Hyperliquid squeeze detector failed: ${summary?.error || 'unknown'}`);
      } else if (Number(summary?.detectionCount || 0) > 0) {
        this.logger.warn(`Hyperliquid squeeze detector flagged ${summary.detectionCount} setup(s).`);
      }
      return summary;
    }).finally(() => {
      this.hyperliquidSqueezeDetectorPromise = null;
  ...
  buildEunbyeolCheckInArchitectAlert(summary = {}) {
    return [
      '[PROACTIVE][EUNBYEOL] Check-in review complete.',
      `Pending Eunbyeol items: ${Number(summary.pendingCount || 0)}`,
  ...
  async runPendingFollowupPhase(event) {
    ...
    const eunbyeolOwnedItems = pendingItems.filter((item) => isEunbyeolOwnedPendingItem(item));
    const jamesOwnedItems = pendingItems.filter((item) => isJamesOwnedPendingItem(item));
    const internalOnlyItems = pendingItems.filter((item) => !isJamesOwnedPendingItem(item));
    const jamesActionableItems = Array.from(new Set([
      ...jamesOwnedItems,
      ...blockedItems.filter((item) => isJamesOwnedPendingItem(item)),
      ...waitingItems.filter((item) => isJamesOwnedPendingItem(item)),
    ]));
    const overdueEunbyeolSchedule = Boolean(dashboard.scheduleDate && (dashboard.scheduleItems || []).length > 0);
    const topItems = [
      ...waitingItems.slice(0, 2).map((item) => `[WAITING] ${item.item}`),
      ...blockedItems.slice(0, 2).map((item) => `[BLOCKED] ${item.item}`),
      ...(overdueEunbyeolSchedule ? (dashboard.scheduleItems || []).slice(0, 2).map((item) => `[EUNBYEOL] ${item}`) : []),
    ].slice(0, 4);
    const jamesTopItems = jamesActionableItems
      .slice(0, 4)
      .map((item) => `[ACTION] ${item.item}`);
    const eunbyeolTopItems = [
      ...eunbyeolOwnedItems.slice(0, 3).map((item) => item.item),
      ...(overdueEunbyeolSchedule ? (dashboard.scheduleItems || []).slice(0, 2) : []),
    ].slice(0, 4);
    ...
    const summary = {
      ...
      overdueEunbyeolSchedule,
      scheduleDate: dashboard.scheduleDate || null,
      topItems,
      jamesActionCount: jamesActionableItems.length,
      jamesTopItems,
      eunbyeolActionCount: eunbyeolOwnedItems.length + (overdueEunbyeolSchedule ? (dashboard.scheduleItems || []).length : 0),
      eunbyeolTopItems,
      internalOnlyCount: internalOnlyItems.length + (overdueEunbyeolSchedule ? (dashboard.scheduleItems || []).length : 0),
      alertLevel,
      notified: shouldNotifyArchitect,
    };
  ...
  async runTokenomistPhase(event) {
    const executedAt = new Date().toISOString();
    const result = await executeNodeScript(this.hmTokenomistUnlocksScriptPath, ['--json', '--hours', '48'], {
      cwd: this.projectRoot,
      timeoutMs: 30_000,
      env: this.runtimeEnv,
    });
    if (!result?.ok) {
      return {
        ok: false,
        key: event?.key || 'tokenomist_scan',
        executedAt,
        error: result?.error || result?.stderr || 'tokenomist_scan_failed',
    ...
    try {
      ...
      return {
        ...
        key: event?.key || 'tokenomist_scan',
        executedAt,
        error: `tokenomist_json_parse_failed:${error.message}`,
        stdout: result.stdout || '',
      };
    }
    return {
  ...
  async maybeRunTokenomistAutomation(nowMs = Date.now()) {
    if (!this.tokenomistEnabled || this.stopping) {
      return { ok: false, skipped: true, reason: 'tokenomist_disabled' };
    }
    if (this.tokenomistPhasePromise) {
      return this.tokenomistPhasePromise;
    }

    const now = nowMs instanceof Date ? nowMs : new Date(nowMs);
    const nextEvent = getNextTokenomistEvent(now, {
      intervalMinutes: this.tokenomistIntervalMinutes,
    });
    const day = buildTokenomistDailySchedule(now, {
      intervalMinutes: this.tokenomistIntervalMinutes,
    });
    const lastProcessedAtMs = this.tokenomistState.lastProcessedAt
      ? new Date(this.tokenomistState.lastProcessedAt).getTime()
      : 0;
    const dueEvents = day.schedule.filter((event) => {
      const scheduledAtMs = new Date(event.scheduledAt).getTime();
      return scheduledAtMs <= now.getTime() && scheduledAtMs > lastProcessedAtMs;
    ...
    this.tokenomistState.nextEvent = this.describeTradingEvent(nextEvent);
    this.persistTokenomistState();

    if (dueEvents.length === 0) {
      this.lastTokenomistSummary = {
        enabled: true,
        status: 'scheduled',
        intervalMinutes: this.tokenomistIntervalMinutes,
        lastProcessedAt: this.tokenomistState.lastProcessedAt || null,
        nextEvent: this.tokenomistState.nextEvent,
      };
      return { ok: false, skipped: true, reason: 'no_due_tokenomist_phase', nextEvent: this.tokenomistState.nextEvent };
    }

    this.tokenomistPhasePromise = (async () => {
      const executed = [];
      for (const dueEvent of dueEvents) {
        const phaseResult = await this.runTokenomistPhase(dueEvent);
        executed.push(phaseResult);
        this.tokenomistState.lastProcessedAt = dueEvent.scheduledAt;
        this.tokenomistState.lastResult = phaseResult;
        this.tokenomistState.lastScan = phaseResult;
        this.persistTokenomistState();
      }

      const upcomingEvent = getNextTokenomistEvent(new Date(), {
        intervalMinutes: this.tokenomistIntervalMinutes,
      });
      this.tokenomistState.nextEvent = this.describeTradingEvent(upcomingEvent);
      this.persistTokenomistState();
      this.lastTokenomistSummary = {
        enabled: true,
      ...
      return { ok: executed.every((entry) => entry.ok), skipped: false, executed, nextEvent: this.tokenomistState.nextEvent };
    })().finally(() => {
      this.tokenomistPhasePromise = null;
    });
  ...
  async maybeRunSparkAutomation(nowMs = Date.now()) {
    ...
    this.sparkMonitorPhasePromise = (async () => {
      ...
      const summary = {
        ...
        hyperliquidListingCount: Number(phaseResult?.hyperliquidListingCount || 0),
  ...
  async runEunbyeolCheckInPhase(event) {
    const scannedAt = new Date().toISOString();
    const dashboard = this.readCaseOperationsDashboard();
    const pendingItems = Array.isArray(dashboard.pendingItems) ? dashboard.pendingItems : [];
    const eunbyeolItems = pendingItems.filter((item) => /은별 input|은별 action|은별/i.test(String(item.blockedOn || '')));
    const commsRows = await Promise.resolve(this.proactiveCommsProvider()).catch(() => []);
    const lastMessageAtMs = getLatestEunbyeolMessageTimestamp(commsRows);
    const silenceMs = lastMessageAtMs > 0 ? Math.max(0, Date.parse(scannedAt) - lastMessageAtMs) : Number.POSITIVE_INFINITY;
    const shouldDraft = eunbyeolItems.length > 0 && silenceMs >= this.eunbyeolCheckInSilenceMs;
    const topItems = eunbyeolItems.slice(0, 4).map((item) => item.item);
    const draft = shouldDraft
      ? this.buildEunbyeolCheckInDraft({ topItems })
      : null;
    const fingerprint = draft ? JSON.stringify({ topItems, lastMessageAtMs }) : null;
    const summary = {
      ok: true,
      key: event?.key || 'eunbyeol_checkin',
      scannedAt,
      pendingCount: eunbyeolItems.length,
    ...
    if (fingerprint) {
      this.eunbyeolCheckInState.lastDraftFingerprint = fingerprint;
      this.eunbyeolCheckInState.lastDraft = {
        createdAt: scannedAt,
        message: draft,
        topItems,
      };
      this.notifyArchitectInternal(this.buildEunbyeolCheckInArchitectAlert(summary), 'eunbyeol_checkin');
      summary.notified = true;
    }
    return summary;
  }

  async maybeRunEunbyeolCheckInAutomation(nowMs = Date.now()) {
    if (!this.eunbyeolCheckInEnabled || this.stopping) {
      return { ok: false, skipped: true, reason: 'eunbyeol_checkin_disabled' };
    }
    if (this.eunbyeolCheckInPhasePromise) {
      return this.eunbyeolCheckInPhasePromise;
    }

    const now = nowMs instanceof Date ? nowMs : new Date(nowMs);
    const nextEvent = getNextEunbyeolCheckInEvent(now, {
      intervalMinutes: this.eunbyeolCheckInIntervalMinutes,
    });
    const day = buildEunbyeolCheckInDailySchedule(now, {
      intervalMinutes: this.eunbyeolCheckInIntervalMinutes,
    });
    const lastProcessedAtMs = this.eunbyeolCheckInState.lastProcessedAt
      ? new Date(this.eunbyeolCheckInState.lastProcessedAt).getTime()
      : 0;
    const dueEvents = day.schedule.filter((event) => {
      const scheduledAtMs = new Date(event.scheduledAt).getTime();
      return scheduledAtMs <= now.getTime() && scheduledAtMs > lastProcessedAtMs;
    ...
    this.eunbyeolCheckInState.nextEvent = this.describeTradingEvent(nextEvent);
    this.persistEunbyeolCheckInState();

    if (dueEvents.length === 0) {
      this.lastEunbyeolCheckInSummary = {
        enabled: true,
        status: 'scheduled',
        intervalMinutes: this.eunbyeolCheckInIntervalMinutes,
        lastProcessedAt: this.eunbyeolCheckInState.lastProcessedAt || null,
        nextEvent: this.eunbyeolCheckInState.nextEvent,
      };
      return { ok: false, skipped: true, reason: 'no_due_eunbyeol_checkin', nextEvent: this.eunbyeolCheckInState.nextEvent };
    }

    this.eunbyeolCheckInPhasePromise = (async () => {
      const executed = [];
      for (const dueEvent of dueEvents) {
        const phaseResult = await this.runEunbyeolCheckInPhase(dueEvent);
        executed.push(phaseResult);
        this.eunbyeolCheckInState.lastProcessedAt = dueEvent.scheduledAt;
        this.eunbyeolCheckInState.lastResult = phaseResult;
        this.persistEunbyeolCheckInState();
      }

      const upcomingEvent = getNextEunbyeolCheckInEvent(new Date(), {
        intervalMinutes: this.eunbyeolCheckInIntervalMinutes,
      });
      this.eunbyeolCheckInState.nextEvent = this.describeTradingEvent(upcomingEvent);
      this.persistEunbyeolCheckInState();
      this.lastEunbyeolCheckInSummary = {
        enabled: true,
      ...
      return { ok: true, skipped: false, executed, nextEvent: this.eunbyeolCheckInState.nextEvent };
    })().finally(() => {
      this.eunbyeolCheckInPhasePromise = null;
    });
  ...
  async runHyperliquidPositionMonitorCycle(trigger = 'manual') {
    if (!this.hyperliquidMonitorEnabled || !this.hyperliquidMonitorOrchestrator) {
      return {
        enabled: false,
        status: 'disabled',
        trigger,
    ...
    const manualActivity = this.getActiveHyperliquidManualActivity();
    if (manualActivity) {
      const pausedResult = {
        ok: true,
        skipped: true,
      ...
      this.lastHyperliquidMonitorSummary = {
        enabled: true,
        status: 'paused_for_manual_activity',
        trigger,
        pollMs: this.hyperliquidMonitorPollMs,
    ...
    const result = await this.hyperliquidMonitorOrchestrator.runDefiMonitorCycle({
      trigger,
      sendTelegram: false,
    });
    const riskExit = trigger === 'startup'
      ? { attempted: false, reason: 'startup_grace', executions: [] }
      : await this.maybeExecuteHyperliquidRiskExit(result, {
        trigger,
    ...
    if (result?.ok === false && result?.error) {
      this.logger.warn(`Hyperliquid monitor failed (${trigger}): ${result.error}`);
    }
  ...
  getActiveHyperliquidManualActivity(nowMs = Date.now()) {
    const activity = readManualHyperliquidActivity(this.hyperliquidManualActivityPath);
    if (!isManualHyperliquidActivityActive(activity, { nowMs })) {
      return null;
    }
    return {
      command: String(activity.command || 'hyperliquid_manual'),
  ...
  async syncHyperliquidPeakStateFromAccountState(accountState = {}, options = {}) {
    if (!this.hyperliquidMonitorOrchestrator || typeof this.hyperliquidMonitorOrchestrator.syncDefiPeakStateFromStatus !== 'function') {
      return null;
    }
    try {
      return await this.hyperliquidMonitorOrchestrator.syncDefiPeakStateFromStatus({
        ok: true,
      ...
      this.logger.warn(`Hyperliquid peak-state sync failed (${options.trigger || 'execution_snapshot'}): ${error?.message || String(error)}`);
      return null;
    }
  }

  async maybeExecuteHyperliquidRiskExit(monitorResult = {}, options = {}) {
    ...
    for (const position of positions) {
      ...
      if (drawdown >= 0.3 && previousAlertThreshold < 0.3) {
        this.notifyTelegramTrading(this.buildHyperliquidGivebackAlert(position));
      }

      const reasons = [];
      if (this.shouldTriggerHyperliquidStop(position)) {
      ...
      // giveback_75 DISABLED — killed James's profitable BLUR trade on a normal pullback. Session 268.
      ...
      const summary = {
        ...
        ok: true,
        reasons,
        position: this.summarizeHyperliquidPosition(position),
        execution: null,
      };
      executions.push(summary);
      this.logger.warn(`[TradeLog] Hyperliquid risk exit requires manual action for ${position.coin} ${position.side} | reasons: ${reasons.join('+')} | pnl: $${toNumber(position?.unrealizedPnl, 0).toFixed(2)} | entry: ${position.entryPx} | mark: ${position.markPrice}`);
      this.notifyTelegramTrading(this.buildHyperliquidManualActionAlert(position, reasons));
    }
  ...
  startHyperliquidPositionMonitor() {
    if (!this.hyperliquidMonitorEnabled || this.hyperliquidMonitorTimer) {
      return;
    }
    this.hyperliquidMonitorPromise = this.runHyperliquidPositionMonitorCycle('startup')
      .catch((error) => {
        this.logger.warn(`Hyperliquid startup monitor failed: ${error.message}`);
        return { ok: false, error: error.message };
      })
      .finally(() => {
        this.hyperliquidMonitorPromise = null;
      });
    this.hyperliquidMonitorTimer = setInterval(() => {
      if (this.hyperliquidMonitorPromise) {
        return;
      }
      this.hyperliquidMonitorPromise = this.runHyperliquidPositionMonitorCycle('interval')
        .catch((error) => {
          this.logger.warn(`Hyperliquid interval monitor failed: ${error.message}`);
          return { ok: false, error: error.message };
        })
        .finally(() => {
          this.hyperliquidMonitorPromise = null;
        });
    }, this.hyperliquidMonitorPollMs);
    if (typeof this.hyperliquidMonitorTimer.unref === 'function') {
      this.hyperliquidMonitorTimer.unref();
    }
  }

  async stopHyperliquidPositionMonitor() {
    if (this.hyperliquidMonitorTimer) {
      clearInterval(this.hyperliquidMonitorTimer);
      this.hyperliquidMonitorTimer = null;
    }
    if (this.hyperliquidMonitorPromise) {
      await Promise.resolve(this.hyperliquidMonitorPromise).catch(() => {});
      this.hyperliquidMonitorPromise = null;
    }
  }
  ...
  async maybeRunPositionAttributionReconciliation(nowMs = Date.now()) {
    ...
    const walletAddress = resolveHyperliquidWalletAddress(this.runtimeEnv);
    try {
      const livePositions = this.positionAttributionSnapshotProvider
        ...
        : await hyperliquidClient.getOpenPositions({ walletAddress, env: this.runtimeEnv });
  ...
  async getCryptoSymbolCandidates(options = {}) {
    const openPositionSymbols = await this.getCurrentHyperliquidPositionSymbols();
    ...
    const symbolsExecutable = orderedSymbols.filter((ticker) => this.isHyperliquidConsensusTicker(ticker));
    this.lastCryptoCoverage = {
  ...
  async buildRangeConvictionSelection(symbols = [], options = {}) {
    ...
    const [bars5m, bars15m, bars1h, accountState] = await Promise.all([
      ...
      this.hyperliquidExecutor?.getAccountState?.().catch(() => null),
    ]);
    ...
    this.lastRangeConvictionSelection = {
      ...
      activePosition: activeRow ? this.summarizeHyperliquidPosition(activeRow.openPosition) : null,
  ...
  async rankConsultationMovers(entries = []) {
    ...
    try {
      nativeBundle = await hyperliquidNativeLayer.buildNativeFeatureBundle({
  ...
  isHyperliquidConsensusTicker(ticker) {
  ...
  syncRangeConvictionState(selection = null) {
    const thesis = selection
      ? {
        ...
        activePosition: selection?.activePosition ? this.summarizeHyperliquidPosition(selection.activePosition) : null,
        updatedAt: new Date().toISOString(),
      }
      : null;
  ...
  async executeRangeConvictionManagement(selection = {}, options = {}) {
    ...
    return {
      ...
      position: this.summarizeHyperliquidPosition(activePosition),
      execution: null,
    };
  }
  ...
  buildHyperliquidAutoExecutionSummary(autoExecution = {}, scheduledAt = null) {
  ...
  buildHyperliquidGivebackAlert(position) {
    ...
    const stopLossText = position?.stopLossVerifiedAt
      ...
      : '';
    return `[TRADING] Hyperliquid ${position.coin} ${String(position.side || '').toUpperCase()} has given back ${(drawdownPct * 100).toFixed(0)}% of peak PnL. Current=$${toNumber(position?.unrealizedPnl, 0).toFixed(2)} peak=$${toNumber(position?.peakUnrealizedPnl, 0).toFixed(2)} mark=$${toNumber(position?.markPrice, 0).toFixed(4)}.${stopLossText}`;
  }

  buildHyperliquidManualActionAlert(position = {}, reasons = []) {
    const reasonText = Array.isArray(reasons) && reasons.length > 0 ? reasons.join('+') : 'risk_exit_signal';
    const stopLossText = position?.stopLossVerifiedAt
      && Number.isFinite(Number(position?.stopLossPrice)) && Number(position.stopLossPrice) > 0
      ? ` stop=$${Number(position.stopLossPrice).toFixed(4)}`
      : '';
    return `[ACTION REQUIRED] Hyperliquid ${position.coin} ${String(position.side || '').toUpperCase()} hit ${reasonText}. Current=$${toNumber(position?.unrealizedPnl, 0).toFixed(2)} entry=$${toNumber(position?.entryPx, 0).toFixed(4)} mark=$${toNumber(position?.markPrice, 0).toFixed(4)}.${stopLossText} Manual-only reset is active, so the supervisor did not close it.`;
  }

  shouldTriggerHyperliquidStop(position = {}) {
  ...
  async runHyperliquidExecutionPhase({ scheduledAt, marketDate, consensusPhase, approvedTrades = [] } = {}) {
    ...
    if (this.manualTradingOnly && !this.hyperliquidScalpModeArmed) {
      const accountState = typeof this.hyperliquidExecutor?.getAccountState === 'function'
        ? await Promise.resolve(this.hyperliquidExecutor.getAccountState()).catch(() => null)
        : null;
      const ethPosition = Array.isArray(accountState?.positions)
        ...
        : null;
      await this.syncHyperliquidPeakStateFromAccountState(accountState || {}, {
      ...
      const result = {
        ...
        approvedTrade: null,
        accountValue: toNumber(accountState?.accountValue, 0),
        position: this.summarizeHyperliquidPosition(ethPosition),
        execution: null,
      };
    ...
    let accountState = await this.hyperliquidExecutor.getAccountState();
    ...
    if (directive.action === 'open_short') {
      if (ethPosition?.side === 'short') {
        ...
        if (execution?.ok !== false) {
          accountState = await this.hyperliquidExecutor.getAccountState();
          ethPosition = Array.isArray(accountState?.positions)
            ? accountState.positions.find((position) => position.coin === 'ETH' && position.side !== 'flat')
            : null;
          result.accountValue = toNumber(accountState?.accountValue, 0);
          result.position = this.summarizeHyperliquidPosition(ethPosition);
        }
      }
    } else if (directive.action === 'close_position') {
      // AUTO-CLOSE DISABLED: agents manage exits manually
      this.logger.info('[HyperliquidExecution] close_position directive BLOCKED — agents manage exits manually');
    ...
    await this.syncHyperliquidPeakStateFromAccountState(accountState, {
      trigger: `hyperliquid_execution_${result.action || 'none'}`,
      checkedAt: scheduledAt || new Date().toISOString(),
      sendTelegram: false,
    });
  ...
  async runCryptoConsensusPhase(event) {
    ...
    try {
      ...
      const consensusPhase = await this.cryptoTradingOrchestrator.runConsensusRound({
        ...
        hyperliquidExecutionDryRun: this.hyperliquidExecutionDryRun,
        hyperliquidExecutionEnv: this.runtimeEnv,
        hyperliquidExecuteScriptPath: this.hmDefiExecuteScriptPath,
        hyperliquidCloseScriptPath: this.hmDefiCloseScriptPath,
        hyperliquidExecutionLeverage: this.hyperliquidExecutionLeverage,
      });
      ...
      const hyperliquidScalpExecutionEnabled = this.hyperliquidExecutionEnabled
        && this.hyperliquidScalpModeArmed
      ...
      if (this.cryptoMonitorOnly) {
        ...
        approved = [];
      } else if (this.manualTradingOnly && !hyperliquidScalpExecutionEnabled && approved.length > 0) {
      ...
      const approvedForMarketOpen = (this.manualTradingOnly && !hyperliquidScalpExecutionEnabled)
        ? []
        : approved;

      const hyperliquidExecution = typeof this.cryptoTradingOrchestrator.maybeAutoExecuteLiveConsensus === 'function'
        ? await this.cryptoTradingOrchestrator.maybeAutoExecuteLiveConsensus(
          hyperliquidScalpExecutionEnabled ? [] : consensusPhase?.results,
          hyperliquidScalpExecutionEnabled ? [] : approved,
          consensusPhase?.defiStatus || {},
          {
            autoExecuteLiveConsensus: this.hyperliquidExecutionEnabled && !this.cryptoMonitorOnly && (!this.manualTradingOnly || this.hyperliquidScalpModeArmed),
            hyperliquidExecutionDryRun: this.hyperliquidExecutionDryRun,
            hyperliquidExecutionEnv: this.runtimeEnv,
            hyperliquidExecuteScriptPath: this.hmDefiExecuteScriptPath,
            hyperliquidCloseScriptPath: this.hmDefiCloseScriptPath,
            hyperliquidExecutionLeverage: this.hyperliquidExecutionLeverage,
            hyperliquidScalpModeArmed: this.hyperliquidScalpModeArmed,
            hyperliquidKillSwitchAction: this.hyperliquidKillSwitchAction,
      ...
      // Execute non-Hyperliquid approved trades immediately after Hyperliquid sizing/execution is settled.
      ...
      if (approvedForMarketOpen.length > 0) {
        ...
        executionResult = await this.cryptoTradingOrchestrator.runMarketOpen({
          ...
          allowHyperliquidLiveExecution: hyperliquidScalpExecutionEnabled,
          hyperliquidScalpModeArmed: this.hyperliquidScalpModeArmed,
          hyperliquidExecutionDryRun: this.hyperliquidExecutionDryRun,
          hyperliquidExecutionEnv: this.runtimeEnv,
          hyperliquidExecuteScriptPath: this.hmDefiExecuteScriptPath,
      ...
      if (hyperliquidExecution?.attempted > 0 && typeof this.hyperliquidExecutor?.getAccountState === 'function') {
        const accountState = await Promise.resolve(this.hyperliquidExecutor.getAccountState()).catch(() => null);
        if (accountState) {
          await this.syncHyperliquidPeakStateFromAccountState(accountState, {
        ...
        this.lastHyperliquidExecutionSummary = this.buildHyperliquidAutoExecutionSummary(hyperliquidExecution, scheduledAt);
        this.logger.info(
          `Hyperliquid auto-execution ${hyperliquidExecution.succeeded}/${hyperliquidExecution.attempted} completed`
        );
      } else {
        this.lastHyperliquidExecutionSummary = this.buildHyperliquidAutoExecutionSummary(hyperliquidExecution, scheduledAt);
      }
      ...
      return {
        ...
        summary: {
          ...
          hyperliquidAction: hyperliquidExecution?.executions?.[0]?.action || 'none',
          hyperliquidExecuted: Number(hyperliquidExecution?.succeeded || 0) > 0,
  ...
  writeStatus(extra = {}) {
    ...
    const payload = {
      ...
      hyperliquidExecution: {
        enabled: this.hyperliquidExecutionEnabled && (!this.manualTradingOnly || this.hyperliquidScalpModeArmed),
        manualOnly: this.manualTradingOnly,
        scalpModeArmed: this.hyperliquidScalpModeArmed,
        dryRun: this.hyperliquidExecutionDryRun,
        autoOpenEnabled: this.hyperliquidScalpModeArmed || String(this.runtimeEnv.SQUIDRUN_ALLOW_HYPERLIQUID_AUTO_OPEN || '').trim() === '1',
        killSwitchAction: this.hyperliquidKillSwitchAction,
        symbolsExecutable,
        lastSummary: this.lastHyperliquidExecutionSummary || null,
      },
      hyperliquidPositionMonitor: {
        enabled: this.hyperliquidMonitorEnabled,
        running: Boolean(this.hyperliquidMonitorTimer || this.hyperliquidMonitorPromise),
        pollMs: this.hyperliquidMonitorPollMs,
        peakStatePath: this.defiPeakPnlPath,
        lastSummary: this.lastHyperliquidMonitorSummary || null,
      },
      smartMoneyScanner: {
      ...
      tokenomistAutomation: {
        enabled: this.tokenomistEnabled,
        running: Boolean(this.tokenomistPhasePromise),
        intervalMinutes: this.tokenomistIntervalMinutes,
        statePath: this.tokenomistStatePath,
        lastProcessedAt: this.tokenomistState.lastProcessedAt || null,
        nextEvent: this.tokenomistState.nextEvent || null,
        lastSummary: this.lastTokenomistSummary || null,
      },
      ...
      eunbyeolCheckInAutomation: {
        enabled: this.eunbyeolCheckInEnabled,
        running: Boolean(this.eunbyeolCheckInPhasePromise),
        intervalMinutes: this.eunbyeolCheckInIntervalMinutes,
        silenceMs: this.eunbyeolCheckInSilenceMs,
        statePath: this.eunbyeolCheckInStatePath,
        lastProcessedAt: this.eunbyeolCheckInState.lastProcessedAt || null,
        nextEvent: this.eunbyeolCheckInState.nextEvent || null,
        lastSummary: this.lastEunbyeolCheckInSummary || null,
      },
