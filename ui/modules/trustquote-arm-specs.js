...
const TRUSTQUOTE_DAY_TO_DAY_ARM_SPECS = Object.freeze([
  ...
  },
  {
    ...
    startupLines: [
      ...
      'Before creating a client/customer for any schedule entry or job, ALWAYS search existing customers first - match on EMAIL + address/unit, not name alone (different units/tenants under the same property manager share an email but are separate records). Link to the right existing record; never create a duplicate, and never collapse different units by name. James should never have to say "existing client".',
