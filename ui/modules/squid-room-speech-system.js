...
function createSquidRoomSpeechSystem(options = {}) {
  ...
  /** Advance one frame. anchors: { [petId]: {mouthX,mouthY,headX,headY} } */
  function frame(nowMs, anchors = {}) {
    for (const entry of pets.values()) {
      const anchor = anchors[entry.petId];
      if (!entry.visible || !anchor) {
        // fail-dark: no anchor -> hide, never guess
        entry.box.dataset.visible = 'false';
        for (let i = 0; i < entry.dots.length; i += 1) entry.dots[i].style.opacity = '0';
        continue;
      }
      entry.box.dataset.visible = 'true';
      ...
      const tailX = Math.min(Math.max(mouthX, entry.posX), entry.posX + boxW);
      const tailY = entry.posY + boxH / 2 <= mouthY ? entry.posY + boxH : entry.posY;
      layoutChainDots(entry.dotStates, mouthX, mouthY, tailX, tailY, nowMs, reducedMotion);
      for (let i = 0; i < entry.dots.length; i += 1) {
        const s = entry.dotStates[i];
        const dot = entry.dots[i];
        dot.style.opacity = '1';
        dot.style.transform = `translate3d(${s.x}px, ${s.y}px, 0) scale(${s.scale})`;
      }
    }
  }
