...
const FIXTURE_REF =
  'ui/__tests__/fixtures/mira-core-kill-switch-wiring-readiness-rollup-through-phase52-contract.json';
const KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE52_SCHEMA_VERSION =
  'squidrun.mira_core.kill_switch_wiring_readiness_rollup_through_phase52.v0';
const VALIDATION_REPORT_SCHEMA_VERSION =
  'squidrun.mira_core.kill_switch_wiring_readiness_rollup_through_phase52_validation_report.v0';
const KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE52_VERSION = 0;
const BASELINE_COMMIT = '3de1b93';

const PROJECT_ROOT = path.resolve(__dirname, '..', '..', '..');

const REQUIRED_OUTPUT_FIELDS = Object.freeze([
  'kill_switch_wiring_readiness_rollup_through_phase52',
  'validation_report',
]);
...
function evidenceRef(store, eventId, relation = 'kill_switch_wiring_readiness_rollup_through_phase52_validation') {
...
function phaseRegistry(contract = {}) {
  ...
  return {
    ...
    evidenceRefs: [
      evidenceRef('git', BASELINE_COMMIT, 'phase52-baseline'),
...
function killSwitchWiringReadinessRollupThroughPhase52IdempotencyKey(dryCheck) {
  return `kill-switch-wiring-readiness-rollup-through-phase52:${stableHash(canonicalDryCheckInput(dryCheck))}`;
}
...
function buildDryCheck(options = {}) {
  ...
  const dryCheck = {
    schema: KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE52_SCHEMA_VERSION,
    version: KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE52_VERSION,
    rollup_id: `kill-switch-wiring-readiness-rollup-through-phase52-${stableHash({
      scope,
  ...
  dryCheck.idempotency_key = killSwitchWiringReadinessRollupThroughPhase52IdempotencyKey(dryCheck);
  return dryCheck;
}
...
function sourceRecommendationOk(dryCheck = {}, contract = {}) {
  ...
  return containsExpectedValues(source, contract.sourceRecommendation || {})
    && source.recommendation_id === 'phase53-kill-switch-wiring-readiness-rollup-through-phase52-contract'
    && source.tier === 'tier1'
...
function checkMap(dryCheck = {}, contract = {}) {
  ...
  return {
    'static-schema-and-baseline-pinned': dryCheck.schema === KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE52_SCHEMA_VERSION
      && dryCheck.version === KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE52_VERSION
      && dryCheck.baseline_commit === BASELINE_COMMIT,
    'accept-schema-baseline-3de1b93': dryCheck.schema === KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE52_SCHEMA_VERSION
      && dryCheck.version === KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE52_VERSION
      && dryCheck.baseline_commit === BASELINE_COMMIT,
    ...
    'accept-commit-chain-36-ending-3de1b93': commitChainOk(dryCheck, contract),
    'static-phase52-tier0-satisfied': satisfiedPriorOk(dryCheck, contract),
    'accept-phase52-validator-satisfied': satisfiedPriorOk(dryCheck, contract),
    'static-phase52-tier1-selected': sourceRecommendationOk(dryCheck, contract),
    'accept-phase52-readiness-rollup-selected': sourceRecommendationOk(dryCheck, contract),
    ...
    'accept-source-refs-include-phase52': sourceRefsOk(dryCheck, contract),
    'static-readiness-rollup-through-phase52-contract-only': readinessRollup,
    'accept-readiness-rollup-through-phase52-contract-only': readinessRollup,
    'static-phase52-readiness-rollup-current': phase52ReadinessSummary,
    'accept-phase52-readiness-rollup-current': phase52ReadinessSummary,
    ...
    'static-readiness-rollup-through-phase52-non-authorizing': readinessRollup,
    'accept-readiness-rollup-through-phase52-non-authorizing': readinessRollup,
    'accept-phase52-report-identity-current': rawReportLiterals.schema === VALIDATION_REPORT_SCHEMA_VERSION
...
function validateDryCheck(dryCheck = {}, contract = {}) {
  ...
  const outputShapeOk = dryCheck.schema === KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE52_SCHEMA_VERSION
...
function buildValidationReport(dryCheck, contract = {}, generatedAt = dryCheck.generated_at) {
  ...
  const report = {
    ...
    version: KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE52_VERSION,
    validation_report_id: `kill-switch-wiring-readiness-rollup-through-phase52-validation-${stableHash({
    ...
    scoped_status: {
      ...
      scoped_phase52_files_only: true,
...
function buildMiraCoreKillSwitchWiringReadinessRollupThroughPhase52(options = {}) {
  const contract = options.contract || {};
  const kill_switch_wiring_readiness_rollup_through_phase52 = buildDryCheck(options);
  const validation_report = buildValidationReport(
    kill_switch_wiring_readiness_rollup_through_phase52,
    contract,
    kill_switch_wiring_readiness_rollup_through_phase52.generated_at,
  );
  const output = { kill_switch_wiring_readiness_rollup_through_phase52, validation_report };
  assertNoForbiddenOutput(output, asArray(contract.forbiddenOutputSubstrings));
  return output;
}

function validateMiraCoreKillSwitchWiringReadinessRollupThroughPhase52Output(output = {}, contract = {}) {
  const checks = [];
  const errors = [];
  const add = (id, ok) => {
  ...
  const dryCheck = output.kill_switch_wiring_readiness_rollup_through_phase52 || {};
  ...
  add('output-shape-complete',
    hasRequiredFields(output, contract.expectedOutputShape?.requiredTopLevelFields || REQUIRED_OUTPUT_FIELDS)
      && dryCheck.schema === KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE52_SCHEMA_VERSION
