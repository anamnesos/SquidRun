...
const FIXTURE_REF =
  'ui/__tests__/fixtures/mira-core-kill-switch-wiring-readiness-rollup-through-phase49-contract.json';
const KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE49_SCHEMA_VERSION =
  'squidrun.mira_core.kill_switch_wiring_readiness_rollup_through_phase49.v0';
const VALIDATION_REPORT_SCHEMA_VERSION =
  'squidrun.mira_core.kill_switch_wiring_readiness_rollup_through_phase49_validation_report.v0';
const KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE49_VERSION = 0;
const BASELINE_COMMIT = '17441a8';

const PROJECT_ROOT = path.resolve(__dirname, '..', '..', '..');

const REQUIRED_OUTPUT_FIELDS = Object.freeze([
  'kill_switch_wiring_readiness_rollup_through_phase49',
  'validation_report',
]);
...
function evidenceRef(store, eventId, relation = 'kill_switch_wiring_readiness_rollup_through_phase49_validation') {
...
function phaseRegistry(contract = {}) {
  ...
  return {
    expected_phases: expected.expected_phases || '1-49',
    current_through_phase: expected.phase_inventory_count || 49,
    phase_inventory_count: expected.phase_inventory_count || 49,
    schema_registry_count: expected.schema_registry_count || 49,
    cli_registry_count: expected.cli_registry_count || 49,
    ...clone(current),
    ...
    evidenceRefs: [
      evidenceRef('git', BASELINE_COMMIT, 'phase50-baseline'),
...
function killSwitchWiringReadinessRollupThroughPhase49IdempotencyKey(dryCheck) {
  return `kill-switch-wiring-readiness-rollup-through-phase49:${stableHash(canonicalDryCheckInput(dryCheck))}`;
}
...
function buildDryCheck(options = {}) {
  ...
  const dryCheck = {
    schema: KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE49_SCHEMA_VERSION,
    version: KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE49_VERSION,
    rollup_id: `kill-switch-wiring-readiness-rollup-through-phase49-${stableHash({
      scope,
  ...
  dryCheck.idempotency_key = killSwitchWiringReadinessRollupThroughPhase49IdempotencyKey(dryCheck);
  return dryCheck;
}
...
function sourceRecommendationOk(dryCheck = {}, contract = {}) {
  ...
  return containsExpectedValues(source, contract.sourceRecommendation || {})
    && source.recommendation_id === 'phase50-kill-switch-wiring-readiness-rollup-through-phase49-contract'
    && source.tier === 'tier1'
...
function closuresOk(dryCheck = {}) {
  return dryCheck.closure_summary?.oracle_194_phase49_review_closure_carried === true
    && dryCheck.closure_summary?.builder_301_phase49_validation_report_carried === true
    && asArray(dryCheck.closure_summary?.closed_review_refs).includes('ORACLE #194');
}

function sourceRefsOk(dryCheck = {}, contract = {}) {
  return valuesMatch(dryCheck.source_refs, contract.sourceRefsExpected || [])
    && asArray(dryCheck.source_refs).length === Number(contract.expectedCounts?.source_ref_count || 19)
    && asArray(dryCheck.source_refs).some((ref) => ref.artifact_id === 'phase49-kill-switch-wiring-noop-boundary-refresh');
}
...
function checkMap(dryCheck = {}, contract = {}) {
  ...
  return {
    'static-schema-and-baseline-pinned': dryCheck.schema === KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE49_SCHEMA_VERSION
      && dryCheck.version === KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE49_VERSION
      && dryCheck.baseline_commit === BASELINE_COMMIT,
    'accept-schema-baseline-17441a8': dryCheck.schema === KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE49_SCHEMA_VERSION
      && dryCheck.version === KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE49_VERSION
      && dryCheck.baseline_commit === BASELINE_COMMIT,
    ...
    'accept-commit-chain-36-ending-17441a8': commitChainOk(dryCheck, contract),
    'static-phase49-tier0-satisfied': satisfiedPriorOk(dryCheck, contract),
    'accept-phase49-validator-satisfied': satisfiedPriorOk(dryCheck, contract),
    ...
    'static-oracle-closure-chain-through-194': closuresOk(dryCheck),
    'accept-oracle-closure-through-194': closuresOk(dryCheck),
    ...
    'static-phase49-boundary-refresh-current': phase49Summary,
    'accept-phase49-boundary-refresh-current': phase49Summary,
    ...
    'accept-source-refs-include-phase49': sourceRefsOk(dryCheck, contract),
...
function validateDryCheck(dryCheck = {}, contract = {}) {
  ...
  const outputShapeOk = dryCheck.schema === KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE49_SCHEMA_VERSION
...
function buildValidationReport(dryCheck, contract = {}, generatedAt = dryCheck.generated_at) {
  ...
  const report = {
    ...
    version: KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE49_VERSION,
    validation_report_id: `kill-switch-wiring-readiness-rollup-through-phase49-validation-${stableHash({
...
function buildMiraCoreKillSwitchWiringReadinessRollupThroughPhase49(options = {}) {
  const contract = options.contract || {};
  const kill_switch_wiring_readiness_rollup_through_phase49 = buildDryCheck(options);
  const validation_report = buildValidationReport(
    kill_switch_wiring_readiness_rollup_through_phase49,
    contract,
    kill_switch_wiring_readiness_rollup_through_phase49.generated_at,
  );
  const output = { kill_switch_wiring_readiness_rollup_through_phase49, validation_report };
  assertNoForbiddenOutput(output, asArray(contract.forbiddenOutputSubstrings));
  return output;
}

function validateMiraCoreKillSwitchWiringReadinessRollupThroughPhase49Output(output = {}, contract = {}) {
  const checks = [];
  const errors = [];
  const add = (id, ok) => {
  ...
  const dryCheck = output.kill_switch_wiring_readiness_rollup_through_phase49 || {};
  ...
  add('output-shape-complete',
    hasRequiredFields(output, contract.expectedOutputShape?.requiredTopLevelFields || REQUIRED_OUTPUT_FIELDS)
      && dryCheck.schema === KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE49_SCHEMA_VERSION
