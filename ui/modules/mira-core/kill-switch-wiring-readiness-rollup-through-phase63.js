...
const FIXTURE_REF =
  'ui/__tests__/fixtures/mira-core-kill-switch-wiring-readiness-rollup-through-phase63-contract.json';
const OUTPUT_FIELD = 'kill_switch_wiring_readiness_rollup_through_phase63';
const KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE64_SCHEMA_VERSION =
  'squidrun.mira_core.kill_switch_wiring_readiness_rollup_through_phase63.v0';
const VALIDATION_REPORT_SCHEMA_VERSION =
  'squidrun.mira_core.kill_switch_wiring_readiness_rollup_through_phase63_validation_report.v0';
const KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE64_VERSION = 0;
const CURRENT_SCOPED_STATUS_KEY = 'scoped_phase64_through_phase63_files_only';
...
function evidenceRef(store, eventId, relation = 'kill_switch_wiring_readiness_rollup_through_phase63_validation') {
...
function normalizeScope(inputSignals = {}) {
  ...
  return {
    ...
    sessionId: inputSignals.sessionId || 'session-361',
...
function phaseRegistry(contract = {}) {
  ...
  return applyLiteralValues({
    ...
    evidenceRefs: [
      evidenceRef('git', BASELINE_COMMIT, 'phase63-baseline'),
...
function killSwitchWiringReadinessRollupThroughPhase64IdempotencyKey(dryCheck = {}) {
  return `kill-switch-wiring-readiness-rollup-through-phase63:${stableHash(canonicalDryCheckInput(dryCheck))}`;
}
...
function buildDryCheck(options = {}) {
  ...
  const manifest = {
    ...
    version: KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE64_VERSION,
    rollup_id: `kill-switch-wiring-readiness-rollup-through-phase63-${stableHash({
      generatedAt,
    ...
    evidenceRefs: [
      evidenceRef('git', BASELINE_COMMIT, 'phase63-baseline'),
      evidenceRef('fixture', FIXTURE_REF, 'phase64-contract'),
    ],
  ...
  manifest.idempotency_key = killSwitchWiringReadinessRollupThroughPhase64IdempotencyKey(manifest);
  return manifest;
}
...
function hybridArtifactMarkers() {
  return [
    ['phase63', 'boundary_refresh_summary'].join('_'),
    ['phase63', 'Boundary', 'Refresh', 'Summary'].join(''),
    ['phase63', 'boundary', 'refresh'].join('-'),
  ];
}
...
function sourceRefsOk(dryCheck = {}, contract = {}) {
  return valuesMatch(dryCheck.source_refs, contract.sourceRefsExpected || [])
    && asArray(dryCheck.source_refs).some((ref) => (
      ref.artifact_id === 'phase63-kill-switch-wiring-readiness-rollup-through-phase62'
    ));
}
...
function readinessRollupOk(dryCheck = {}, contract = {}) {
  ...
  return containsExpectedValues(summary, contract.readinessRollupSummaryExpected || {})
    && summary.current_through_phase === 63
    && summary.phase22_caveat_blocks_phase64_fixture === false
    && summary.phase63_scoped_status_closure_carried === true
    && summary.phase63_readiness_rollup_validation_current === true
    && summary.phase63_duplicate_registry_count_closure_carried === true
...
function phase63ReadinessRollupSummaryOk(dryCheck = {}, contract = {}) {
  ...
  return containsExpectedValues(summary, contract.phase63ReadinessRollupSummaryExpected || {})
    ...
    && summary.oracle_review_ref === 'ORACLE #288'
...
function duplicateRegistryClosureOk(dryCheck = {}, contract = {}) {
  const counts = contract.expectedCounts || {};
  return counts.schema_registry_count === 63
    && counts.cli_registry_count === 63
    && dryCheck.phase_registry?.schema_registry_count === 63
    && dryCheck.phase_registry?.cli_registry_count === 63
    ...
    && dryCheck.readiness_rollup_summary?.phase63_duplicate_registry_count_closure_carried === true
    && dryCheck.phase_registry?.phase63_duplicate_registry_count_closure_current === true
    && dryCheck.current_truth?.phase63_duplicate_registry_count_closure_current === true
    && dryCheck.closure_summary?.phase63_duplicate_registry_count_closure_carried === true
    && dryCheck.phase63_readiness_rollup_summary?.duplicate_registry_count_closure
      === 'phase_registry_schema_registry_count_and_cli_registry_count_bound_to_expectedCounts_63_in_phase64_with_phase63_closure_carried';
}
...
function phase22CaveatOk(dryCheck = {}) {
  return dryCheck.readiness_rollup_summary?.phase22_date_sensitive_expiry_caveat
    ...
    && dryCheck.readiness_rollup_summary?.phase22_caveat_blocks_phase64_fixture === false
    && dryCheck.phase63_readiness_rollup_summary?.phase22_caveat_out_of_scope_non_blocking === true
    && dryCheck.phase63_readiness_rollup_summary?.phase22_caveat_non_authorizing === true;
}
...
function validationReportIdentityOk(report = {}, contract = {}) {
  return report.schema === (contract.expectedOutputShape?.reportSchema || VALIDATION_REPORT_SCHEMA_VERSION)
    ...
    && report.version === KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE64_VERSION;
}
...
function checkMap(dryCheck = {}, contract = {}, report = null) {
  ...
  const idempotencyOk = dryCheck.idempotency_key === killSwitchWiringReadinessRollupThroughPhase64IdempotencyKey(dryCheck);
  ...
  return allIds.reduce((acc, id) => {
    ...
    if (/registry|commit-chain|baseline|literal|shape|manifest/.test(id)) ok = literalOk && countBindingOk;
...
function buildValidationReport(dryCheck, contract = {}, generatedAt = dryCheck.generated_at) {
  ...
  const report = {
    ...
    validation_report_id: `kill-switch-wiring-readiness-rollup-through-phase63-validation-${stableHash({
    ...
    summary: {
      ...
      phase63_validation_green: dryCheck.phase63_readiness_rollup_summary?.validation_green,
...
function buildMiraCoreKillSwitchWiringReadinessRollupThroughPhase64(options = {}) {
  const contract = options.contract || {};
...
function validateMiraCoreKillSwitchWiringReadinessRollupThroughPhase64Output(output = {}, contract = {}) {
  const checks = [];
  const errors = [];
  const add = (id, ok) => {
