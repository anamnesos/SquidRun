...
const FIXTURE_REF =
  'ui/__tests__/fixtures/mira-core-kill-switch-wiring-readiness-rollup-through-phase59-contract.json';
const KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE59_SCHEMA_VERSION =
  'squidrun.mira_core.kill_switch_wiring_readiness_rollup_through_phase59.v0';
const VALIDATION_REPORT_SCHEMA_VERSION =
  'squidrun.mira_core.kill_switch_wiring_readiness_rollup_through_phase59_validation_report.v0';
const KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE59_VERSION = 0;
const CURRENT_SCOPED_STATUS_KEY = 'scoped_phase60_through_phase59_files_only';
...
const BASELINE_COMMIT = 'ea795c8';

const PROJECT_ROOT = path.resolve(__dirname, '..', '..', '..');

const REQUIRED_OUTPUT_FIELDS = Object.freeze([
  'kill_switch_wiring_readiness_rollup_through_phase59',
  'validation_report',
]);
...
function evidenceRef(store, eventId, relation = 'kill_switch_wiring_readiness_rollup_through_phase59_validation') {
...
function phaseRegistry(contract = {}) {
  ...
  return {
    ...
    phase59_commit: BASELINE_COMMIT,
    ...
    evidenceRefs: [
      evidenceRef('git', BASELINE_COMMIT, 'phase59-baseline'),
...
function killSwitchWiringReadinessRollupThroughPhase59IdempotencyKey(dryCheck) {
  return `kill-switch-wiring-readiness-rollup-through-phase59:${stableHash(canonicalDryCheckInput(dryCheck))}`;
}
...
function buildDryCheck(options = {}) {
  ...
  const dryCheck = {
    schema: KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE59_SCHEMA_VERSION,
    version: KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE59_VERSION,
    rollup_id: `kill-switch-wiring-readiness-rollup-through-phase59-${stableHash({
      scope,
  ...
  dryCheck.idempotency_key = killSwitchWiringReadinessRollupThroughPhase59IdempotencyKey(dryCheck);
  return dryCheck;
}
...
function sourceRefsOk(dryCheck = {}, contract = {}) {
  ...
  return valuesMatch(dryCheck.source_refs, contract.sourceRefsExpected || [])
    ...
    && ['phase50', 'phase51', 'phase52', 'phase53', 'phase54', 'phase55', 'phase56', 'phase57', 'phase58', 'phase59'].every((phase) => (
...
function checkMap(dryCheck = {}, contract = {}) {
  ...
  return {
    'static-schema-and-baseline-pinned': dryCheck.schema === KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE59_SCHEMA_VERSION
      && dryCheck.version === KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE59_VERSION
      && dryCheck.baseline_commit === BASELINE_COMMIT,
    'accept-schema-baseline-08881c8': dryCheck.schema === KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE59_SCHEMA_VERSION
      && dryCheck.version === KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE59_VERSION
      && dryCheck.baseline_commit === BASELINE_COMMIT,
...
function validateDryCheck(dryCheck = {}, contract = {}) {
  ...
  const outputShapeOk = dryCheck.schema === KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE59_SCHEMA_VERSION
...
function buildValidationReport(dryCheck, contract = {}, generatedAt = dryCheck.generated_at) {
  ...
  const report = {
    ...
    version: KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE59_VERSION,
    validation_report_id: `kill-switch-wiring-readiness-rollup-through-phase59-validation-${stableHash({
...
function buildMiraCoreKillSwitchWiringReadinessRollupThroughPhase59(options = {}) {
  const contract = options.contract || {};
  const kill_switch_wiring_readiness_rollup_through_phase59 = buildDryCheck(options);
  const validation_report = buildValidationReport(
    kill_switch_wiring_readiness_rollup_through_phase59,
    contract,
    kill_switch_wiring_readiness_rollup_through_phase59.generated_at,
  );
  const output = { kill_switch_wiring_readiness_rollup_through_phase59, validation_report };
  assertNoForbiddenOutput(output, asArray(contract.forbiddenOutputSubstrings));
  return output;
}

function validateMiraCoreKillSwitchWiringReadinessRollupThroughPhase59Output(output = {}, contract = {}) {
  const checks = [];
  const errors = [];
  const add = (id, ok) => {
  ...
  const dryCheck = output.kill_switch_wiring_readiness_rollup_through_phase59 || {};
  ...
  add('output-shape-complete',
    hasRequiredFields(output, contract.expectedOutputShape?.requiredTopLevelFields || REQUIRED_OUTPUT_FIELDS)
      && dryCheck.schema === KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE59_SCHEMA_VERSION
...
module.exports = {
  ...
  KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE59_SCHEMA_VERSION,
  VALIDATION_REPORT_SCHEMA_VERSION,
  assertNoForbiddenOutput,
  buildMiraCoreKillSwitchWiringReadinessRollupThroughPhase59,
  killSwitchWiringReadinessRollupThroughPhase59IdempotencyKey,
  stableHash,
  validateMiraCoreKillSwitchWiringReadinessRollupThroughPhase59Output,
};
