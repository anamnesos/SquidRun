...
const FIXTURE_REF =
  'ui/__tests__/fixtures/mira-core-kill-switch-wiring-noop-dry-check-contract.json';
const KILL_SWITCH_WIRING_NOOP_DRY_CHECK_SCHEMA_VERSION =
  'squidrun.mira_core.kill_switch_wiring_noop_dry_check.v0';
const VALIDATION_REPORT_SCHEMA_VERSION =
  'squidrun.mira_core.kill_switch_wiring_noop_dry_check_validation_report.v0';
const KILL_SWITCH_WIRING_NOOP_DRY_CHECK_VERSION = 0;
const BASELINE_COMMIT = '2bff807';

const PROJECT_ROOT = path.resolve(__dirname, '..', '..', '..');

const REQUIRED_OUTPUT_FIELDS = Object.freeze([
  'kill_switch_wiring_noop_dry_check',
  'validation_report',
]);
...
function evidenceRef(store, eventId, relation = 'kill_switch_wiring_noop_dry_check_validation') {
...
function normalizeScope(inputSignals = {}) {
  ...
  return {
    ...
    sessionId: inputSignals.sessionId || 'session-348',
...
function phaseRegistry(contract = {}) {
  ...
  return {
    expected_phases: expected.expected_phases || '1-47',
    current_through_phase: expected.phase_inventory_count || 47,
    phase_inventory_count: expected.phase_inventory_count || 47,
    ...clone(current),
    phase47_commit: BASELINE_COMMIT,
    phase47_delta: clone(expected.phase47_delta || {}),
    ...
    evidenceRefs: [
      evidenceRef('git', BASELINE_COMMIT, 'phase48-baseline'),
...
function registry(kind, contract = {}) {
  const count = Number(contract.expectedCounts?.[`${kind}_registry_count`] || 47);
...
function canonicalDryCheckInput(dryCheck = {}) {
  return {
    ...
    noop_dry_check_summary: dryCheck.noop_dry_check_summary,
...
function killSwitchWiringNoopDryCheckIdempotencyKey(dryCheck) {
  return `kill-switch-wiring-noop-dry-check:${stableHash(canonicalDryCheckInput(dryCheck))}`;
}
...
function buildDryCheck(options = {}) {
  ...
  const dryCheck = {
    schema: KILL_SWITCH_WIRING_NOOP_DRY_CHECK_SCHEMA_VERSION,
    version: KILL_SWITCH_WIRING_NOOP_DRY_CHECK_VERSION,
    dry_check_id: `kill-switch-wiring-noop-dry-check-${stableHash({
      scope,
    ...
    noop_dry_check_summary: withEvidence(
      contract.noopDryCheckSummaryExpected || {},
      'fixture',
      'noop-dry-check-summary',
    ),
  ...
  dryCheck.idempotency_key = killSwitchWiringNoopDryCheckIdempotencyKey(dryCheck);
  return dryCheck;
}
...
function registryCountsOk(dryCheck = {}, contract = {}) {
  const count = Number(contract.expectedCounts?.phase_inventory_count || 47);
...
function commitChainOk(dryCheck = {}, contract = {}) {
  ...
  return valuesMatch(dryCheck.commit_chain, expected)
    && asArray(dryCheck.commit_chain).length === Number(contract.expectedCounts?.commit_chain_count || 35)
...
function sourceRecommendationOk(dryCheck = {}, contract = {}) {
  ...
  return containsExpectedValues(source, contract.sourceRecommendation || {})
    && source.recommendation_id === 'phase48-kill-switch-wiring-noop-dry-check-contract'
    && source.tier === 'tier1'
...
function closuresOk(dryCheck = {}) {
  return dryCheck.closure_summary?.oracle_186_phase47_review_closure_carried === true
    && asArray(dryCheck.closure_summary?.closed_review_refs).includes('ORACLE #186');
}

function sourceRefsOk(dryCheck = {}, contract = {}) {
  return valuesMatch(dryCheck.source_refs, contract.sourceRefsExpected || [])
    && asArray(dryCheck.source_refs).length === Number(contract.expectedCounts?.source_ref_count || 17)
    && asArray(dryCheck.source_refs).some((ref) => ref.artifact_id === 'phase47-kill-switch-wiring-preimplementation-checklist');
}

function noopSummaryOk(dryCheck = {}, contract = {}) {
  const summary = dryCheck.noop_dry_check_summary || {};
  return containsExpectedValues(summary, contract.noopDryCheckSummaryExpected || {})
    && summary.noop_dry_check_count === asArray(dryCheck.noop_dry_check_items).length
...
function unsafeActionDriftOk(dryCheck = {}, contract = {}) {
  const relevant = [
    ...
    dryCheck.noop_dry_check_summary,
...
function checkMap(dryCheck = {}, contract = {}) {
  ...
  const noopSummary = noopSummaryOk(dryCheck, contract);
  ...
  return {
    'static-schema-and-baseline-pinned': dryCheck.schema === KILL_SWITCH_WIRING_NOOP_DRY_CHECK_SCHEMA_VERSION
      && dryCheck.version === KILL_SWITCH_WIRING_NOOP_DRY_CHECK_VERSION
      && dryCheck.baseline_commit === BASELINE_COMMIT,
    'accept-schema-baseline-2bff807': dryCheck.schema === KILL_SWITCH_WIRING_NOOP_DRY_CHECK_SCHEMA_VERSION
      && dryCheck.version === KILL_SWITCH_WIRING_NOOP_DRY_CHECK_VERSION
      && dryCheck.baseline_commit === BASELINE_COMMIT,
    'static-phase-registries-through-47': registryCountsOk(dryCheck, contract),
    'accept-registries-47-47-47': registryCountsOk(dryCheck, contract),
    'static-commit-chain-count-and-tail': commitChainOk(dryCheck, contract),
    'accept-commit-chain-35-ending-2bff807': commitChainOk(dryCheck, contract),
    'static-phase47-tier0-satisfied': satisfiedPriorOk(dryCheck, contract),
    'accept-phase47-validator-satisfied': satisfiedPriorOk(dryCheck, contract),
    'static-phase47-tier1-selected': sourceRecommendationOk(dryCheck, contract),
    'accept-phase48-noop-dry-check-selected': sourceRecommendationOk(dryCheck, contract),
    'static-phase35-through-47-current': phasesCurrentOk(dryCheck, contract),
    'accept-phase35-through-47-current': phasesCurrentOk(dryCheck, contract),
    ...
    'static-oracle-closure-chain-through-186': closuresOk(dryCheck),
    'accept-oracle-closure-through-186': closuresOk(dryCheck),
    'static-noop-summary-contract-only': noopSummary,
    'accept-noop-summary-contract-only': noopSummary,
    ...
    'accept-blocked-future-slices-four': blockedOk
      && asArray(dryCheck.blocked_future_slices).length === Number(contract.expectedCounts?.blocked_future_slice_count || 4),
    ...
    'dry-check-contract-complete': outputFieldsOk && literalOk,
    ...
    'noop-summary-bound': noopSummary,
...
function validateDryCheck(dryCheck = {}, contract = {}) {
  ...
  const outputShapeOk = dryCheck.schema === KILL_SWITCH_WIRING_NOOP_DRY_CHECK_SCHEMA_VERSION
  ...
  add('dry-check-contract-complete',
    outputShapeOk
...
function buildValidationReport(dryCheck, contract = {}, generatedAt = dryCheck.generated_at) {
  ...
  const report = {
    ...
    version: KILL_SWITCH_WIRING_NOOP_DRY_CHECK_VERSION,
    validation_report_id: `kill-switch-wiring-noop-dry-check-validation-${stableHash({
      dry_check_key: dryCheck.idempotency_key,
      generatedAt,
    }).slice(0, 12)}`,
    ...
    scoped_status: {
      ...
      scoped_phase48_files_only: true,
...
function buildMiraCoreKillSwitchWiringNoopDryCheck(options = {}) {
  const contract = options.contract || {};
  const kill_switch_wiring_noop_dry_check = buildDryCheck(options);
  const validation_report = buildValidationReport(
    kill_switch_wiring_noop_dry_check,
    contract,
    kill_switch_wiring_noop_dry_check.generated_at,
  );
  const output = { kill_switch_wiring_noop_dry_check, validation_report };
  assertNoForbiddenOutput(output, asArray(contract.forbiddenOutputSubstrings));
  return output;
}

function validateMiraCoreKillSwitchWiringNoopDryCheckOutput(output = {}, contract = {}) {
  const checks = [];
  const errors = [];
  const add = (id, ok) => {
  ...
  const dryCheck = output.kill_switch_wiring_noop_dry_check || {};
  ...
  add('output-shape-complete',
    hasRequiredFields(output, contract.expectedOutputShape?.requiredTopLevelFields || REQUIRED_OUTPUT_FIELDS)
      && dryCheck.schema === KILL_SWITCH_WIRING_NOOP_DRY_CHECK_SCHEMA_VERSION
