...
const FIXTURE_REF =
  'ui/__tests__/fixtures/mira-core-kill-switch-wiring-readiness-rollup-through-phase51-contract.json';
const KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE51_SCHEMA_VERSION =
  'squidrun.mira_core.kill_switch_wiring_readiness_rollup_through_phase51.v0';
const VALIDATION_REPORT_SCHEMA_VERSION =
  'squidrun.mira_core.kill_switch_wiring_readiness_rollup_through_phase51_validation_report.v0';
const KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE51_VERSION = 0;
const BASELINE_COMMIT = '35febf1';

const PROJECT_ROOT = path.resolve(__dirname, '..', '..', '..');

const REQUIRED_OUTPUT_FIELDS = Object.freeze([
  'kill_switch_wiring_readiness_rollup_through_phase51',
  'validation_report',
]);
...
function evidenceRef(store, eventId, relation = 'kill_switch_wiring_readiness_rollup_through_phase51_validation') {
...
function currentTruthValues(contract = {}) {
  return {
    ...
    kill_switch_wiring_readiness_rollup_through_phase51_remains_non_authorizing: true,
...
function killSwitchWiringReadinessRollupThroughPhase51IdempotencyKey(dryCheck) {
  return `kill-switch-wiring-readiness-rollup-through-phase51:${stableHash(canonicalDryCheckInput(dryCheck))}`;
}
...
function buildDryCheck(options = {}) {
  ...
  const dryCheck = {
    schema: KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE51_SCHEMA_VERSION,
    version: KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE51_VERSION,
    rollup_id: `kill-switch-wiring-readiness-rollup-through-phase51-${stableHash({
      scope,
  ...
  dryCheck.idempotency_key = killSwitchWiringReadinessRollupThroughPhase51IdempotencyKey(dryCheck);
  return dryCheck;
}
...
function sourceRecommendationOk(dryCheck = {}, contract = {}) {
  ...
  return containsExpectedValues(source, contract.sourceRecommendation || {})
    && source.recommendation_id === 'phase52-kill-switch-wiring-readiness-rollup-through-phase51-contract'
    && source.tier === 'tier1'
...
function checkMap(dryCheck = {}, contract = {}) {
  ...
  return {
    'static-schema-and-baseline-pinned': dryCheck.schema === KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE51_SCHEMA_VERSION
      && dryCheck.version === KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE51_VERSION
      && dryCheck.baseline_commit === BASELINE_COMMIT,
    'accept-schema-baseline-35febf1': dryCheck.schema === KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE51_SCHEMA_VERSION
      && dryCheck.version === KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE51_VERSION
      && dryCheck.baseline_commit === BASELINE_COMMIT,
    ...
    'accept-commit-chain-36-ending-35febf1': commitChainOk(dryCheck, contract),
    'static-phase51-tier0-satisfied': satisfiedPriorOk(dryCheck, contract),
    'accept-phase51-validator-satisfied': satisfiedPriorOk(dryCheck, contract),
    'static-phase51-tier1-selected': sourceRecommendationOk(dryCheck, contract),
    'accept-phase51-readiness-rollup-selected': sourceRecommendationOk(dryCheck, contract),
    ...
    'static-readiness-rollup-through-phase51-contract-only': readinessRollup,
    'accept-readiness-rollup-through-phase51-contract-only': readinessRollup,
    'static-phase51-readiness-rollup-current': phase51ReadinessSummary,
    'accept-phase51-readiness-rollup-current': phase51ReadinessSummary,
    ...
    'static-readiness-rollup-through-phase51-non-authorizing': readinessRollup,
    'accept-readiness-rollup-through-phase51-non-authorizing': readinessRollup,
...
function validateDryCheck(dryCheck = {}, contract = {}) {
  ...
  const outputShapeOk = dryCheck.schema === KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE51_SCHEMA_VERSION
...
function buildValidationReport(dryCheck, contract = {}, generatedAt = dryCheck.generated_at) {
  ...
  const report = {
    ...
    version: KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE51_VERSION,
    validation_report_id: `kill-switch-wiring-readiness-rollup-through-phase51-validation-${stableHash({
    ...
    scoped_status: {
      ...
      scoped_phase51_files_only: true,
...
function buildMiraCoreKillSwitchWiringReadinessRollupThroughPhase51(options = {}) {
  const contract = options.contract || {};
  const kill_switch_wiring_readiness_rollup_through_phase51 = buildDryCheck(options);
  const validation_report = buildValidationReport(
    kill_switch_wiring_readiness_rollup_through_phase51,
    contract,
    kill_switch_wiring_readiness_rollup_through_phase51.generated_at,
  );
  const output = { kill_switch_wiring_readiness_rollup_through_phase51, validation_report };
  assertNoForbiddenOutput(output, asArray(contract.forbiddenOutputSubstrings));
  return output;
}

function validateMiraCoreKillSwitchWiringReadinessRollupThroughPhase51Output(output = {}, contract = {}) {
  const checks = [];
  const errors = [];
  const add = (id, ok) => {
  ...
  const dryCheck = output.kill_switch_wiring_readiness_rollup_through_phase51 || {};
  ...
  add('output-shape-complete',
    hasRequiredFields(output, contract.expectedOutputShape?.requiredTopLevelFields || REQUIRED_OUTPUT_FIELDS)
      && dryCheck.schema === KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE51_SCHEMA_VERSION
