...
const FIXTURE_REF =
  'ui/__tests__/fixtures/mira-core-kill-switch-wiring-readiness-rollup-through-phase56-contract.json';
const KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE56_SCHEMA_VERSION =
  'squidrun.mira_core.kill_switch_wiring_readiness_rollup_through_phase56.v0';
const VALIDATION_REPORT_SCHEMA_VERSION =
  'squidrun.mira_core.kill_switch_wiring_readiness_rollup_through_phase56_validation_report.v0';
const KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE56_VERSION = 0;
const BASELINE_COMMIT = 'a7dd3d6';

const PROJECT_ROOT = path.resolve(__dirname, '..', '..', '..');

const REQUIRED_OUTPUT_FIELDS = Object.freeze([
  'kill_switch_wiring_readiness_rollup_through_phase56',
  'validation_report',
]);
...
function evidenceRef(store, eventId, relation = 'kill_switch_wiring_readiness_rollup_through_phase56_validation') {
...
function phaseRegistry(contract = {}) {
  ...
  return {
    ...
    phase56_commit: BASELINE_COMMIT,
    ...
    evidenceRefs: [
      evidenceRef('git', BASELINE_COMMIT, 'phase56-baseline'),
...
function killSwitchWiringReadinessRollupThroughPhase56IdempotencyKey(dryCheck) {
  return `kill-switch-wiring-readiness-rollup-through-phase56:${stableHash(canonicalDryCheckInput(dryCheck))}`;
}
...
function buildDryCheck(options = {}) {
  ...
  const dryCheck = {
    schema: KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE56_SCHEMA_VERSION,
    version: KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE56_VERSION,
    rollup_id: `kill-switch-wiring-readiness-rollup-through-phase56-${stableHash({
      scope,
  ...
  dryCheck.idempotency_key = killSwitchWiringReadinessRollupThroughPhase56IdempotencyKey(dryCheck);
  return dryCheck;
}
...
function sourceRefsOk(dryCheck = {}, contract = {}) {
  ...
  return valuesMatch(dryCheck.source_refs, contract.sourceRefsExpected || [])
    ...
    && ['phase50', 'phase51', 'phase52', 'phase53', 'phase54', 'phase55', 'phase56'].every((phase) => (
...
function checkMap(dryCheck = {}, contract = {}) {
  ...
  const phase22CaveatOk = dryCheck.readiness_rollup_summary?.phase22_date_sensitive_expiry_caveat
    ...
    && dryCheck.readiness_rollup_summary?.phase22_caveat_blocks_phase57_fixture === false
  ...
  return {
    'static-schema-and-baseline-pinned': dryCheck.schema === KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE56_SCHEMA_VERSION
      && dryCheck.version === KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE56_VERSION
      && dryCheck.baseline_commit === BASELINE_COMMIT,
    'accept-schema-baseline-08881c8': dryCheck.schema === KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE56_SCHEMA_VERSION
      && dryCheck.version === KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE56_VERSION
      && dryCheck.baseline_commit === BASELINE_COMMIT,
...
function validateDryCheck(dryCheck = {}, contract = {}) {
  ...
  const outputShapeOk = dryCheck.schema === KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE56_SCHEMA_VERSION
...
function buildValidationReport(dryCheck, contract = {}, generatedAt = dryCheck.generated_at) {
  ...
  const report = {
    ...
    version: KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE56_VERSION,
    validation_report_id: `kill-switch-wiring-readiness-rollup-through-phase56-validation-${stableHash({
    ...
    scoped_status: {
      ...
      scoped_phase56_files_only: true,
...
function buildMiraCoreKillSwitchWiringReadinessRollupThroughPhase56(options = {}) {
  const contract = options.contract || {};
  const kill_switch_wiring_readiness_rollup_through_phase56 = buildDryCheck(options);
  const validation_report = buildValidationReport(
    kill_switch_wiring_readiness_rollup_through_phase56,
    contract,
    kill_switch_wiring_readiness_rollup_through_phase56.generated_at,
  );
  const output = { kill_switch_wiring_readiness_rollup_through_phase56, validation_report };
  assertNoForbiddenOutput(output, asArray(contract.forbiddenOutputSubstrings));
  return output;
}

function validateMiraCoreKillSwitchWiringReadinessRollupThroughPhase56Output(output = {}, contract = {}) {
  const checks = [];
  const errors = [];
  const add = (id, ok) => {
  ...
  const dryCheck = output.kill_switch_wiring_readiness_rollup_through_phase56 || {};
  ...
  add('output-shape-complete',
    hasRequiredFields(output, contract.expectedOutputShape?.requiredTopLevelFields || REQUIRED_OUTPUT_FIELDS)
      && dryCheck.schema === KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE56_SCHEMA_VERSION
...
module.exports = {
  ...
  KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE56_SCHEMA_VERSION,
  VALIDATION_REPORT_SCHEMA_VERSION,
  assertNoForbiddenOutput,
  buildMiraCoreKillSwitchWiringReadinessRollupThroughPhase56,
  killSwitchWiringReadinessRollupThroughPhase56IdempotencyKey,
  stableHash,
  validateMiraCoreKillSwitchWiringReadinessRollupThroughPhase56Output,
};
