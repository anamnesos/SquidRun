...
const FIXTURE_REF =
  'ui/__tests__/fixtures/mira-core-kill-switch-wiring-readiness-rollup-through-phase65-contract.json';
const OUTPUT_FIELD = 'kill_switch_wiring_readiness_rollup_through_phase65';
const KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE66_SCHEMA_VERSION =
  'squidrun.mira_core.kill_switch_wiring_readiness_rollup_through_phase65.v0';
const VALIDATION_REPORT_SCHEMA_VERSION =
  'squidrun.mira_core.kill_switch_wiring_readiness_rollup_through_phase65_validation_report.v0';
const KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE66_VERSION = 0;
const CURRENT_SCOPED_STATUS_KEY = 'scoped_phase66_through_phase65_files_only';
...
function evidenceRef(store, eventId, relation = 'kill_switch_wiring_readiness_rollup_through_phase65_validation') {
...
function normalizeScope(inputSignals = {}) {
  ...
  return {
    ...
    sessionId: inputSignals.sessionId || 'session-389',
...
function phaseRegistry(contract = {}) {
  ...
  return applyLiteralValues({
    ...
    evidenceRefs: [
      evidenceRef('git', BASELINE_COMMIT, 'phase65-baseline'),
...
function killSwitchWiringReadinessRollupThroughPhase66IdempotencyKey(dryCheck = {}) {
  return `kill-switch-wiring-readiness-rollup-through-phase65:${stableHash(canonicalDryCheckInput(dryCheck))}`;
}
...
function buildDryCheck(options = {}) {
  ...
  const manifest = {
    ...
    version: KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE66_VERSION,
    rollup_id: `kill-switch-wiring-readiness-rollup-through-phase65-${stableHash({
      generatedAt,
    ...
    evidenceRefs: [
      evidenceRef('git', BASELINE_COMMIT, 'phase65-baseline'),
      evidenceRef('fixture', FIXTURE_REF, 'phase66-contract'),
    ],
  ...
  manifest.idempotency_key = killSwitchWiringReadinessRollupThroughPhase66IdempotencyKey(manifest);
  return manifest;
}
...
function hybridArtifactMarkers() {
  return [
    ['phase65', 'boundary_refresh_summary'].join('_'),
    ['phase65', 'Boundary', 'Refresh', 'Summary'].join(''),
    ['phase65', 'boundary', 'refresh'].join('-'),
  ];
}
...
function sourceRefsOk(dryCheck = {}, contract = {}) {
  return valuesMatch(dryCheck.source_refs, contract.sourceRefsExpected || [])
    && asArray(dryCheck.source_refs).some((ref) => (
      ref.artifact_id === 'phase65-kill-switch-wiring-readiness-rollup-through-phase64'
    ));
}
...
function readinessRollupOk(dryCheck = {}, contract = {}) {
  ...
  return containsExpectedValues(summary, contract.readinessRollupSummaryExpected || {})
    && summary.current_through_phase === 65
    && summary.phase22_caveat_blocks_phase66_fixture === false
...
function duplicateRegistryClosureOk(dryCheck = {}, contract = {}) {
  const counts = contract.expectedCounts || {};
  return counts.schema_registry_count === 65
    && counts.cli_registry_count === 65
    && dryCheck.phase_registry?.schema_registry_count === 65
    && dryCheck.phase_registry?.cli_registry_count === 65
    ...
    && dryCheck.readiness_rollup_summary?.phase65_duplicate_registry_count_closure_carried === true
    && dryCheck.phase_registry?.phase65_duplicate_registry_count_closure_current === true
    && dryCheck.current_truth?.phase65_duplicate_registry_count_closure_current === true
    && dryCheck.closure_summary?.phase65_duplicate_registry_count_closure_carried === true
    && dryCheck.phase65_readiness_rollup_summary?.duplicate_registry_count_closure
      === 'phase_registry_schema_registry_count_and_cli_registry_count_bound_to_expectedCounts_65_in_phase66_with_phase65_closure_carried';
}
...
function phase22CaveatOk(dryCheck = {}) {
  ...
  return summary.phase22_date_sensitive_expiry_caveat
    ...
    && allBlockFlagsFalse
    && dryCheck.phase65_readiness_rollup_summary?.phase22_caveat_out_of_scope_non_blocking === true
    && dryCheck.phase65_readiness_rollup_summary?.phase22_caveat_non_authorizing === true;
}
...
function validationReportIdentityOk(report = {}, contract = {}) {
  return report.schema === (contract.expectedOutputShape?.reportSchema || VALIDATION_REPORT_SCHEMA_VERSION)
    ...
    && report.version === KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE66_VERSION;
}
...
function checkMap(dryCheck = {}, contract = {}, report = null) {
  ...
  const idempotencyOk = dryCheck.idempotency_key === killSwitchWiringReadinessRollupThroughPhase66IdempotencyKey(dryCheck);
  ...
  return allIds.reduce((acc, id) => {
    ...
    if (/readiness-rollup-through-phase65-contract-only|readiness-rollup-through-phase65-non-authorizing/.test(id)) {
      ok = readinessOk && phase65SummaryOk && phase22Ok;
    }
    acc[id] = ok === true;
    return acc;
  }, {
...
function buildValidationReport(dryCheck, contract = {}, generatedAt = dryCheck.generated_at) {
  ...
  const report = {
    ...
    validation_report_id: `kill-switch-wiring-readiness-rollup-through-phase65-validation-${stableHash({
    ...
    summary: {
      ...
      phase65_validation_green: dryCheck.phase65_readiness_rollup_summary?.validation_green,
...
function buildMiraCoreKillSwitchWiringReadinessRollupThroughPhase66(options = {}) {
  const contract = options.contract || {};
...
function validateMiraCoreKillSwitchWiringReadinessRollupThroughPhase66Output(output = {}, contract = {}) {
  const checks = [];
  const errors = [];
  const add = (id, ok) => {
