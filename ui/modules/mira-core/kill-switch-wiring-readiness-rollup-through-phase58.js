...
const FIXTURE_REF =
  'ui/__tests__/fixtures/mira-core-kill-switch-wiring-readiness-rollup-through-phase58-contract.json';
const KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE58_SCHEMA_VERSION =
  'squidrun.mira_core.kill_switch_wiring_readiness_rollup_through_phase58.v0';
const VALIDATION_REPORT_SCHEMA_VERSION =
  'squidrun.mira_core.kill_switch_wiring_readiness_rollup_through_phase58_validation_report.v0';
const KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE58_VERSION = 0;
const CURRENT_SCOPED_STATUS_KEY = 'scoped_phase59_through_phase58_files_only';
const STALE_SCOPED_STATUS_KEYS = [
  'scoped_phase56_files_only',
  'scoped_phase58_through_phase57_files_only',
];
const BASELINE_COMMIT = '02d48cc';

const PROJECT_ROOT = path.resolve(__dirname, '..', '..', '..');

const REQUIRED_OUTPUT_FIELDS = Object.freeze([
  'kill_switch_wiring_readiness_rollup_through_phase58',
  'validation_report',
]);
...
function evidenceRef(store, eventId, relation = 'kill_switch_wiring_readiness_rollup_through_phase58_validation') {
...
function phaseRegistry(contract = {}) {
  ...
  return {
    ...
    phase58_commit: BASELINE_COMMIT,
    ...
    evidenceRefs: [
      evidenceRef('git', BASELINE_COMMIT, 'phase58-baseline'),
...
function killSwitchWiringReadinessRollupThroughPhase58IdempotencyKey(dryCheck) {
  return `kill-switch-wiring-readiness-rollup-through-phase58:${stableHash(canonicalDryCheckInput(dryCheck))}`;
}
...
function buildDryCheck(options = {}) {
  ...
  const dryCheck = {
    schema: KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE58_SCHEMA_VERSION,
    version: KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE58_VERSION,
    rollup_id: `kill-switch-wiring-readiness-rollup-through-phase58-${stableHash({
      scope,
  ...
  dryCheck.idempotency_key = killSwitchWiringReadinessRollupThroughPhase58IdempotencyKey(dryCheck);
  return dryCheck;
}
...
function sourceRefsOk(dryCheck = {}, contract = {}) {
  ...
  return valuesMatch(dryCheck.source_refs, contract.sourceRefsExpected || [])
    ...
    && ['phase50', 'phase51', 'phase52', 'phase53', 'phase54', 'phase55', 'phase56', 'phase57', 'phase58'].every((phase) => (
...
function checkMap(dryCheck = {}, contract = {}) {
  ...
  const phase22CaveatOk = dryCheck.readiness_rollup_summary?.phase22_date_sensitive_expiry_caveat
    ...
    && dryCheck.readiness_rollup_summary?.phase22_caveat_blocks_phase59_fixture === false
  ...
  return {
    'static-schema-and-baseline-pinned': dryCheck.schema === KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE58_SCHEMA_VERSION
      && dryCheck.version === KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE58_VERSION
      && dryCheck.baseline_commit === BASELINE_COMMIT,
    'accept-schema-baseline-08881c8': dryCheck.schema === KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE58_SCHEMA_VERSION
      && dryCheck.version === KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE58_VERSION
      && dryCheck.baseline_commit === BASELINE_COMMIT,
...
function validateDryCheck(dryCheck = {}, contract = {}) {
  ...
  const outputShapeOk = dryCheck.schema === KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE58_SCHEMA_VERSION
...
function buildValidationReport(dryCheck, contract = {}, generatedAt = dryCheck.generated_at) {
  ...
  const report = {
    ...
    version: KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE58_VERSION,
    validation_report_id: `kill-switch-wiring-readiness-rollup-through-phase58-validation-${stableHash({
...
function buildMiraCoreKillSwitchWiringReadinessRollupThroughPhase58(options = {}) {
  const contract = options.contract || {};
  const kill_switch_wiring_readiness_rollup_through_phase58 = buildDryCheck(options);
  const validation_report = buildValidationReport(
    kill_switch_wiring_readiness_rollup_through_phase58,
    contract,
    kill_switch_wiring_readiness_rollup_through_phase58.generated_at,
  );
  const output = { kill_switch_wiring_readiness_rollup_through_phase58, validation_report };
  assertNoForbiddenOutput(output, asArray(contract.forbiddenOutputSubstrings));
  return output;
}

function validateMiraCoreKillSwitchWiringReadinessRollupThroughPhase58Output(output = {}, contract = {}) {
  const checks = [];
  const errors = [];
  const add = (id, ok) => {
  ...
  const dryCheck = output.kill_switch_wiring_readiness_rollup_through_phase58 || {};
  ...
  add('output-shape-complete',
    hasRequiredFields(output, contract.expectedOutputShape?.requiredTopLevelFields || REQUIRED_OUTPUT_FIELDS)
      && dryCheck.schema === KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE58_SCHEMA_VERSION
...
module.exports = {
  ...
  KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE58_SCHEMA_VERSION,
  VALIDATION_REPORT_SCHEMA_VERSION,
  assertNoForbiddenOutput,
  buildMiraCoreKillSwitchWiringReadinessRollupThroughPhase58,
  killSwitchWiringReadinessRollupThroughPhase58IdempotencyKey,
  stableHash,
  validateMiraCoreKillSwitchWiringReadinessRollupThroughPhase58Output,
};
