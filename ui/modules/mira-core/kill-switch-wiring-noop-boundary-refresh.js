...
const FIXTURE_REF =
  'ui/__tests__/fixtures/mira-core-kill-switch-wiring-noop-boundary-refresh-contract.json';
const KILL_SWITCH_WIRING_NOOP_BOUNDARY_REFRESH_SCHEMA_VERSION =
  'squidrun.mira_core.kill_switch_wiring_noop_boundary_refresh.v0';
const VALIDATION_REPORT_SCHEMA_VERSION =
  'squidrun.mira_core.kill_switch_wiring_noop_boundary_refresh_validation_report.v0';
const KILL_SWITCH_WIRING_NOOP_BOUNDARY_REFRESH_VERSION = 0;
const BASELINE_COMMIT = 'd57e546';

const PROJECT_ROOT = path.resolve(__dirname, '..', '..', '..');

const REQUIRED_OUTPUT_FIELDS = Object.freeze([
  'kill_switch_wiring_noop_boundary_refresh',
  'validation_report',
]);
...
function evidenceRef(store, eventId, relation = 'kill_switch_wiring_noop_boundary_refresh_validation') {
...
function phaseRegistry(contract = {}) {
  ...
  return {
    expected_phases: expected.expected_phases || '1-48',
    current_through_phase: expected.phase_inventory_count || 48,
    phase_inventory_count: expected.phase_inventory_count || 48,
    ...clone(current),
...
function registry(kind, contract = {}) {
  const count = Number(contract.expectedCounts?.[`${kind}_registry_count`] || 48);
...
function killSwitchWiringNoopBoundaryRefreshIdempotencyKey(dryCheck) {
  return `kill-switch-wiring-noop-boundary-refresh:${stableHash(canonicalDryCheckInput(dryCheck))}`;
}
...
function buildDryCheck(options = {}) {
  ...
  const dryCheck = {
    schema: KILL_SWITCH_WIRING_NOOP_BOUNDARY_REFRESH_SCHEMA_VERSION,
    version: KILL_SWITCH_WIRING_NOOP_BOUNDARY_REFRESH_VERSION,
    refresh_id: `kill-switch-wiring-noop-boundary-refresh-${stableHash({
      scope,
  ...
  dryCheck.idempotency_key = killSwitchWiringNoopBoundaryRefreshIdempotencyKey(dryCheck);
  return dryCheck;
}
...
function registryCountsOk(dryCheck = {}, contract = {}) {
  const count = Number(contract.expectedCounts?.phase_inventory_count || 48);
...
function commitChainOk(dryCheck = {}, contract = {}) {
  ...
  return valuesMatch(dryCheck.commit_chain, expected)
    && asArray(dryCheck.commit_chain).length === Number(contract.expectedCounts?.commit_chain_count || 36)
...
function sourceRecommendationOk(dryCheck = {}, contract = {}) {
  ...
  return containsExpectedValues(source, contract.sourceRecommendation || {})
    && source.recommendation_id === 'phase49-kill-switch-wiring-noop-boundary-refresh-contract'
    && source.tier === 'tier1'
...
function closuresOk(dryCheck = {}) {
  return dryCheck.closure_summary?.oracle_190_phase48_review_closure_carried === true
    && asArray(dryCheck.closure_summary?.closed_review_refs).includes('ORACLE #190');
}

function sourceRefsOk(dryCheck = {}, contract = {}) {
  return valuesMatch(dryCheck.source_refs, contract.sourceRefsExpected || [])
    && asArray(dryCheck.source_refs).length === Number(contract.expectedCounts?.source_ref_count || 18)
    && asArray(dryCheck.source_refs).some((ref) => ref.artifact_id === 'phase48-kill-switch-wiring-noop-dry-check');
}
...
function checkMap(dryCheck = {}, contract = {}) {
  ...
  return {
    'static-schema-and-baseline-pinned': dryCheck.schema === KILL_SWITCH_WIRING_NOOP_BOUNDARY_REFRESH_SCHEMA_VERSION
      && dryCheck.version === KILL_SWITCH_WIRING_NOOP_BOUNDARY_REFRESH_VERSION
      && dryCheck.baseline_commit === BASELINE_COMMIT,
    'accept-schema-baseline-d57e546': dryCheck.schema === KILL_SWITCH_WIRING_NOOP_BOUNDARY_REFRESH_SCHEMA_VERSION
      && dryCheck.version === KILL_SWITCH_WIRING_NOOP_BOUNDARY_REFRESH_VERSION
      && dryCheck.baseline_commit === BASELINE_COMMIT,
    'static-phase-registries-through-48': registryCountsOk(dryCheck, contract),
    ...
    'accept-commit-chain-36-ending-d57e546': commitChainOk(dryCheck, contract),
    'static-phase48-tier0-satisfied': satisfiedPriorOk(dryCheck, contract),
    'accept-phase48-validator-satisfied': satisfiedPriorOk(dryCheck, contract),
    'static-phase48-tier1-selected': sourceRecommendationOk(dryCheck, contract),
    'accept-phase49-noop-boundary-refresh-selected': sourceRecommendationOk(dryCheck, contract),
    'static-phase35-through-48-current': phasesCurrentOk(dryCheck, contract),
    'accept-phase35-through-48-current': phasesCurrentOk(dryCheck, contract),
    ...
    'static-oracle-closure-chain-through-190': closuresOk(dryCheck),
    'accept-oracle-closure-through-190': closuresOk(dryCheck),
...
function validateDryCheck(dryCheck = {}, contract = {}) {
  ...
  const outputShapeOk = dryCheck.schema === KILL_SWITCH_WIRING_NOOP_BOUNDARY_REFRESH_SCHEMA_VERSION
...
function buildValidationReport(dryCheck, contract = {}, generatedAt = dryCheck.generated_at) {
  ...
  const report = {
    ...
    version: KILL_SWITCH_WIRING_NOOP_BOUNDARY_REFRESH_VERSION,
    validation_report_id: `kill-switch-wiring-noop-boundary-refresh-validation-${stableHash({
    ...
    scoped_status: {
      ...
      scoped_phase49_files_only: true,
...
function buildMiraCoreKillSwitchWiringNoopBoundaryRefresh(options = {}) {
  const contract = options.contract || {};
  const kill_switch_wiring_noop_boundary_refresh = buildDryCheck(options);
  const validation_report = buildValidationReport(
    kill_switch_wiring_noop_boundary_refresh,
    contract,
    kill_switch_wiring_noop_boundary_refresh.generated_at,
  );
  const output = { kill_switch_wiring_noop_boundary_refresh, validation_report };
  assertNoForbiddenOutput(output, asArray(contract.forbiddenOutputSubstrings));
  return output;
}

function validateMiraCoreKillSwitchWiringNoopBoundaryRefreshOutput(output = {}, contract = {}) {
  const checks = [];
  const errors = [];
  const add = (id, ok) => {
  ...
  const dryCheck = output.kill_switch_wiring_noop_boundary_refresh || {};
  ...
  add('output-shape-complete',
    hasRequiredFields(output, contract.expectedOutputShape?.requiredTopLevelFields || REQUIRED_OUTPUT_FIELDS)
      && dryCheck.schema === KILL_SWITCH_WIRING_NOOP_BOUNDARY_REFRESH_SCHEMA_VERSION
