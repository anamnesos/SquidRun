...
const FIXTURE_REF =
  'ui/__tests__/fixtures/mira-core-kill-switch-wiring-readiness-rollup-through-phase66-contract.json';
const OUTPUT_FIELD = 'kill_switch_wiring_readiness_rollup_through_phase66';
const KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE67_SCHEMA_VERSION =
  'squidrun.mira_core.kill_switch_wiring_readiness_rollup_through_phase66.v0';
const VALIDATION_REPORT_SCHEMA_VERSION =
  'squidrun.mira_core.kill_switch_wiring_readiness_rollup_through_phase66_validation_report.v0';
const KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE67_VERSION = 0;
const CURRENT_SCOPED_STATUS_KEY = 'scoped_phase67_through_phase66_files_only';
const STALE_SCOPED_STATUS_KEYS = Object.freeze([
  ...
  'scoped_phase65_through_phase64_files_only',
  'scoped_phase66_through_phase65_files_only',
]);
const BASELINE_COMMIT = '65d9343';
...
const REQUIRED_DRY_CHECK_FIELDS = Object.freeze([
  ...
  'phase65_readiness_rollup_summary',
  'phase66_readiness_rollup_summary',
]);
...
function evidenceRef(store, eventId, relation = 'kill_switch_wiring_readiness_rollup_through_phase66_validation') {
...
function normalizeScope(inputSignals = {}) {
  ...
  return {
    ...
    sessionId: inputSignals.sessionId || 'session-392',
...
function phaseRegistry(contract = {}) {
  ...
  return applyLiteralValues({
    ...
    phase65_delta: clone(expected.phase65_delta || {}),
    phase66_delta: clone(expected.phase66_delta || {}),
    evidenceRefs: [
      evidenceRef('git', BASELINE_COMMIT, 'phase66-baseline'),
...
function closureSummaryValues(contract = {}) {
  return {
    ...
    oracle_297_phase65_review_closure_carried: true,
    phase65_manifest_schema_consistency_closure_carried: true,
    phase65_scoped_status_prior_closure_only_carried: true,
    phase65_scoped_status_closure_carried: true,
    phase65_duplicate_registry_count_closure_carried: true,
    oracle_301_phase66_review_closure_carried: true,
    phase66_manifest_schema_consistency_closure_carried: true,
    phase66_scoped_status_prior_closure_only_carried: true,
    phase66_scoped_status_closure_carried: true,
    phase66_duplicate_registry_count_closure_carried: true,
    phase66_fixture_literal_fix_closure_carried: true,
  };
}
...
function canonicalDryCheckInput(dryCheck = {}) {
  const copy = {
    ...
    phase65_readiness_rollup_summary: dryCheck.phase65_readiness_rollup_summary,
    phase66_readiness_rollup_summary: dryCheck.phase66_readiness_rollup_summary,
...
function killSwitchWiringReadinessRollupThroughPhase67IdempotencyKey(dryCheck = {}) {
  return `kill-switch-wiring-readiness-rollup-through-phase66:${stableHash(canonicalDryCheckInput(dryCheck))}`;
}
...
function buildDryCheck(options = {}) {
  ...
  const manifest = {
    ...
    version: KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE67_VERSION,
    rollup_id: `kill-switch-wiring-readiness-rollup-through-phase66-${stableHash({
      generatedAt,
    ...
    phase65_readiness_rollup_summary: withEvidence(
      contract.phase65ReadinessRollupSummaryExpected || {},
      'fixture',
      'phase65-readiness-rollup-summary',
    ),
    phase66_readiness_rollup_summary: withEvidence(
      contract.phase66ReadinessRollupSummaryExpected || {},
      'fixture',
      'phase66-readiness-rollup-summary',
    ),
    ...
    evidenceRefs: [
      evidenceRef('git', BASELINE_COMMIT, 'phase66-baseline'),
      evidenceRef('fixture', FIXTURE_REF, 'phase67-contract'),
    ],
  ...
  manifest.idempotency_key = killSwitchWiringReadinessRollupThroughPhase67IdempotencyKey(manifest);
  return manifest;
}
...
function hybridArtifactMarkers() {
  return [
    ['phase66', 'boundary_refresh_summary'].join('_'),
    ['phase66', 'Boundary', 'Refresh', 'Summary'].join(''),
    ['phase66', 'boundary', 'refresh'].join('-'),
  ];
}
...
function sourceRefsOk(dryCheck = {}, contract = {}) {
  return valuesMatch(dryCheck.source_refs, contract.sourceRefsExpected || [])
    && asArray(dryCheck.source_refs).some((ref) => (
      ref.artifact_id === 'phase66-kill-switch-wiring-readiness-rollup-through-phase65'
    ));
}
...
function readinessRollupOk(dryCheck = {}, contract = {}) {
  ...
  return containsExpectedValues(summary, contract.readinessRollupSummaryExpected || {})
    && summary.current_through_phase === 66
    && summary.phase22_caveat_blocks_phase67_fixture === false
    ...
    && summary.phase65_validator_satisfied === true
    && summary.phase65_readiness_rollup_validation_current === true
    && summary.phase65_manifest_schema_consistency_closure_carried === true
    && summary.phase65_scoped_status_prior_closure_only === true
    && summary.phase65_scoped_status_closure_carried === true
    && summary.phase65_duplicate_registry_count_closure_carried === true
    && summary.phase66_validator_satisfied === true
    && summary.phase66_readiness_rollup_validation_current === true
    && summary.phase66_manifest_schema_consistency_closure_carried === true
    && summary.phase66_scoped_status_prior_closure_only === true
    && summary.phase66_scoped_status_closure_carried === true
    && summary.phase66_duplicate_registry_count_closure_carried === true
    && summary.phase66_fixture_literal_fix_closure_carried === true
...
function phase65ReadinessRollupSummaryOk(dryCheck = {}, contract = {}) {
  const summary = dryCheck.phase65_readiness_rollup_summary || {};
  const expected = contract.phase65ReadinessRollupSummaryExpected || {};
  return containsExpectedValues(summary, expected)
    && summary.committed_baseline === expected.committed_baseline
    && summary.validation_green === true
    && summary.oracle_review_ref === 'ORACLE #297'
    && summary.architect_accept_ref === 'ARCH #927'
    && summary.builder_commit_ref === 'baseline 0ed11ca'
    && summary.manifest_schema_consistency_closure
      === 'manifest_schema_bound_to_expectedManifestShape_requiredLiteralValues_schema_and_report_schema_bound_to_expectedOutputShape_reportSchema'
    && summary.phase62_scoped_status_prior_closure_only === true
    && summary.old_phase64_current_scoped_ids_absent === true
    && summary.phase65_scoped_status_prior_closure_only === true
    && summary.scoped_status_closure
...
function phase65ScopedClosureOk(dryCheck = {}) {
  return dryCheck.readiness_rollup_summary?.phase65_scoped_status_closure_carried === true
    && dryCheck.readiness_rollup_summary?.phase65_scoped_status_prior_closure_only === true
    && dryCheck.phase65_readiness_rollup_summary?.phase62_scoped_status_prior_closure_only === true
    && dryCheck.phase65_readiness_rollup_summary?.old_phase64_current_scoped_ids_absent === true
    && dryCheck.phase65_readiness_rollup_summary?.phase65_scoped_status_prior_closure_only === true
    && dryCheck.phase65_readiness_rollup_summary?.scoped_status_closure
      === 'scoped_phase65_through_phase64_files_only_bound_and_stale_phase56_phase58_phase59_phase60_phase61_phase62_phase63_phase64_rejected'
    && dryCheck.closure_summary?.phase65_scoped_status_prior_closure_only_carried === true
    && dryCheck.closure_summary?.phase65_scoped_status_closure_carried === true;
}

function phase65ManifestSchemaClosureOk(dryCheck = {}) {
  return dryCheck.readiness_rollup_summary?.phase65_manifest_schema_consistency_closure_carried === true
    && dryCheck.phase_registry?.phase65_manifest_schema_consistency_closure_current === true
    && dryCheck.current_truth?.phase65_manifest_schema_consistency_closure_current === true
    && dryCheck.closure_summary?.phase65_manifest_schema_consistency_closure_carried === true
    && dryCheck.phase65_readiness_rollup_summary?.manifest_schema_consistency_closure
      === 'manifest_schema_bound_to_expectedManifestShape_requiredLiteralValues_schema_and_report_schema_bound_to_expectedOutputShape_reportSchema';
}

function phase66ReadinessRollupSummaryOk(dryCheck = {}, contract = {}) {
  const summary = dryCheck.phase66_readiness_rollup_summary || {};
  const expected = contract.phase66ReadinessRollupSummaryExpected || {};
  return containsExpectedValues(summary, expected)
    && summary.committed_baseline === expected.committed_baseline
    && summary.validation_green === true
    && summary.oracle_review_ref === 'ORACLE #301'
    && summary.architect_accept_ref === 'ARCH #936'
    && summary.builder_commit_ref === 'baseline 65d9343'
    && summary.manifest_schema_consistency_closure
      === 'manifest_schema_bound_to_expectedManifestShape_requiredLiteralValues_schema_and_report_schema_bound_to_expectedOutputShape_reportSchema'
    && summary.phase62_scoped_status_prior_closure_only === true
    && summary.old_phase64_current_scoped_ids_absent === true
    && summary.phase65_scoped_status_prior_closure_only === true
    && summary.phase66_scoped_status_prior_closure_only === true
    && summary.fixture_literal_fix_closure
...
function phase66ScopedClosureOk(dryCheck = {}) {
  return dryCheck.readiness_rollup_summary?.phase66_scoped_status_closure_carried === true
    && dryCheck.readiness_rollup_summary?.phase66_scoped_status_prior_closure_only === true
    && dryCheck.phase66_readiness_rollup_summary?.phase62_scoped_status_prior_closure_only === true
    && dryCheck.phase66_readiness_rollup_summary?.old_phase64_current_scoped_ids_absent === true
    && dryCheck.phase66_readiness_rollup_summary?.phase65_scoped_status_prior_closure_only === true
    && dryCheck.phase66_readiness_rollup_summary?.phase66_scoped_status_prior_closure_only === true
    && dryCheck.phase66_readiness_rollup_summary?.scoped_status_closure
      === 'scoped_phase66_through_phase65_files_only_bound_and_stale_phase56_phase58_phase59_phase60_phase61_phase62_phase63_phase64_phase65_rejected'
    && dryCheck.closure_summary?.phase66_scoped_status_prior_closure_only_carried === true
    && dryCheck.closure_summary?.phase66_scoped_status_closure_carried === true;
}

function phase66ManifestSchemaClosureOk(dryCheck = {}) {
  return dryCheck.readiness_rollup_summary?.phase66_manifest_schema_consistency_closure_carried === true
    && dryCheck.phase_registry?.phase66_manifest_schema_consistency_closure_current === true
    && dryCheck.current_truth?.phase66_manifest_schema_consistency_closure_current === true
    && dryCheck.closure_summary?.phase66_manifest_schema_consistency_closure_carried === true
    && dryCheck.phase66_readiness_rollup_summary?.manifest_schema_consistency_closure
      === 'manifest_schema_bound_to_expectedManifestShape_requiredLiteralValues_schema_and_report_schema_bound_to_expectedOutputShape_reportSchema';
}

function phase66FixtureLiteralFixClosureOk(dryCheck = {}) {
  return dryCheck.readiness_rollup_summary?.phase66_fixture_literal_fix_closure_carried === true
    && dryCheck.phase_registry?.phase66_fixture_literal_fix_closure_current === true
    && dryCheck.current_truth?.phase66_fixture_literal_fix_closure_current === true
    && dryCheck.closure_summary?.phase66_fixture_literal_fix_closure_carried === true
    && dryCheck.phase66_readiness_rollup_summary?.fixture_literal_fix_closure
      === 'commit_chain_length_53_and_last_0ed11ca_bound_in_phase66_fixture_and_carried_to_phase67';
}

function duplicateRegistryClosureOk(dryCheck = {}, contract = {}) {
  const counts = contract.expectedCounts || {};
  return counts.schema_registry_count === 66
    && counts.cli_registry_count === 66
    && dryCheck.phase_registry?.schema_registry_count === 66
    && dryCheck.phase_registry?.cli_registry_count === 66
    ...
    && dryCheck.readiness_rollup_summary?.phase66_duplicate_registry_count_closure_carried === true
    && dryCheck.phase_registry?.phase66_duplicate_registry_count_closure_current === true
    && dryCheck.current_truth?.phase66_duplicate_registry_count_closure_current === true
    && dryCheck.closure_summary?.phase66_duplicate_registry_count_closure_carried === true
    && dryCheck.phase66_readiness_rollup_summary?.duplicate_registry_count_closure
      === 'phase_registry_schema_registry_count_and_cli_registry_count_bound_to_expectedCounts_66_in_phase67_with_phase66_closure_carried';
}

function phase22CaveatOk(dryCheck = {}) {
  const summary = dryCheck.readiness_rollup_summary || {};
  const allBlockFlagsFalse = Object.entries(summary)
    .every(([key, value]) => !/^phase22_caveat_blocks_phase\d+_fixture$/.test(key) || value === false);
  return summary.phase22_date_sensitive_expiry_caveat
      === 'non_authorizing_non_blocking_out_of_scope'
    && summary.phase22_caveat_authorizes_runtime === false
    && allBlockFlagsFalse
    && dryCheck.phase66_readiness_rollup_summary?.phase22_caveat_out_of_scope_non_blocking === true
    && dryCheck.phase66_readiness_rollup_summary?.phase22_caveat_non_authorizing === true;
}
...
function summariesOk(dryCheck = {}, contract = {}) {
  const summaryPairs = [
    ...
    ['phase65_readiness_rollup_summary', contract.phase65ReadinessRollupSummaryExpected],
    ['phase66_readiness_rollup_summary', contract.phase66ReadinessRollupSummaryExpected],
...
function validationReportIdentityOk(report = {}, contract = {}) {
  return report.schema === (contract.expectedOutputShape?.reportSchema || VALIDATION_REPORT_SCHEMA_VERSION)
    ...
    && report.version === KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE67_VERSION;
}
...
function checkMap(dryCheck = {}, contract = {}, report = null) {
  ...
  const idempotencyOk = dryCheck.idempotency_key === killSwitchWiringReadinessRollupThroughPhase67IdempotencyKey(dryCheck);
  ...
  const phase65SummaryOk = phase65ReadinessRollupSummaryOk(dryCheck, contract);
  const phase65ScopedOk = phase65ScopedClosureOk(dryCheck);
  const phase65ManifestSchemaOk = phase65ManifestSchemaClosureOk(dryCheck);
  const phase66SummaryOk = phase66ReadinessRollupSummaryOk(dryCheck, contract);
  const phase66ScopedOk = phase66ScopedClosureOk(dryCheck);
  const phase66ManifestSchemaOk = phase66ManifestSchemaClosureOk(dryCheck);
  const phase66FixtureLiteralOk = phase66FixtureLiteralFixClosureOk(dryCheck);
  ...
  return allIds.reduce((acc, id) => {
    ...
    if (/phase65-readiness-rollup-current/.test(id)) ok = phase65SummaryOk;
    if (/phase65-scoped-status-policy-carried-as-prior-closure/.test(id)) ok = phase65ScopedOk;
    if (/phase65-scoped-status-closure-carried/.test(id)) ok = phase65ScopedOk;
    if (/phase65-manifest-schema-consistency-closure-carried/.test(id)) ok = phase65ManifestSchemaOk;
    if (/phase66-readiness-rollup-current/.test(id)) ok = phase66SummaryOk;
    if (/phase66-scoped-status-policy-carried-as-prior-closure/.test(id)) ok = phase66ScopedOk;
    if (/phase66-scoped-status-closure-carried/.test(id)) ok = phase66ScopedOk;
    if (/phase66-manifest-schema-consistency-closure-carried/.test(id)) ok = phase66ManifestSchemaOk;
    if (/phase66-fixture-literal-fix-closure-carried/.test(id)) ok = phase66FixtureLiteralOk;
    ...
    if (/registry|commit-chain|baseline|literal|shape/.test(id)
      && !/duplicate-registry-count/.test(id)
    ...
    if (/readiness-rollup-through-phase66-contract-only|readiness-rollup-through-phase66-non-authorizing/.test(id)) {
      ok = readinessOk && phase66SummaryOk && phase22Ok;
    }
    acc[id] = ok === true;
    return acc;
  }, {
...
function buildValidationReport(dryCheck, contract = {}, generatedAt = dryCheck.generated_at) {
  ...
  const report = {
    ...
    validation_report_id: `kill-switch-wiring-readiness-rollup-through-phase66-validation-${stableHash({
    ...
    summary: {
      ...
      phase66_validation_green: dryCheck.phase66_readiness_rollup_summary?.validation_green,
...
function buildMiraCoreKillSwitchWiringReadinessRollupThroughPhase67(options = {}) {
  const contract = options.contract || {};
...
function validateMiraCoreKillSwitchWiringReadinessRollupThroughPhase67Output(output = {}, contract = {}) {
  const checks = [];
  const errors = [];
  const add = (id, ok) => {
