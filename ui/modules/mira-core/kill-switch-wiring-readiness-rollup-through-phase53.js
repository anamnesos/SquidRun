...
const FIXTURE_REF =
  'ui/__tests__/fixtures/mira-core-kill-switch-wiring-readiness-rollup-through-phase53-contract.json';
const KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE53_SCHEMA_VERSION =
  'squidrun.mira_core.kill_switch_wiring_readiness_rollup_through_phase53.v0';
const VALIDATION_REPORT_SCHEMA_VERSION =
  'squidrun.mira_core.kill_switch_wiring_readiness_rollup_through_phase53_validation_report.v0';
const KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE53_VERSION = 0;
const BASELINE_COMMIT = '08881c8';

const PROJECT_ROOT = path.resolve(__dirname, '..', '..', '..');

const REQUIRED_OUTPUT_FIELDS = Object.freeze([
  'kill_switch_wiring_readiness_rollup_through_phase53',
  'validation_report',
]);
...
function evidenceRef(store, eventId, relation = 'kill_switch_wiring_readiness_rollup_through_phase53_validation') {
...
function phaseRegistry(contract = {}) {
  ...
  return {
    ...
    evidenceRefs: [
      evidenceRef('git', BASELINE_COMMIT, 'phase53-baseline'),
...
function killSwitchWiringReadinessRollupThroughPhase53IdempotencyKey(dryCheck) {
  return `kill-switch-wiring-readiness-rollup-through-phase53:${stableHash(canonicalDryCheckInput(dryCheck))}`;
}
...
function buildDryCheck(options = {}) {
  ...
  const dryCheck = {
    schema: KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE53_SCHEMA_VERSION,
    version: KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE53_VERSION,
    rollup_id: `kill-switch-wiring-readiness-rollup-through-phase53-${stableHash({
      scope,
  ...
  dryCheck.idempotency_key = killSwitchWiringReadinessRollupThroughPhase53IdempotencyKey(dryCheck);
  return dryCheck;
}
...
function sourceRecommendationOk(dryCheck = {}, contract = {}) {
  ...
  return containsExpectedValues(source, contract.sourceRecommendation || {})
    && source.recommendation_id === 'phase54-kill-switch-wiring-readiness-rollup-through-phase53-contract'
    && source.tier === 'tier1'
...
function sourceRefsOk(dryCheck = {}, contract = {}) {
  return valuesMatch(dryCheck.source_refs, contract.sourceRefsExpected || [])
    ...
    && asArray(dryCheck.source_refs).some((ref) => ref.artifact_id === 'phase51-kill-switch-wiring-readiness-rollup-through-phase50')
    && asArray(dryCheck.source_refs).some((ref) => ref.artifact_id === 'phase52-kill-switch-wiring-readiness-rollup-through-phase51')
    && asArray(dryCheck.source_refs).some((ref) => ref.artifact_id === 'phase53-kill-switch-wiring-readiness-rollup-through-phase52');
}
...
function phase53HybridArtifactAbsent(dryCheck = {}, contract = {}) {
  ...
  return phase53HybridArtifactMarkers().every((marker) => (
...
function checkMap(dryCheck = {}, contract = {}) {
  ...
  const hybridAbsent = phase53HybridArtifactAbsent(dryCheck, contract);

  return {
    'static-schema-and-baseline-pinned': dryCheck.schema === KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE53_SCHEMA_VERSION
      && dryCheck.version === KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE53_VERSION
      && dryCheck.baseline_commit === BASELINE_COMMIT,
    'accept-schema-baseline-08881c8': dryCheck.schema === KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE53_SCHEMA_VERSION
      && dryCheck.version === KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE53_VERSION
      && dryCheck.baseline_commit === BASELINE_COMMIT,
    ...
    'accept-no-phase53-hybrid-artifact': hybridAbsent,
    'static-phase53-hybrid-artifact-absent': hybridAbsent,
...
function validateDryCheck(dryCheck = {}, contract = {}) {
  ...
  const outputShapeOk = dryCheck.schema === KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE53_SCHEMA_VERSION
...
function buildValidationReport(dryCheck, contract = {}, generatedAt = dryCheck.generated_at) {
  ...
  const report = {
    ...
    version: KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE53_VERSION,
    validation_report_id: `kill-switch-wiring-readiness-rollup-through-phase53-validation-${stableHash({
...
function buildMiraCoreKillSwitchWiringReadinessRollupThroughPhase53(options = {}) {
  const contract = options.contract || {};
  const kill_switch_wiring_readiness_rollup_through_phase53 = buildDryCheck(options);
  const validation_report = buildValidationReport(
    kill_switch_wiring_readiness_rollup_through_phase53,
    contract,
    kill_switch_wiring_readiness_rollup_through_phase53.generated_at,
  );
  const output = { kill_switch_wiring_readiness_rollup_through_phase53, validation_report };
  assertNoForbiddenOutput(output, asArray(contract.forbiddenOutputSubstrings));
  return output;
}

function validateMiraCoreKillSwitchWiringReadinessRollupThroughPhase53Output(output = {}, contract = {}) {
  const checks = [];
  const errors = [];
  const add = (id, ok) => {
  ...
  const dryCheck = output.kill_switch_wiring_readiness_rollup_through_phase53 || {};
  ...
  add('output-shape-complete',
    hasRequiredFields(output, contract.expectedOutputShape?.requiredTopLevelFields || REQUIRED_OUTPUT_FIELDS)
      && dryCheck.schema === KILL_SWITCH_WIRING_READINESS_ROLLUP_THROUGH_PHASE53_SCHEMA_VERSION
