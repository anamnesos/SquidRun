...
const SOURCE_ACTION_SUBSTRATE_REGISTRY = Object.freeze([
  ...
  },
  {
    ...
    status: 'active',
    priority: 94,
    existing_seams: ['memory-broker.js', 'hm-memory-broker.js recall', 'mira-memory-curiosity.js', 'cognitive-memory-api.js', 'memory-search/retrieve', 'team-memory/*', 'memory-ingest/*', 'hm-memory-api.js retrieve'],
    first_probe: 'Use the unified memory broker first, then inspect lower-level memory seams only when the broker result is weak or missing a store.',
  ...
  },
  {
    ...
    scope: 'hybrid_vector_graph_episodic_recall',
    strategy: SUBSTRATE_STRATEGIES.active_memory,
    status: 'active',
    priority: 92,
    existing_seams: ['ui/modules/memory-broker.js recall(query, context)', 'ui/scripts/hm-memory-broker.js recall', 'deliverHumanMessageWithRecall unified recall injection'],
    first_probe: 'Run hm-memory-broker recall for the active question and route from the merged contributors rather than a single store.',
    why_this_strategy: 'The broker turns vector/cognitive, graph/team, and episodic/ledger recall into one ranked action surface so Mira stops treating memory as fragmented archives.',
  },
  {
  ...
  {
    source: 'mcp_compatible_connectors',
    scope: 'ecosystem_ports_when_native_seams_are_not_enough',
    strategy: SUBSTRATE_STRATEGIES.mcp_connector,
    status: 'active',
    priority: 61,
    existing_seams: ['mcp-bridge.js', 'browser/email/calendar MCP-compatible connector candidates', 'source/action substrate connector map'],
    first_probe: 'Choose an MCP-compatible connector only where ecosystem access is faster than a native SquidRun adapter or code-mode wrapper.',
    why_this_strategy: 'MCP is a port shape inside the broader source/action substrate; native SquidRun seams still win when local code already exposes the signal.',
  },
  {
    ...
    first_probe: 'Read compact local calendar/message artifact metadata plus hm-comms sender/target/status/timestamp counts before any body reads, sends, or calendar mutations.',
    why_this_strategy: 'Native SquidRun comms is the first useful calendar/message seam because it exposes obligation and thread pressure without jumping to calendar or Gmail APIs.',
  },
]);
