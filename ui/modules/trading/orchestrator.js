...
function normalizeSymbols(symbols) {
	return dataIngestion.normalizeSymbols(Array.isArray(symbols) ? symbols : (symbols ? [symbols] : watchlist.getTickers()));
}
...
class TradingOrchestrator {
	...
	async backfillMissingSignals(symbols, options = {}) {
		...
		for (const [agentId, missingTickers] of missingByAgent) {
			const signalOptions = alpacaClient
				? { client: alpacaClient, emptyPortfolio: Boolean(options.emptyPortfolio) }
	...
	async runPreMarket(options = {}) {
		...
		const calendarDay = await tradingScheduler.getCalendarDay(options.date || new Date(), options).catch(() => null);
		...
		const autoGeneratedSignals = await this.backfillMissingSignals(symbols, { ...options, emptyPortfolio });
	...
	async runConsensusRound(options = {}) {
		...
		const snapshots = options.snapshots
			|| this.state.phases.premarket?.marketContext?.snapshots
	...
	applyWatchlistUpdates(options = {}) {
		...
		for (const entry of additions) {
			...
			watchlist.addToWatchlist(entry.ticker, entry.name || entry.ticker, entry.sector || 'Unspecified');
		}
	...
	async runEndOfDay(options = {}) {
		...
		const updatedWatchlist = typeof options.updateWatchlist === 'function'
			? await options.updateWatchlist({ watchlist: watchlist.getWatchlist(), trades: todaysTrades, accountState })
