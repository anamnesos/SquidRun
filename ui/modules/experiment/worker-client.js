...
const DEFAULT_REQUEST_TIMEOUT_MS = parsePositiveInt(
  Number.parseInt(process.env.SQUIDRUN_EXPERIMENT_WORKER_REQUEST_TIMEOUT_MS || '15000', 10),
  15000
);
...
function quarantineWorker(worker, reason = 'unresponsive') {
  ...
  const err = new Error(`experiment worker quarantined (${reason})`);
  err.code = 'EXPERIMENT_WORKER_QUARANTINED';
  ...
  try {
    ...
    log.warn('ExperimentWorker', `Failed to kill quarantined worker (${reason}): ${killErr.message}`);
  }
}
