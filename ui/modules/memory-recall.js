...
function shouldBoostKoreanCase(context = {}, message = '') {
  ...
  return /은별|eunbyeol|전명삼|힐스테이트|hillstate|큐라인|qeline|동인동디엠|관세청|customs|korean fraud/.test(lowered);
}

function shouldBoostJamesDirectInput(context = {}) {
  if (context.assumeJamesDirectInput === true) return true;
...
function buildRecallQueryFromMessage(message, context = {}) {
  ...
  if (shouldBoostJamesDirectInput(context)) {
...
function buildStartupRecallQuery(context = {}) {
  ...
  const seed = {
    architect: 'current session priorities recent decisions user preferences active investigations blockers eunbyeol james telegram customs trading',
    builder: 'current session priorities recent decisions user preferences active investigations blockers runtime implementation supervisor trading eunbyeol james telegram',
    oracle: 'current session priorities recent decisions user preferences active investigations blockers research documentation evidence customs eunbyeol james telegram',
  };
...
function buildTimeAwareness(input = {}) {
  ...
  const hyperliquidCheckMs = selectLatestTimestamp(
    override.lastHyperliquidCheckAtMs,
    supervisorStatus?.hyperliquidPositionMonitor?.lastSummary?.checkedAt,
    supervisorStatus?.hyperliquidPositionMonitor?.checkedAt
  );
  raw.lastHyperliquidCheckAtMs = hyperliquidCheckMs;
  ...
  if (shouldBoostKoreanCase(input, input.message || input.text || input.query || '')) {
    ...
    const eunbyeolMs = selectLatestTimestamp(
      override.lastEunbyeolMessageAtMs,
      eunbyeolFromComms?.timestampMs
    );
    raw.lastEunbyeolMessageAtMs = eunbyeolMs;
    if (Number.isFinite(eunbyeolMs)) {
      lines.push(
        `Last 은별 message: ${describeElapsedSince(eunbyeolMs, {
          nowMs,
          zoneLabel: 'KST',
          zoneId: 'Asia/Seoul',
        })}`
