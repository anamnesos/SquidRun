...
const DEFAULT_HTML_PATH = path.join(__dirname, '..', '..', 'live-task-audit-sidecar.html');
...
function createLiveTaskAuditSidecarWindow(deps = {}) {
  const BrowserWindow = deps.BrowserWindow;
  if (!BrowserWindow || typeof BrowserWindow !== 'function') {
    throw new Error('createLiveTaskAuditSidecarWindow requires deps.BrowserWindow constructor');
  }
...
module.exports = {
  ...
  DEFAULT_WINDOW_OPTIONS,
  FORCED_WEB_PREFERENCES,
  createLiveTaskAuditSidecarWindow,
};
