...
- ui/modules/main/system-protected-evals.js: Pure static system-eval registry/runner for protected zero-fail SquidRun behaviors. Phase 4A binds the `accepted.unverified` visible-delivery invariant to concrete `hm-send.js` route-proof source refs and focused tests, with mutation-failable checks and no runtime, network, write, send, restart, or dispatcher side effects.
