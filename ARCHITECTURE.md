...
- mira/runtime/src/mission-control-route-preview.ts: State-root writer/list reader for Mission Control internal-route previews saved as `pending_internal_review` history under `MIRA_STATE_ROOT/mission-control/route-previews`, preserving manual-execution/no-send/no-route-flip/no-provider audit fields and storing no executable command.
