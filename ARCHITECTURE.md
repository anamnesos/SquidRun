...
- ui/modules/main/pane-failure-monitor.js: Discrete PTY failure monitor for core and workroom panes. It provider-gates exact CLI usage-refusal output, consumes unexpected process-exit events, latches one Today system row plus one Telegram attempt per pane/event state, and rearms on the next PTY spawn; it deliberately adds no silence timers or generic rate-limit heuristics.
...
- ui/modules/main/squidrun-authority-dispatcher.js: Pure Phase 3 authority dispatcher for Builder #67 policy buckets; converts salience/work/action candidates into audit-first `rank_only_no_permission`, `safe_auto`, `caution_gated`, or `approval_required` decisions with metadata-scope checks, required proofs, denied capabilities, and James checkpoints, while performing no direct side effects, restart activation, runtime watcher changes, network egress, or file writes.
...
- ui/modules/pane-session-id-store.js: Persists a stable per-pane CLI session UUID (`runtime/pane-session-ids.json`, `crypto.randomUUID`) so `--resume <id>` on respawn matches the `--session-id <id>` that created it — "stable across restarts" = stable-because-persisted, same id in / same id out. The one-shot `freshPaneSessionOnNextSpawn` setting feeds the existing `remintPaneSessionId` path on the next create/spawn for panes 1-3, then clears itself so James can choose fresh versus resume without hand-editing the store.
...
- ui/modules/telegram-poller.js: Exports start, stop, isRunning, _internals, .... Telegram `getUpdates` and media requests have bounded deadlines so a hung HTTPS request cannot park `pollInFlight` forever; heartbeat state records `poller.lastPollAt`/status and watchdog liveness keys off that poll freshness rather than process aliveness.
