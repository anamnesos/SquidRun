...
- mira/runtime/src/mission-control-route-preview.ts: State-root writer/list reader for Mission Control internal-route previews saved as `pending_internal_review` history under `MIRA_STATE_ROOT/mission-control/route-previews`, explicit saved-preview-token promotion into reviewable internal-route requests under `MIRA_STATE_ROOT/mission-control/internal-route-requests`, approve/reject/edit owned-work continuation metadata under `MIRA_STATE_ROOT/mission-control/owned-work-continuations`, and read-only follow-through recommendations derived from continuation metadata, preserving manual-execution/no-send/no-route-flip/no-provider audit fields and storing no executable command.
