...
- ui/modules/main/system-protected-evals.js: Pure static system-eval registry/runner for protected zero-fail SquidRun behaviors. Phase 4A binds the `accepted.unverified` visible-delivery invariant to concrete `hm-send.js` route-proof refs; Phase 4B binds materialized full-message read requirements to daemon full-agent-message refs and preview-only decision fixtures; Phase 4C binds route metadata authority to `squidrun-app.js` routeInjectMessage guard refs and tempting-body mismatch fixtures. Cases are mutation-failable and have no runtime, network, write, send, restart, or dispatcher side effects.
