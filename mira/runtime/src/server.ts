...
export async function route(request: IncomingMessage, response: ServerResponse): Promise<void> {
  ...
  if (request.method === "POST" && requestUrl.pathname === "/mission-control/internal-delivery-previews") {
    try {
      const body = await readJsonBody(request);
      const preview = createMissionControlInternalDeliveryPreview(body);
  ...
  if (request.method === "POST" && requestUrl.pathname === "/mission-control/dispatch-readiness") {
    try {
      const body = await readJsonBody(request);
      const readiness = createMissionControlDispatchReadiness(body);
  ...
  if (request.method === "POST" && requestUrl.pathname === "/mission-control/internal-send-activation-designs") {
    try {
      const body = await readJsonBody(request);
      const design = createMissionControlInternalSendActivationDesign(body);
  ...
  if (request.method === "POST" && requestUrl.pathname === "/mission-control/internal-send-activation-requests") {
    try {
      const body = await readJsonBody(request);
      const requestPreview = createMissionControlInternalSendActivationRequest(body);
      sendJson(response, 200, requestPreview);
    } catch (error) {
      sendJson(response, 400, errorPayload(error));
    }
    return;
  ...
  if (request.method === "POST" && requestUrl.pathname === "/mission-control/internal-send-activation-implementation-readiness") {
    try {
      const body = await readJsonBody(request);
      const readiness = createMissionControlInternalSendActivationImplementationReadiness(body);
      sendJson(response, 200, readiness);
    } catch (error) {
      sendJson(response, 400, errorPayload(error));
    }
  ...
  if (requestUrl.pathname === "/mission-control/internal-delivery-previews") {
    sendJson(response, 200, listMissionControlInternalDeliveryPreviews(process.env, { includeInternal: includeInternalFields(requestUrl) }));
    return;
  }
  ...
  if (requestUrl.pathname === "/mission-control/internal-send-activation-designs") {
    sendJson(response, 200, listMissionControlInternalSendActivationDesigns(process.env, { includeInternal: includeInternalFields(requestUrl) }));
    return;
  }
