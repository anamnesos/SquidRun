...
async function checkAndDownload() {
  ...
  if (d.ok === false) { console.log('API error'); return; }
  console.log('Updates:', d.result.length);
  for (const u of d.result) {
    const m = u.message;
    if (m === undefined || String(m.chat.id) !== '8754356993') continue;
    ...
    if (hasPhoto) {
      ...
      if (fileData.ok) {
        ...
        const outPath = 'D:/projects/Korean Fraud/dalser-police-text-'+u.update_id+'.jpg';
