...
if [ "$SOURCE" != "compact" ] && command -v node >/dev/null 2>&1; then
  STARTUP_HEALTH=$(
    node "$PROJECT_DIR/ui/scripts/hm-health-snapshot.js" "$PROJECT_DIR" "--profile=$PROFILE" --markdown 2>> "$AUDIT_DIR/startup-health-errors.log"
  )
fi
...
let body = raw.trim();
const notes = [];
// Mirror ui/modules/startup-ai-briefing.js readStartupBriefingForInjection:
// a skipped/failed generation must OMIT the old briefing prose entirely so
// months-old blockers can't masquerade as "age: 0 minutes" live context.
if (status?.skipped === true) {
  notes.push(`AI briefing skipped: ${status.skipReason || 'provider_disabled'} (intentional, non-blocking); old ai-briefing.md omitted so historical prose cannot create live blockers.`);
  body = '';
} else if (status?.ok === false) {
  notes.push(`UNTRUSTED AI BRIEFING: latest generation failed (${status.error || status.reason || 'unknown_error'}); old ai-briefing.md omitted so historical prose cannot create live blockers.`);
  body = '';
...
if [ "$PROFILE" = "eunbyeol" ]; then
  ...
  COMBINED_CONTEXT="$PREFIX\n\nSYSTEM HEALTH (auto-fixed where safe):\n$STARTUP_HEALTH\n\nMANDATORY READS BEFORE DOING ANYTHING:\n1. $LATEST_HANDOFF_PATH - Current session handoff with open items, live positions, and James's feedback\n2. workspace/knowledge/trading-operations.md - Hyperliquid=REAL money, Alpaca=FAKE. Verify read-only/manual-only state first.\n3. workspace/knowledge/case-operations.md - 은별 pending items and routing rules.\n\n$ADDITIONAL_HANDOFF_CONTEXT\n\nROUTING: 은별=send-long-telegram.js 8754356993. James=hm-send.js telegram (ENGLISH ONLY). Use Builder/Oracle not subagents.\n\nTRADING: Hyperliquid is real money; Alpaca is paper. Read workspace/knowledge/trading-operations.md before trading-related work. Verify current read-only live state first; if unavailable or manual-only, report that state. Do not seek, place, close, or modify trades from startup context alone.\n\n$AI_BRIEFING\n\n$AGENCY_LAYER"
fi
