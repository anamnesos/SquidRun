...
TRADING (REAL MONEY BOUNDARY):
- Hyperliquid is REAL money. Alpaca is paper.
- MUST read workspace/knowledge/trading-operations.md before any trading-related reply.
- Verify a current read-only live state first. If only public-core/manual-only state is available, report that live trade state is unverified or manual-only.
- Do not seek, place, close, or modify trades from compaction context alone.
