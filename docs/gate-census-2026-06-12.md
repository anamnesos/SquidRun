...
**G5b. Kill-scope escape (census reopened, Architect #36)**: the same executor run's kill phase almost certainly took down Eunbyeol's standalone install at 16:02Z — her exit stamps sit exactly inside our execution window (attribution being confirmed from executor logs; her side contained with zero loss, relaunch in motion). One live-fire run exposed BOTH executor defects: wrong launch target (G5) and too-wide kill scope (G5b). **Verdict: FIX (Builder) — kill scope restricted to main-install-path-only.** Process fix shipped with this exhibit: the pre-restart gate in ROLES.md (live + workspace template) now requires an explicit "side-profile/standalone installs unaffected" line item in the restart-risk review AND in post-restart verification — my S442 GO review checked our own panes and squid-room and had no line for her install; that omission is part of this exhibit.
