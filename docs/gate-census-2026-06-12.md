...
4. Fourth member, `flushPendingPaneDeliveries` requeue (:8511) — items left the queue only on `verified === true`; accepted-unverified requeued with attemptCount+1 (max 5), up to 4 duplicate re-deliveries to a busy pane. Found by the S443 sweep; **FIXED 556a8ff8** (terminal-accepted statuses are delivered/queue-clearing, not replay fuel).
5. Fifth member, scoped Telegram fallback chain (:14068 → handleScopedTelegramInboundFallback :10901) — **CONFIRMED REAL from source** (`forwardScopedTelegramInboundToProfileWindow` has no messageId/dedup; only writes trigger text), so a terminal-accepted delivery fell through to an unconditional trigger write — one duplicate per busy-pane scoped Telegram inbound (bounded per message, chronic across messages). **FIXED 556a8ff8** (caller treats terminal-accepted as success, skips fallback).
Remaining sweep hits triaged as lawful telemetry (status mapping, no payload retention). 278/278 focused app tests on the member-4/5 fix. **Sequencing ruling (Architect #50)**: the Eunbyeol version cut did NOT hold for members 4/5 — her build already fixed everything that bit her, #4 bounded at 5, hold-creep on a fallback lane is the worse trade; both fixes ride trunk immediately behind the cut. Reopener clause: a confirmed ACTIVE UNBOUNDED double-delivery re-evaluates her build. Post-confirmation read: #5 is real but bounded (≤1 dup per inbound), her cut predates 556a8ff8 — reopener decision is Architect's, evidence above. Shared-index note (Architect #48 ruling after the second same-hour race): a claim covers ALL of one agent's lanes, one committer at a time; a third cross-agent race triggers the tooling conversation.
...
## G6. Eunbyeol-stall investigation spillover (Architect #38, session 443)

**G6a. Hot-watcher chronic cost — NEEDS-CHECK (next pass).** The stall refutation evidence showed watcher-worker burning ~2,984 CPU-seconds at 859MB on OUR OWN healthy dev main window. It's a thin chokidar stat-poller (reads no contents), but the steady-state cost is real and system-wide. Cross-window corroboration (eunbyeol Builder's rider, 2026-06-12): EVERY instance on this machine runs exactly one watcher-worker at ~60% of a core / ~880MB working set, healthy instances included — chronic cost confirmed independent of any defect. Next-pass question, same shape as every census verdict: which consumer justifies the polling cadence and scope? If no decision reads what the watcher's churn produces, that's a diary at CPU prices.

**G6b. Withdrawn root cause — method exhibit, no fix needed.** The Eunbyeol-side Architect CONFIRMED a root cause, then withdrew it pre-deadline when their Builder's source trace refuted it — `daemon.log` was in the watcher's ignore regex all along. Same lesson as the E2 correction (G1), different window: **confirmation pressure does not survive a source read.** The census method rule generalizes — before confirming any root cause, read the source that would refute it. Withdrawal-on-evidence is the system working, and it goes in the record as a positive exhibit.
...
## G7. Main-process logger blackout — **FIX (swallow keeps never-throw; silence loses its license)**

From the cross-window wedge investigation (spec relayed via Architect, hm-1781285144344): both wedged Eunbyeol boots produced ZERO main-process log lines while short-lived CLI children wrote to the SAME app.log fine — so when init() threw (or the buffered writer failed), the error went into a logger designed to eat it. Source read (`ui/modules/logger.js`): `ensureLogDir` swallows mkdir failure with "If file logging fails, keep console logging working"; the buffered writer's `markBroken` silently disables failed streams. **Verdict: the never-throw half of swallow-by-design keeps its justification — a logger must not take the app down. The SILENT half has none.** A logger that can die without leaving a mark anywhere is a diary that eats its own pages, and main.js's `init().catch(log.error)` routes the most important error of a boot into exactly that hole. FIX: on first swallowed dir/write failure, emit one synchronous stderr line + drop a `logger-blackout` marker (sync append, bypassing the buffered writer) so a blackout is detectable from outside. Builder's boot-sequence.jsonl breadcrumbs (in flight, from the spec) cover the init path specifically; this verdict covers the logger as a class. Same family as G2/G6c: an instrument whose failure mode is indistinguishable from "nothing happening" fails the falsifiability bar. **EXECUTED dd52561f** (same boot-observability pass as the breadcrumbs): first swallowed logger dir/write/stream failure emits sync stderr + appends `.squidrun/runtime/logger-blackout.jsonl` via appendFileSync; 304/304 focused tests. Restart docs updated in step: packaged-install boot verification now reads boot-sequence.jsonl, and a present logger-blackout.jsonl means fall back to breadcrumbs/process evidence instead of app.log recency.

## G8. CLOSING EXHIBIT — the data-root saga (S443, boot 4 PASS 18:26Z)

One mechanical defect — **intra-process resolver divergence on packaged installs**: within a single process, the settings read path and the runtime write path bound to DIFFERENT data roots — masqueraded, over six hours and three boots, as five separate defects: a boot hang, a Telegram 409 war (her split-brain poller fighting ours for the same bot token), session-counter corruption (her boots incrementing OUR counter), a daemon.pid clobber (hers overwrote ours; restored live by hand), and pane over-spawn. Three theories were killed by source reads and live interrogation before the real one stood.

**The instrument lesson, sharpest of the day**: the G7 boot breadcrumbs worked PERFECTLY — into the wrong file. Boot 3's complete trail landed in OUR root while everyone watched HER root see nothing and called it a hang. Her boots were six seconds all day; six hours went to diagnosing the observability, not the boot. A perfect instrument bound to the wrong reality is worse than no instrument: it manufactures confident evidence of a false world. This extends the census falsifiability bar — a diagnostic artifact must carry proof of WHICH world it was written from.

**The cure**: b93f1913 (manifest-first root binding for packaged installs) + a2b95b60 (root-coherence assert at boot, installed deployments only, failing on all four loud channels at once: breadcrumb, sync stderr, marker file, degraded status — now a hard gate for packaged cuts) + e2f6acd7 (launch-windows 20s race + substep breadcrumbs, lawful regardless). The acceptance check was authored by the eunbyeol side — the side of the wall that felt the symptoms. Boot 4 from 0.1.34-a2b95b60: init:complete in ten seconds, session 22 on HER counter, root-coherence green, poller takeover clean, panes exactly 3.

Closing line, the eunbyeol Architect's words, earned: **"the manifest-first binding did in ten seconds what three boots could not do in six hours."**
