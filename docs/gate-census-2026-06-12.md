...
**G6a. Hot-watcher chronic cost — NEEDS-CHECK (next pass).** The stall refutation evidence showed watcher-worker burning ~2,984 CPU-seconds at 859MB on OUR OWN healthy dev main window. It's a thin chokidar stat-poller (reads no contents), but the steady-state cost is real and system-wide. Next-pass question, same shape as every census verdict: which consumer justifies the polling cadence and scope? If no decision reads what the watcher's churn produces, that's a diary at CPU prices.
