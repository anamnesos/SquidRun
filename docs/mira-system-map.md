...
| Telegram / channel access | Current SquidRun Mira product surface. | SquidRun routes main text-only Telegram inbound to Mira Live only when the env flag allows it; commands, ops, media, failures, duplicate replies, and cross-profile cases are guarded. | New Mira does not own Telegram route control or any clean direct channel today. | `ui/modules/main/squidrun-app.js`, `ui/scripts/hm-telegram*.js`, `ui/modules/telegram-poller.js`, `ui/__tests__/hm-telegram*.test.js`. | Decide and prove Mira's first direct channel: Telegram-only first, or Telegram plus local UI. |
...
| New Mira runtime service | `mira/runtime/src/*.ts`, `mira/runtime/dist/*.js`, `mira/runtime/package.json`, `mira/tools/start-local-runtime.js` | Local Node/TypeScript runtime with health, status, turn, model, work, autonomy, voice correction, and UI endpoints. Built to prototype Mira outside Electron. | PROTOTYPE / KEEP | Can start a loopback local workbench on `127.0.0.1:47373` with dev state, read-only proof endpoints, and read-only browser boot. | Not current Telegram, off-PC, action, or restart-continuity owner. Runtime start/workbench/browser-boot proof did not run a model turn or external action. | Node/TypeScript, OpenAI/Ollama env, port 47373; `mira-start-local-runtime`, runtime bridge/state/status tests; 2026-05-20 loopback start, GET-only workbench, and browser boot proof. | Removing loses extraction path and local prototype. | Prove the first explicit user-action path separately: turn, draft, or direct channel, with no accidental boot side effect. |
| New Mira local UI | `mira/ui/index.html`, `mira/ui/app.js`, `mira/ui/styles.css` | Browser UI for runtime turns, model status, work queues, autonomy, and local context. Built to inspect New Mira without embedding it back into SquidRun. | PROTOTYPE / KEEP | Serves from the local runtime when it is running; default browser boot hydrates session, capability, model, work, and autonomy state with GET-only calls. | Not the current user-facing Mira surface in SquidRun or Telegram. Default boot is not a model turn; explicit turns still require a user action. | `mira/runtime/src/server.ts`, runtime endpoints; `ui/__tests__/mira-runtime-ui-read-only-boot.test.js`, runtime/start-local tests, and 2026-05-20 browser boot proof. | Removing makes New Mira harder to inspect. | Decide/prove whether the first product UI is local web, Telegram, or both, after explicit action paths are covered. |
...
| Telegram route and external text channel | `ui/modules/main/squidrun-app.js`, `ui/modules/telegram-poller.js`, `ui/modules/main/telegram-poller-worker.js`, `ui/scripts/hm-telegram*.js`, root `telegram-poller.js` | Receives Telegram, separates commands/ops/media, optionally routes main text to Mira Live, sends/dedupes/chunks replies. | LIVE / KEEP | Current external text path when configured. | Not owned by New Mira and not a general external-send permission. | Telegram bot env, poller state, SquidRun app; `hm-telegram*`, `telegram-poller`, routing tests. | Removing breaks external chat, fallback safety, and route protections. | Decide/prove Mira's clean direct channel before moving ownership. |
...
James does need one direct product decision at a time when the map exposes a real fork, for example: "Should Mira's first direct channel be Telegram-only first, or Telegram plus local UI?"
