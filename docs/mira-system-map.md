...
- New Mira runtime turns now pass a first visible-reply parity gate for current Mira-style preamble, assistant-shape, and backstage/proof-label blocking; bad local turn text is held instead of shown.
...
| Visible Mira reply | Current SquidRun Mira product surface; New Mira runtime prototype for local turns. | SquidRun can produce visible Mira replies through Mira Live -> Mira Lab prompt reply -> text attachment/language gates. New Mira runtime turns now have a first parity visible-reply gate for preamble, assistant-shape, and backstage/proof-label blocking, and held text does not leak diagnostics. | New Mira runtime turns are prototype; they are not the current live Telegram reply path and do not yet prove full Mira Live audit/transcript/held-reply parity. | `ui/modules/mira-live-entrypoint.js`, `ui/modules/mira-lab-surface.js`, `ui/modules/ipc/mira-lab-handlers.js`, `ui/modules/mira-core/text-model-attachment-v1.js`, `mira/runtime/src/turn.ts`, `ui/__tests__/mira-runtime-turn-visible-reply-parity.test.js`. | Extend parity to full Mira Live held-reply audit/transcript behavior before replacing current visible reply paths. |
...
- Extend New Mira visible-reply parity from first language/backstage gate into full Mira Live held-reply audit/transcript behavior.
