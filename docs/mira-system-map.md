...
- Default New Mira runtime-turn JSON export/replay through `read-runtime-turns.js --json` now uses the same clean held-reply projection; explicit `--include-internal` is required for safe gate/status diagnostics and still strips rejected generated text fields.
...
This extends the same ephemeral harness through `mira/tools/read-runtime-turns.js --json`. Default JSON export/replay uses public held-reply projection; explicit `--include-internal` exposes safe gate/status metadata only, and both modes exclude rejected generated text.
...
- Define the diagnostic-review policy for whether raw rejected generated text can ever be viewed or exported, and keep default user-facing surfaces clean meanwhile.
