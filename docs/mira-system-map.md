...
| Tests, fixtures, wrappers, and map guard | `ui/__tests__/mira-*.test.js`, `ui/__tests__/hm-mira-*.test.js`, `ui/__tests__/helpers/mira-reviewed-import-fixture.js`, `ui/__tests__/fixtures/mira-*-contract.json`, `ui/__tests__/fixtures/mira-progress-contract-v0.json`, `ui/scripts/hm-mira-core-*.js`, `ui/scripts/hm-mira-progress.js`, `ui/scripts/mira-system-map-guard.js`, `scripts/pre-commit.sh` | Regression coverage, CLI/status seams, fixture contracts, self-contained reviewed-import source fixtures, and source-of-truth enforcement for current and historical Mira modules. | TRANSITION / KEEP-PARK SPLIT | High value for live speech, Telegram, restart, voice, deterministic progress accounting, and CI-portable reviewed-import validation. Reviewed-import tests seed real temp source files from the tracked queue/contract so `source_exists` and `ok:true` remain real without depending on private local workspace files. The map guard blocks staged Mira-owned changes unless this map is staged too. | Not product behavior by itself, not proof that old scaffolding is live, not a live-data import, and not a promotion of New Mira import authority. | Jest, fixtures, module builders, wrappers, `ui/__tests__/mira-progress-v0.test.js`, `mira-reviewed-import-plan.test.js`, `mira-state-import-tooling.test.js`, `mira-system-map-guard.test.js`. | Removing blindly hides regressions, breaks proof chains, reintroduces CI/private-file drift, or lets new Mira surfaces drift away from this map. | Keep live tests; keep the guard path list aligned with this map; keep reviewed-import tests self-contained with real seeded source fixtures; consolidate duplicate phase fixtures only after replacement coverage. |
