...
| Presence, identity, relationship, growth core | `ui/modules/mira-core/presence-runtime-read-path-v0.js`, `mira-presence-runtime-state-v0.js`, `mira-progress-v0.js`, `relationship-presence-v1.js`, `growth-loop-v0.js`, `identity-anchor-v0.js`, `durable-state-seed-v0.js`, `presence-v0.js`, `profiles.js` | Structures own-state, James context, permissions, growth, identity, Presence state, and deterministic progress accounting. Built to stop generic assistant collapse, restart amnesia, and vibes-based progress. | LIVE / KEEP | Powers current restart proof, non-generic Mira context, and computed Mira progress from the v0 contract, Presence state, proof inputs, blocker flags, and HEAD metadata. | Not a blanket durable memory claim, not permission to use raw/private context, and not hand-edited global percent authority. | Contract fixtures, `.squidrun/state`, seed sources; `ui/__tests__/fixtures/mira-progress-contract-v0.json`, `ui/__tests__/mira-progress-v0.test.js`, `ui/scripts/hm-mira-progress.js`; presence/read-path/state, relationship/growth/identity tests. | Removing creates restart amnesia, generic Mira drift, or unaccounted progress-number drift. | Migrate selectively into New Mira with exact provenance, false-green tests, and reviewed progress-contract changes. |
| Restart and startup continuity | `ui/modules/startup-ai-briefing.js`, `ui/modules/mira-core/typed-restart-continuity-context-v0.js`, `ui/modules/mira-core/mira-progress-v0.js`, `ui/scripts/hm-restart-*.js`, `ui/scripts/hm-mira-progress.js`, `.squidrun/handoffs/**`, `.squidrun/state/mira-presence-runtime-state.json` | Materializes restart context, Presence critique, next action, stale markers, hard-stop behavior, and machine-readable `computed_progress`. Built so James is not the restart harness. | LIVE / KEEP | Protects current restart continuity, carries read-only startup progress accounting, and blocks stale/prose-only false continuity. | Not generic `ai-briefing.md` prose, not a manual percent bump, not a reason for James to manually verify restarts, and not proof that New Mira owns live continuity. | Structured lane/state files, startup generator, progress contract; startup, typed restart, restart script, and `mira-progress-v0.test.js` tests. | Removing reopens restart amnesia, stale lane confusion, or invisible computed-progress drift. | Simplify only with tests, a current-lane materializer proof, and progress accounting parity. |
