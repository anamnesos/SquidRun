...
| Restart and startup continuity | `ui/modules/startup-ai-briefing.js`, `ui/modules/mira-core/typed-restart-continuity-context-v0.js`, `ui/modules/mira-core/mira-progress-v0.js`, `ui/modules/mira-core/mira-progress-proof-inputs-v0.js`, `ui/scripts/hm-restart-*.js`, `ui/scripts/hm-mira-progress.js`, `ui/scripts/hm-mira-visible-presence-proof.js`, `.squidrun/handoffs/**`, `.squidrun/runtime/mira-progress-proof-inputs-v0.json`, `.squidrun/state/mira-presence-runtime-state.json` | Materializes restart context, Presence critique, next action, stale markers, hard-stop behavior, machine-readable `computed_progress`, and fresh visible Presence proof intake. Built so James is not the restart harness. | LIVE / KEEP | Protects current restart continuity, carries read-only startup progress accounting, and blocks stale/prose-only false continuity; startup consumes the same HEAD/worktree-bound progress proof path as `hm-mira-progress`. | Not generic `ai-briefing.md` prose, not a manual percent bump, not a reason for James to manually verify restarts, not proof that New Mira owns live continuity, and not a freshness bypass when the proof artifact HEAD/worktree no longer matches the current clean repository state. | Structured lane/state files, startup generator, progress contract, HEAD/worktree-bound proof artifact; startup, typed restart, restart script, `mira-progress-v0.test.js`, `mira-progress-proof-inputs-v0.test.js`, and `startup-ai-briefing.test.js` tests. | Removing reopens restart amnesia, stale lane confusion, invisible computed-progress drift, or stale proof carryover. | Simplify only with tests, a current-lane materializer proof, and progress accounting parity. |
