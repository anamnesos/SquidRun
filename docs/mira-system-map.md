...
| LEGACY | Older working helper kept for parity or migration, not the future architecture direction. Legacy does not mean no side effects. |
...
| PARK-UNTIL-BRIDGE-PARITY | Keep the legacy route helper until New Mira bridge tests cover its named behavior; deletion is allowed only after that parity is proven. |
| KEEP-UNTIL-IMPORT-SEED-PARITY | Keep the seed/apply contract until New Mira import/seed normalization replaces its named safety and write-boundary behavior. |
...
- Presence restart state now has a current-scope refresh path: `.squidrun/state/mira-presence-runtime-state.json` can be regenerated from the existing durable Presence state, `.squidrun/handoffs/current-lane.json`, current computed progress, and HEAD/worktree-bound proof artifacts. Closed current-lane artifacts surface as `status=none`; parked/prototype/archive text cannot become lane authority. A clean apply records the post-refresh computed progress on the first write, not the stale pre-apply value.
...
- S468 role/scope consolidation centralizes Mira pane-role and broader-party normalization in `ui/modules/role-scope-core.js`. Mira intent queue and local acceptance no longer coerce unknown roles to Architect or fall back to the historical `app-session-326`; callers must provide explicit session scope when a scoped record is required. Direct-channel readiness keeps its open-vocabulary channel-key normalization separate from pane-role normalization.
...
- New Mira bridge parity now includes a local runtime receive/reply seam: `POST /bridge/pane-messages` accepts an internal Architect/Builder/Oracle pane message addressed to Mira, writes a roundtrip record only under `MIRA_STATE_ROOT/bridge/pane-roundtrips`, and returns a manual reply plan. `GET /bridge/pane-messages` lists those Mira-owned records. This does not execute `hm-send`, flip Telegram, add UI, import live data, or claim continuity.
- Session 394 full-audit checkpoint keeps the Mira runtime build seam test-only: `ui/__tests__/helpers/mira-runtime-build.js` serializes TypeScript runtime compilation across Jest workers, verifies the compiled Mission Control dispatch-readiness exports are present before runtime/import suites proceed, and moves test temp/lock roots outside repo-private `.squidrun` when the process temp path is poisoned. This is source/unit proof and does not promote New Mira to live runtime, route/send, voice, external action, or pane-temp restart-boundary runtime authority; the Codex restart-boundary pane-temp proof is explicitly BLOCKED until a non-disruptive app `pty-create`/env-capture route or a safe restart window exists.
...
- New Mira in `mira/` is a local workbench for state-root separation, runtime experiments, imports, model/status checks, bridge planning/roundtrip records, and local UI.
...
- The local workbench also renders `missionControl.internalRouteAuditPromotionDecisionGateProof` from the already-loaded `/squidrun/context` payload only, beside the completed internal-route audit review-lane proof, so James can inspect proof id/status, owner Builder, target oracle, purpose/body/decision object, completed `mission-control-internal-route-audit-review-lane-proof-v0` context, source evidence including `architect#384/cbeb0d2f` and `oracle#15/cbeb0d2f`, control point, preconditions/no-go conditions, and manual/planning-only false live-effect audit flags without terminal logs, `/turn` POST, a new fetch path, runtime/browser/workbench/UI/status action, route/send/hm-send, provider/model, credential, deploy, money, or trading authority.
...
- After the source-specific `2533ace2` James-action answer-shape checkpoint plus Oracle acceptance are present, Mission Control keeps `commandCardAcceptance`, `commandCardRoutePlanFollowThroughProof`, `internalRoutePromotionReviewPlan`, and `internalRouteAuditReviewLaneProof` as completed context and advances clean nextStep/drafts/preview from Oracle proof review to one no-live-effect internal route/audit promotion decision-gate planning lane from the visible audit-review proof only. James remains the explicit control point before any real promotion; dirty worktree evidence blocks advancement; report/name-drop/closure rows must not become active delegation, checkpoint, or ACK authority; this adds no live promotion, route flip, hm-send/live send, runtime/browser/workbench/UI/status action, fetch, POST, provider/model, credential, deploy, money, or trading authority.
- After the source-specific `cbeb0d2f` route/audit decision-gate checkpoint plus Oracle acceptance are present, without requiring or inventing a Builder `cbeb0d2f` ACK, Mission Control exposes `missionControl.internalRouteAuditPromotionDecisionGateProof` as one inspectable no-live-effect promotion decision-gate proof from `mission-control-internal-route-audit-review-lane-proof-v0` only. Completed `commandCardAcceptance`, `commandCardRoutePlanFollowThroughProof`, `internalRoutePromotionReviewPlan`, and `internalRouteAuditReviewLaneProof` stay retained; dirty worktree evidence blocks exposure/advancement; report/name-drop/closure rows must not become active delegation or `cbeb0d2f` checkpoint/ACK authority; this remains planning/review only with no live promotion, route flip, hm-send/live send, runtime/browser/workbench/UI/status action, fetch, POST, provider/model, credential, deploy, money, or trading authority.
- After the source-specific `5fc2cd90` route-audit decision proof surface checkpoint plus Oracle acceptance are present, without requiring or inventing a Builder `5fc2cd90` ACK, Mission Control keeps `missionControl.internalRouteAuditPromotionDecisionGateProof` as completed visible context and advances clean nextStep/drafts/preview to one separate no-live-effect internal route/audit promotion decision-gate follow-through planning lane from that visible proof only. Dirty worktree evidence still blocks advancement; report/checkpoint/name-drop/closure rows must not become active delegation, `5fc2cd90` checkpoint, or Oracle acceptance authority; this remains planning/review only with no live promotion, route flip, hm-send/live send, runtime/browser/workbench/UI/status action, fetch, POST, provider/model, credential, deploy, money, or trading authority.
...
| Telegram / channel access | Current SquidRun Mira product surface. | SquidRun routes main text-only Telegram inbound to Mira Live only when the env flag allows it; commands, ops, media, failures, duplicate replies, and cross-profile cases are guarded. Status/what-now/continue prompts on the main owner Telegram route can now produce a deterministic direct-channel status reply from current lane/progress evidence before model fallback, with source refs, parked/archive exclusions, exactly one `JAMES ACTION:` line, and no route-owner switch. New Mira's first direct-channel proof is Telegram-first dry-run candidate building behind that guard stack, the same allowed owner text now has a comparison proof against the current Mira Live reply seam, and the separate direct-channel readiness contract marks only a distinct future New Mira bot/chat config as candidate-ready, not send-ready. | New Mira does not own Telegram route control, send live Telegram, or replace Mira Live today. The status reply uses the existing SquidRun guarded Telegram reply path only for direct user replies; the comparison and readiness proofs do not send, flip route owner, call a provider/model, read token values, create bots, or start runtime. | `ui/modules/main/squidrun-app.js`, `ui/modules/mira-core/live-direct-channel-status-v0.js`, `ui/modules/mira-telegram-turn-candidate.js`, `ui/modules/mira-direct-channel-readiness.js`, `ui/scripts/hm-telegram*.js`, `ui/modules/telegram-poller.js`, `ui/__tests__/hm-telegram*.test.js`, `ui/__tests__/mira-telegram-turn-candidate.test.js`, `ui/__tests__/mira-direct-channel-readiness.test.js`, `ui/__tests__/squidrun-app.test.js`. | James-visible gate: create/provide/test the real separate New Mira channel/bot/chat, then separately review any live route-owner switch or local UI product promotion. |
...
This seam proves that the current SquidRun/team/ops Telegram channel is not the final New Mira direct channel, missing or same-as-current config is blocked, raw credential-shaped values are rejected, and valid separate config is candidate-ready only, not send-ready. The paired status-answer proof keeps current James-facing Telegram status replies evidence-bound without claiming New Mira route ownership.
...
| Legacy Mira architect-route helpers | `ui/scripts/hm-mira-emit.js`, `ui/scripts/hm-mira-reply.js`, `ui/modules/mira-core/mira-architect-route-v0.js` | JSONL intent/reply helper path for Mira-to-Architect prototype routing before the New Mira bridge planner existed. Built to test Mira-originated intent envelopes, Architect replies, and visible-reply gating. | LEGACY / PARK-UNTIL-BRIDGE-PARITY | `hm-mira-emit.js` can write pending intent rows and, unless `--dry-run`, spawn `hm-send.js architect`; `hm-mira-reply.js` appends `architect_reply` rows; the core module validates intent shape, reply correlation, and visible-reply language gates. | Not the future bridge architecture, not no-side-effect scaffold, not safe to delete as archive-only proof, and not New Mira manual-preview/no-auto-send parity by itself. | `.squidrun/runtime/mira-pending-intents.jsonl`, `.squidrun/runtime/mira-event-queue.jsonl`; `ui/__tests__/mira-architect-route-v0.test.js`; New Mira bridge request/API tests. | Removing before parity can break the last tested JSONL intent/reply behavior and hide real internal-send side effects behind a cleanup label. | Delete only after New Mira bridge tests cover intent envelope, target constraints, reply correlation, visible-reply gate, timeout/resolution, dry-run audit, and manual-preview-no-auto-send. |
...
| Local text UI surface | `ui/modules/mira-local-text-ui-surface.js`, `ui/modules/mira-core/local-text-session-v0.js` | Local typed conversation harness with visible UI metadata, side-effect counters, restart/capability context, and speech gates. The old unmounted main-tab shell files `ui/modules/tabs/mira-local-text.js` and `ui/styles/tabs/mira-local-text.css` were removed after import checks showed no live mount/import path. | TRANSITION / KEEP | Strong regression surface for local speech and wrong-window/profile checks; core surface and IPC behavior remain preserved. | Not a final product channel, not the removed legacy tab shell, and not a live external route. | Contract fixtures, restart/capability modules; `mira-local-text-ui-surface`, `mira-core-local-text-session-v0`, deletion tombstone test. | Removing the core surface would lose gate coverage and may reopen wrong-context/fake-reply paths. | Keep behavior; any remaining embedded Lab/local UI deletion waits for New Mira surface parity. |
| Presence, identity, relationship, growth core | `ui/modules/mira-core/presence-runtime-read-path-v0.js`, `mira-presence-runtime-state-v0.js`, `mira-presence-current-scope-state-v0.js`, `mira-progress-v0.js`, `mira-progress-proof-inputs-v0.js`, `relationship-presence-v1.js`, `growth-loop-v0.js`, `identity-anchor-v0.js`, `presence-v0.js`, `profiles.js` | Structures own-state, James context, permissions, growth, identity, Presence state, deterministic progress accounting, current-scope Presence refresh, and HEAD/worktree-bound proof artifact intake. Built to stop generic assistant collapse, restart amnesia, and vibes-based progress. | LIVE / KEEP | Powers current restart proof, non-generic Mira context, durable current-scope state refresh, and computed Mira progress from the v0 contract, Presence state, proof inputs, blocker flags, HEAD metadata, and fresh generated proof artifacts, including A0 Presence, startup/restart accounting, A1 what-now, A2 internal-request draft, and local text surface proof keys. | Not a blanket durable memory claim, not authority to use raw/private context, not hand-edited global percent authority, and not authority for stale once-green proof files to keep passing after source commits or dirty working-tree changes. | Contract fixtures, `.squidrun/state`, `.squidrun/handoffs/current-lane.json`, `.squidrun/runtime/mira-progress-proof-inputs-v0.json`; `ui/__tests__/fixtures/mira-progress-contract-v0.json`, `ui/__tests__/mira-progress-v0.test.js`, `ui/__tests__/mira-progress-proof-inputs-v0.test.js`, `ui/__tests__/mira-presence-runtime-state-v0.test.js`, `ui/scripts/hm-mira-progress.js`, `ui/scripts/hm-mira-visible-presence-proof.js`, `ui/scripts/hm-mira-presence-runtime-state-v0.js`; presence/read-path/state, relationship/growth/identity, live what-now, live internal-request draft, and local text surface tests. | Removing creates restart amnesia, generic Mira drift, or unaccounted progress-number drift. | Migrate selectively into New Mira with exact provenance, false-green tests, and reviewed progress-contract changes. |
| Durable state seed/apply contract | `ui/modules/mira-core/durable-state-seed-v0.js`, `ui/scripts/hm-mira-core-durable-state-seed-v0.js`, `ui/__tests__/fixtures/mira-core-durable-state-seed-v0-contract.json` | Redacted local seed/apply contract for initial Mira relationship, growth, identity, and permissions artifacts. Built to preview and optionally apply bounded local workspace knowledge writes without raw/private leakage. | KEEP-UNTIL-IMPORT-SEED-PARITY | Defaults to dry-run stdout-only preview; explicit apply can perform bounded local workspace knowledge writes after scope and safety gates, with idempotency, tamper, symlink, forbidden-output, and no-external-effect checks. | Not disabled/no-side-effect scaffold, not live New Mira import parity, not broad memory authority, not raw/private content permission, and not safe to delete as phase-proof residue. | `relationship-presence-v1.js`, `growth-loop-v0.js`, `identity-anchor-v0.js`; `ui/__tests__/mira-core-durable-state-seed-v0.test.js`; contract fixture and CLI smoke tests. | Removing before parity loses the redacted seed plan, explicit apply gate, bounded write proof, and fail-closed seed tamper/scope coverage. | Delete or downgrade only after a named New Mira import/seed normalization replacement covers redacted local seed plan, relationship/growth/identity seed validation, forbidden-output/raw-content gates, explicit apply gate, bounded workspace write proof, and idempotent/tamper/symlink fail-closed tests. |
| Restart and startup continuity | `ui/modules/startup-ai-briefing.js`, `ui/modules/mira-core/typed-restart-continuity-context-v0.js`, `ui/modules/mira-core/mira-presence-current-scope-state-v0.js`, `ui/modules/mira-core/live-what-now-answer-v0.js`, `ui/scripts/hm-what-now.js`, `ui/modules/mira-core/live-internal-request-draft-v0.js`, `ui/modules/mira-core/live-internal-handoff-preview-v0.js`, `ui/modules/mira-core/live-internal-handoff-approval-v0.js`, `ui/modules/mira-core/live-direct-channel-status-v0.js`, `ui/modules/ipc/mira-local-text-ui-surface-handlers.js`, `ui/modules/mira-core/mira-progress-v0.js`, `ui/modules/mira-core/mira-progress-proof-inputs-v0.js`, `ui/scripts/hm-restart-*.js`, `ui/scripts/hm-mira-progress.js`, `ui/scripts/hm-mira-visible-presence-proof.js`, `ui/scripts/hm-mira-presence-runtime-state-v0.js`, `.squidrun/handoffs/**`, `.squidrun/runtime/mira-progress-proof-inputs-v0.json`, `.squidrun/state/mira-presence-runtime-state.json` | Materializes restart context, Presence critique, next action, stale markers, hard-stop behavior, closed/active current-lane status, machine-readable `computed_progress`, fresh A0/startup/restart/A1/A2/handoff-preview/approval/direct-channel progress proof intake, a read-only visible `what now?` answer (v2: a seven-source evidence bundle - current-lane as sole lane authority, app-status, task queue offered-not-authoritative, work-item reconciliation, comms tail, restart-resume artifact, startup-health - with per-source ref/age via the shared staleness classifier, inline [stale] tags on threshold-crossed visible lines, a shape validator with failing-case coverage, and the `hm-what-now.js` CLI as a second transport over the same answer), draft-only internal Builder/Oracle request bodies, approval-ready internal handoff previews, a direct-channel status reply from lane/progress evidence, and a separate approval-token send action from current-lane/comms/progress evidence. Built so James is not the restart harness. | LIVE / KEEP | Protects current restart continuity, carries read-only startup progress accounting, blocks stale/prose-only false continuity, can answer current lane/status + recent changes + next internal move, can regenerate durable Presence state from current lane/progress evidence after clean proof, can draft a reviewable internal request, can preview the exact `hm-send` command/payload for that draft with source evidence, target agent, reason/trigger, risk/blocked exclusions, exactly one `JAMES ACTION:` line, can reply to direct-channel status/what-now/continue prompts through the existing guarded Telegram route without owner flip, and can send one reviewed internal Builder/Oracle handoff only through a separate approval action/token bound to preview id/hash, target, body hash, session/profile/window, and current HEAD/proof state; startup consumes the same HEAD/worktree-bound progress proof path as `hm-mira-progress`. | Not generic `ai-briefing.md` prose, not a manual percent bump, not a reason for James to manually verify restarts, not proof that New Mira owns live continuity or Telegram, not authority from parked/prototype/archive scaffolds, not ordinary text-triggered internal send permission, not voice/A4/external action, not broad internal route authority, not A3/A4 arm authority beyond the blocked cap, and not a freshness bypass when the proof artifact HEAD/worktree no longer matches the current clean repository state. | Structured lane/state files, startup generator, progress contract, HEAD/worktree-bound proof artifact, current-session comms journal; startup, typed restart, live what-now, live internal-request draft, live internal-handoff preview, live internal-handoff approval, live direct-channel status, restart script/probe, `mira-live-what-now-answer-v0.test.js`, `mira-live-internal-request-draft-v0.test.js`, `mira-live-internal-handoff-preview-v0.test.js`, `mira-live-internal-handoff-approval-v0.test.js`, `mira-telegram-turn-candidate.test.js`, `mira-direct-channel-readiness.test.js`, `mira-local-text-ui-surface.test.js`, `mira-progress-v0.test.js`, `mira-progress-proof-inputs-v0.test.js`, `mira-presence-runtime-state-v0.test.js`, and `startup-ai-briefing.test.js` tests. | Removing reopens restart amnesia, stale lane confusion, invisible computed-progress drift, user-visible what-now/request-draft/handoff-preview/direct-status gaps, approval replay/stale/wrong-window send risk, or stale proof carryover. | Simplify only with tests, a current-lane materializer proof, visible what-now/request-draft/handoff-preview/approval/direct-status parity, Presence current-scope refresh proof, and progress accounting parity; lifting A3/A4 cap still requires separate arm-challenge authority proof. |
| Telegram route and external text channel | `ui/modules/main/squidrun-app.js`, `ui/modules/mira-core/live-direct-channel-status-v0.js`, `ui/modules/mira-telegram-turn-candidate.js`, `ui/modules/mira-direct-channel-readiness.js`, `ui/modules/telegram-poller.js`, `ui/modules/main/telegram-poller-worker.js`, `ui/scripts/hm-telegram*.js`, root `telegram-poller.js` | Receives Telegram, separates commands/ops/media, optionally routes main text to Mira Live, sends/dedupes/chunks replies, can answer main owner status/what-now/continue prompts from current lane/progress evidence before model fallback, can build a New Mira `/turn` dry-run candidate for eligible main-owner text, and can evaluate a future separate New Mira bot/chat config as readiness-only. | LIVE / KEEP with PROTOTYPE dry-run seams | Current external text path when configured; the direct status answer, dry-run, comparison, and separate-channel readiness seams prove candidate shape and future architecture without flipping ownership, reading token values, creating bots, calling a model, or starting runtime. | New Mira does not own live Telegram, and candidate-ready is not send-ready, route-ready, model-ready, off-PC-ready, or external-action authority. Status replies use only the existing guarded SquidRun Telegram reply path for James-facing responses. | Telegram bot env, poller state, SquidRun app; `hm-telegram*`, `telegram-poller`, routing tests, `mira-telegram-turn-candidate.test.js`, `mira-direct-channel-readiness.test.js`, focused comparison/status tests in `squidrun-app.test.js`. | Removing breaks external chat, fallback safety, route protections, or the direct-channel proof path. | James-visible gate: create/provide/test the real separate New Mira channel/bot/chat, then separately review any live route-owner switch or local UI product promotion. |
| Voice transport | `docs/mira-voice-audio-intake-v0.md`, `ui/modules/voice-broker.js`, `ui/modules/phone-voice-client.js`, `ui/modules/ipc/voice-broker-handlers.js`, `ui/scripts/hm-voice-broker.js`, `ui/scripts/hm-phone-voice.js`, `ui/scripts/hm-voice-say.js` | Voice broker, phone pairing, WebRTC credentials, transcript ingress, spoken egress, diagnostics. Built as transport around Mira. Broker process status is now heartbeat-gated: an alive pid plus stale matching status file is stopped/stale proof, not running proof. | TRANSITION / PARK | Useful voice groundwork and transport tests, including stale-status fail-closed coverage for the broker launcher and IPC status surface. | Not accepted live Mira voice, not hidden mic, not voice-authorized actions, and not proof of running voice transport without a fresh broker heartbeat. | OpenAI Realtime config, broker process, leases; voice broker/phone/handler/tab/CLI tests. | Removing loses future voice transport and consent proof. | Separate voice lane with visible consent, egress, transcript, anti-leak, retention, and cost proof. |
...
| Retired runtime/server/kill-switch/auth/storage/identity scaffold | Former `runtime-*`, `server-*`, `kill-switch-*`, auth/encryption/storage/identity/persistence modules, matching `hm-mira-core-*` wrappers/tests/fixtures, and phase49-66 rollups | Historical/future readiness proof for runtime/server/control/kill-switch/auth/encryption/storage/identity. Consolidated under one disabled/no-side-effect contract so the app keeps the safety truth without preserving every executable proof chain. | RETIRED | The executable proof chains were deleted after the replacement contract and hardened consumer scan went green. `ui/__tests__/fixtures/mira-core-disabled-no-side-effect-contract-v0.json` + `ui/__tests__/mira-core-disabled-no-side-effect-contract-v0.test.js` now assert reference-only status, all side-effect flags false, all counters zero, no live consumers/imports outside the retired set, negative fixtures that fail if a flag flips true or a family is marked live/ready, hardened import/whitespace/dynamic-require and child-process wrapper consumer scans including tests, and exact-path dangle checks for `ARCHITECTURE.md` plus `docs/codebase-index.md`. | Not current product power, not a live runtime/server/kill switch, not auth/storage/KMS behavior, not route authority, not a reason to delete emit/reply/route/seed. | Replacement contract/test, this map, and live protected rows for Presence/progress/local text, New Mira bridge, legacy route helpers, and durable seed. | Recreating the old chains reopens stale executable-proof clutter; deleting the replacement contract would lose the no-side-effect invariant. | Future promotion requires a new map row, named lane, acceptance test, and explicit capability proof; keep emit/reply/route helpers until bridge parity and keep durable seed until import-seed parity. |
...
| Tests, fixtures, wrappers, and map guard | `ui/__tests__/mira-*.test.js`, `ui/__tests__/hm-mira-*.test.js`, `ui/__tests__/helpers/mira-reviewed-import-fixture.js`, `ui/__tests__/fixtures/mira-*-contract.json`, `ui/__tests__/fixtures/mira-progress-contract-v0.json`, `mira/tools/normalize-core-dry-run.js`, `ui/scripts/hm-mira-core-*.js`, `ui/scripts/hm-mira-progress.js`, `ui/scripts/mira-system-map-guard.js`, `scripts/pre-commit.sh` | Regression coverage, CLI/status seams, fixture contracts, self-contained reviewed-import and normalized-core source fixtures, and source-of-truth enforcement for current and historical Mira modules. | TRANSITION / KEEP-PARK SPLIT | High value for live speech, Telegram, restart, voice, deterministic progress accounting, and CI-portable reviewed-import/normalized-core validation. Reviewed-import tests seed real temp source files from the tracked queue/contract so `source_exists` and `ok:true` remain real without depending on private local workspace files. Normalized-core import tests seed real temp source JSON and use explicit dry-run CLI path flags (`--contract`, `--report`, `--approval`) so preview/apply assertions remain real without reading private local `workspace/knowledge` files. The map guard blocks staged Mira-owned changes unless this map is staged too. | Not product behavior by itself, not proof that old scaffolding is live, not a live-data import, and not a promotion of New Mira import authority. | Jest, fixtures, module builders, wrappers, `ui/__tests__/mira-progress-v0.test.js`, `mira-reviewed-import-plan.test.js`, `mira-state-import-tooling.test.js`, `mira-normalized-core-import-contract.test.js`, `mira-system-map-guard.test.js`. | Removing blindly hides regressions, breaks proof chains, reintroduces CI/private-file drift, or lets new Mira surfaces drift away from this map. | Keep live tests; keep the guard path list aligned with this map; keep reviewed-import and normalized-core tests self-contained with real seeded source fixtures; consolidate duplicate phase fixtures only after replacement coverage. |
...
- Quiet curiosity bursts are fenced to the Mira Lab record only: scheduled bursts, quiet-burst schedule install/run-now, and generated reviewed burst plans write/read `.squidrun/runtime/mira-curiosity-items.jsonl` and `.squidrun/runtime/mira-curiosity-bursts.jsonl` but do not unsolicited-`hm-send` to Architect/Builder/Oracle. The legacy `--route-interesting` curiosity-burst flag is tolerated for old callers but now records `dispatch.status=not_sent` with `reason=curiosity_burst_lab_only`; explicit internal delivery remains the separate reviewed direct-route/active-initiative path.
...
### Checkpoint 2026-07-04 - S468 role/scope consolidation and face-jargon extraction

- Map row/capability changed: Mira core intent/local-acceptance normalization, typed restart role validation, direct-channel readiness naming, and the supporting tests/fixtures now align with `ui/modules/role-scope-core.js`. The face-jargon extraction adds shared sanitizer infrastructure but does not complete the later sender-authored face/fail-dark product fix.
- Continue or stop: team continues after the internal commit/review gate. This is source/test/map consolidation only.
- Evidence: S468 focused seam tests passed during the audit, and the full current Jest gate passed before commit authorization. The pre-commit Mira map guard required this same-commit map update.
- Restart/process-start required: NO. The change is code/test/doc only and does not require app restart to validate the committed source.
- Parked/prototype/archive/delete-after-parity: New Mira direct-channel and local UI remain prototype/dry-run; live Telegram ownership, live sends, provider/model use, external actions, durable writes beyond named state roots, and face/fail-dark completion remain unpromoted.
- `JAMES ACTION: NONE`
