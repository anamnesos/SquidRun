...
| Presence, identity, relationship, growth core | `ui/modules/mira-core/presence-runtime-read-path-v0.js`, `mira-presence-runtime-state-v0.js`, `mira-presence-current-scope-state-v0.js`, `mira-progress-v0.js`, `mira-progress-proof-inputs-v0.js`, `relationship-presence-v1.js`, `growth-loop-v0.js`, `identity-anchor-v0.js`, `durable-state-seed-v0.js`, `presence-v0.js`, `profiles.js` | Structures own-state, James context, permissions, growth, identity, Presence state, deterministic progress accounting, current-scope Presence refresh, and HEAD/worktree-bound proof artifact intake. Built to stop generic assistant collapse, restart amnesia, and vibes-based progress. | LIVE / KEEP | Powers current restart proof, non-generic Mira context, durable current-scope state refresh, and computed Mira progress from the v0 contract, Presence state, proof inputs, blocker flags, HEAD metadata, and fresh generated proof artifacts, including A0 Presence, startup/restart accounting, A1 what-now, A2 internal-request draft, and local text surface proof keys. | Not a blanket durable memory claim, not permission to use raw/private context, not hand-edited global percent authority, and not permission for stale once-green proof files to keep passing after source commits or dirty working-tree changes. | Contract fixtures, `.squidrun/state`, seed sources, `.squidrun/handoffs/current-lane.json`, `.squidrun/runtime/mira-progress-proof-inputs-v0.json`; `ui/__tests__/fixtures/mira-progress-contract-v0.json`, `ui/__tests__/mira-progress-v0.test.js`, `ui/__tests__/mira-progress-proof-inputs-v0.test.js`, `ui/__tests__/mira-presence-runtime-state-v0.test.js`, `ui/scripts/hm-mira-progress.js`, `ui/scripts/hm-mira-visible-presence-proof.js`, `ui/scripts/hm-mira-presence-runtime-state-v0.js`; presence/read-path/state, relationship/growth/identity, live what-now, live internal-request draft, and local text surface tests. | Removing creates restart amnesia, generic Mira drift, or unaccounted progress-number drift. | Migrate selectively into New Mira with exact provenance, false-green tests, and reviewed progress-contract changes. |
