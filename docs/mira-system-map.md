...
| Visible Mira reply | Current SquidRun Mira product surface; New Mira runtime prototype for local turns. | SquidRun can produce visible Mira replies through Mira Live -> Mira Lab prompt reply -> text attachment/language gates. New Mira runtime turns now have a parity visible-reply gate for preamble, assistant-shape, and backstage/proof-label blocking, and held runtime journal records carry gate status/reason while storing only the clean held reply. | New Mira runtime turns are prototype; they are not the current live Telegram reply path and do not yet prove full Mira Live IPC transcript/replay or visible-route replacement parity. | `ui/modules/mira-live-entrypoint.js`, `ui/modules/mira-lab-surface.js`, `ui/modules/ipc/mira-lab-handlers.js`, `ui/modules/mira-core/text-model-attachment-v1.js`, `mira/runtime/src/turn.ts`, `mira/runtime/src/turn-journal.ts`, `ui/__tests__/mira-runtime-turn-visible-reply-parity.test.js`. | Extend parity to endpoint/UI transcript and replay behavior before replacing current visible reply paths. |
...
- Extend New Mira reply parity from deterministic runtime held-journal metadata into endpoint/UI transcript and replay behavior.
