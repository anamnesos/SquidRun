...
| Restart and startup continuity | `ui/modules/startup-ai-briefing.js`, `ui/modules/mira-core/typed-restart-continuity-context-v0.js`, `ui/modules/mira-core/live-what-now-answer-v0.js`, `ui/modules/mira-core/mira-progress-v0.js`, `ui/modules/mira-core/mira-progress-proof-inputs-v0.js`, `ui/scripts/hm-restart-*.js`, `ui/scripts/hm-mira-progress.js`, `ui/scripts/hm-mira-visible-presence-proof.js`, `.squidrun/handoffs/**`, `.squidrun/runtime/mira-progress-proof-inputs-v0.json`, `.squidrun/state/mira-presence-runtime-state.json` | Materializes restart context, Presence critique, next action, stale markers, hard-stop behavior, machine-readable `computed_progress`, fresh visible Presence proof intake, and a read-only visible `what now?` answer from current-lane/comms evidence. Built so James is not the restart harness. | LIVE / KEEP | Protects current restart continuity, carries read-only startup progress accounting, blocks stale/prose-only false continuity, and can answer current lane/status + recent changes + next internal move with exactly one `JAMES ACTION:` line and no send/runtime POST/external action; startup consumes the same HEAD/worktree-bound progress proof path as `hm-mira-progress`. | Not generic `ai-briefing.md` prose, not a manual percent bump, not a reason for James to manually verify restarts, not proof that New Mira owns live continuity, not authority from parked/prototype/archive scaffolds, and not a freshness bypass when the proof artifact HEAD/worktree no longer matches the current clean repository state. | Structured lane/state files, startup generator, progress contract, HEAD/worktree-bound proof artifact, current-session comms journal; startup, typed restart, live what-now, restart script, `mira-live-what-now-answer-v0.test.js`, `mira-progress-v0.test.js`, `mira-progress-proof-inputs-v0.test.js`, and `startup-ai-briefing.test.js` tests. | Removing reopens restart amnesia, stale lane confusion, invisible computed-progress drift, user-visible what-now gaps, or stale proof carryover. | Simplify only with tests, a current-lane materializer proof, visible what-now parity, and progress accounting parity. |
