...
| Restart and startup continuity | `ui/modules/startup-ai-briefing.js`, `ui/modules/mira-core/typed-restart-continuity-context-v0.js`, `ui/modules/mira-core/mira-presence-current-scope-state-v0.js`, `ui/modules/mira-core/live-what-now-answer-v0.js`, `ui/modules/mira-core/live-internal-request-draft-v0.js`, `ui/modules/mira-core/live-internal-handoff-preview-v0.js`, `ui/modules/mira-core/mira-progress-v0.js`, `ui/modules/mira-core/mira-progress-proof-inputs-v0.js`, `ui/scripts/hm-restart-*.js`, `ui/scripts/hm-mira-progress.js`, `ui/scripts/hm-mira-visible-presence-proof.js`, `ui/scripts/hm-mira-presence-runtime-state-v0.js`, `.squidrun/handoffs/**`, `.squidrun/runtime/mira-progress-proof-inputs-v0.json`, `.squidrun/state/mira-presence-runtime-state.json` | Materializes restart context, Presence critique, next action, stale markers, hard-stop behavior, closed/active current-lane status, machine-readable `computed_progress`, fresh A0/startup/restart/A1/A2/handoff-preview progress proof intake, a read-only visible `what now?` answer, draft-only internal Builder/Oracle request bodies, and approval-ready internal handoff previews from current-lane/comms/progress evidence. Built so James is not the restart harness. | LIVE / KEEP | Protects current restart continuity, carries read-only startup progress accounting, blocks stale/prose-only false continuity, can answer current lane/status + recent changes + next internal move, can regenerate durable Presence state from current lane/progress evidence after clean proof, can draft a reviewable internal request, and can preview the exact `hm-send` command/payload for that draft with source evidence, target agent, reason/trigger, risk/blocked exclusions, exactly one `JAMES ACTION:` line, and no hm-send/model/runtime POST/external action; startup consumes the same HEAD/worktree-bound progress proof path as `hm-mira-progress`. | Not generic `ai-briefing.md` prose, not a manual percent bump, not a reason for James to manually verify restarts, not proof that New Mira owns live continuity, not authority from parked/prototype/archive scaffolds, not an internal send/route/POST/model/external-action permission, not A3/A4 arm authority, and not a freshness bypass when the proof artifact HEAD/worktree no longer matches the current clean repository state. | Structured lane/state files, startup generator, progress contract, HEAD/worktree-bound proof artifact, current-session comms journal; startup, typed restart, live what-now, live internal-request draft, live internal-handoff preview, restart script/probe, `mira-live-what-now-answer-v0.test.js`, `mira-live-internal-request-draft-v0.test.js`, `mira-live-internal-handoff-preview-v0.test.js`, `mira-local-text-ui-surface.test.js`, `mira-progress-v0.test.js`, `mira-progress-proof-inputs-v0.test.js`, `mira-presence-runtime-state-v0.test.js`, and `startup-ai-briefing.test.js` tests. | Removing reopens restart amnesia, stale lane confusion, invisible computed-progress drift, user-visible what-now/request-draft/handoff-preview gaps, or stale proof carryover. | Simplify only with tests, a current-lane materializer proof, visible what-now/request-draft/handoff-preview parity, Presence current-scope refresh proof, and progress accounting parity. |
