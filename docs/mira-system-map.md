...
| Runtime, server, kill-switch, auth, storage, and identity disabled scaffold | `ui/modules/mira-core/runtime-*.js`, `server-*.js`, `kill-switch-*.js`, `auth-binding.js`, `encryption-key.js`, `identity-signing.js`, `storage-retention.js`, `persistence-audit.js`, matching `hm-mira-core-*` wrappers/tests/fixtures, and phase49-66 rollups | Historical/future readiness proof for runtime/server/control/kill-switch/auth/encryption/storage/identity. Consolidated under one disabled/no-side-effect contract so the app keeps the safety truth without preserving every executable proof chain. | ARCHIVE / PARK or RETIRED | `ui/__tests__/fixtures/mira-core-disabled-no-side-effect-contract-v0.json` + `ui/__tests__/mira-core-disabled-no-side-effect-contract-v0.test.js` assert reference-only status, all side-effect flags false, all counters zero, no live consumers/imports outside the collapse set, negative fixtures that fail if a flag flips true or a family is marked live/ready, hardened import/whitespace/dynamic-require and child-process wrapper consumer scans including tests, and exact-path dangle checks for `ARCHITECTURE.md` plus `docs/codebase-index.md`. | Not current product power, not a live runtime/server/kill switch, not auth/storage/KMS behavior, not route authority, not a reason to delete emit/reply/route/seed. | Replacement contract/test, this map, and live protected rows for Presence/progress/local text, New Mira bridge, legacy route helpers, and durable seed. | Removing before the replacement contract is green loses the no-side-effect invariant; keeping the old chains after that point keeps a stale executable museum. | After contract-green and consumer check, delete collapsed scaffold; keep emit/reply/route helpers until bridge parity and keep durable seed until import-seed parity. |
