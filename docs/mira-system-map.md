...
| Presence, identity, relationship, growth core | `ui/modules/mira-core/presence-runtime-read-path-v0.js`, `mira-presence-runtime-state-v0.js`, `mira-progress-v0.js`, `mira-progress-proof-inputs-v0.js`, `relationship-presence-v1.js`, `growth-loop-v0.js`, `identity-anchor-v0.js`, `durable-state-seed-v0.js`, `presence-v0.js`, `profiles.js` | Structures own-state, James context, permissions, growth, identity, Presence state, deterministic progress accounting, and HEAD/worktree-bound proof artifact intake. Built to stop generic assistant collapse, restart amnesia, and vibes-based progress. | LIVE / KEEP | Powers current restart proof, non-generic Mira context, and computed Mira progress from the v0 contract, Presence state, proof inputs, blocker flags, HEAD metadata, and fresh generated proof artifacts, including A0 Presence, A1 what-now, A2 internal-request draft, and local text surface proof keys. | Not a blanket durable memory claim, not permission to use raw/private context, not hand-edited global percent authority, and not permission for stale once-green proof files to keep passing after source commits or dirty working-tree changes. | Contract fixtures, `.squidrun/state`, seed sources, `.squidrun/runtime/mira-progress-proof-inputs-v0.json`; `ui/__tests__/fixtures/mira-progress-contract-v0.json`, `ui/__tests__/mira-progress-v0.test.js`, `ui/__tests__/mira-progress-proof-inputs-v0.test.js`, `ui/scripts/hm-mira-progress.js`, `ui/scripts/hm-mira-visible-presence-proof.js`; presence/read-path/state, relationship/growth/identity, live what-now, live internal-request draft, and local text surface tests. | Removing creates restart amnesia, generic Mira drift, or unaccounted progress-number drift. | Migrate selectively into New Mira with exact provenance, false-green tests, and reviewed progress-contract changes. |
| Restart and startup continuity | `ui/modules/startup-ai-briefing.js`, `ui/modules/mira-core/typed-restart-continuity-context-v0.js`, `ui/modules/mira-core/live-what-now-answer-v0.js`, `ui/modules/mira-core/live-internal-request-draft-v0.js`, `ui/modules/mira-core/mira-progress-v0.js`, `ui/modules/mira-core/mira-progress-proof-inputs-v0.js`, `ui/scripts/hm-restart-*.js`, `ui/scripts/hm-mira-progress.js`, `ui/scripts/hm-mira-visible-presence-proof.js`, `.squidrun/handoffs/**`, `.squidrun/runtime/mira-progress-proof-inputs-v0.json`, `.squidrun/state/mira-presence-runtime-state.json` | Materializes restart context, Presence critique, next action, stale markers, hard-stop behavior, machine-readable `computed_progress`, fresh A0/A1/A2 progress proof intake, a read-only visible `what now?` answer, and draft-only internal Builder/Oracle request bodies from current-lane/comms evidence. Built so James is not the restart harness. | LIVE / KEEP | Protects current restart continuity, carries read-only startup progress accounting, blocks stale/prose-only false continuity, can answer current lane/status + recent changes + next internal move, and can draft a reviewable internal request with source evidence, target agent, proposed body, reason/trigger, exclusions, exactly one `JAMES ACTION:` line, and no hm-send/model/runtime POST/external action; startup consumes the same HEAD/worktree-bound progress proof path as `hm-mira-progress`. | Not generic `ai-briefing.md` prose, not a manual percent bump, not a reason for James to manually verify restarts, not proof that New Mira owns live continuity, not authority from parked/prototype/archive scaffolds, not an internal send/route/POST/model/external-action permission, and not a freshness bypass when the proof artifact HEAD/worktree no longer matches the current clean repository state. | Structured lane/state files, startup generator, progress contract, HEAD/worktree-bound proof artifact, current-session comms journal; startup, typed restart, live what-now, live internal-request draft, restart script, `mira-live-what-now-answer-v0.test.js`, `mira-live-internal-request-draft-v0.test.js`, `mira-local-text-ui-surface.test.js`, `mira-progress-v0.test.js`, `mira-progress-proof-inputs-v0.test.js`, and `startup-ai-briefing.test.js` tests. | Removing reopens restart amnesia, stale lane confusion, invisible computed-progress drift, user-visible what-now/request-draft gaps, or stale proof carryover. | Simplify only with tests, a current-lane materializer proof, visible what-now/request-draft parity, and progress accounting parity. |
