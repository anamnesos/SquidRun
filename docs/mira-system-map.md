...
| Telegram / channel access | Current SquidRun Mira product surface. | SquidRun routes main text-only Telegram inbound to Mira Live only when the env flag allows it; commands, ops, media, failures, duplicate replies, and cross-profile cases are guarded. New Mira's first direct-channel proof is Telegram-first dry-run candidate building behind that guard stack, and the same allowed owner text now has a comparison proof against the current Mira Live reply seam. | New Mira does not own Telegram route control, send live Telegram, or replace Mira Live today. The comparison proof does not send, flip route owner, call a provider/model, or start runtime. | `ui/modules/main/squidrun-app.js`, `ui/modules/mira-telegram-turn-candidate.js`, `ui/scripts/hm-telegram*.js`, `ui/modules/telegram-poller.js`, `ui/__tests__/hm-telegram*.test.js`, `ui/__tests__/mira-telegram-turn-candidate.test.js`, `ui/__tests__/squidrun-app.test.js`. | Separately review any live route promotion, separate New Mira channel, or local UI product promotion as a James-visible choice/test gate. |
...
| Telegram route and external text channel | `ui/modules/main/squidrun-app.js`, `ui/modules/mira-telegram-turn-candidate.js`, `ui/modules/telegram-poller.js`, `ui/modules/main/telegram-poller-worker.js`, `ui/scripts/hm-telegram*.js`, root `telegram-poller.js` | Receives Telegram, separates commands/ops/media, optionally routes main text to Mira Live, sends/dedupes/chunks replies, and can build a New Mira `/turn` dry-run candidate for eligible main-owner text. | LIVE / KEEP with PROTOTYPE dry-run seam | Current external text path when configured; the dry-run and comparison seams prove candidate shape without sending or flipping ownership. | New Mira does not own live Telegram, and the dry-run/comparison candidate is not a send path, route change, model call, runtime start, or external action. | Telegram bot env, poller state, SquidRun app; `hm-telegram*`, `telegram-poller`, routing tests, `mira-telegram-turn-candidate.test.js`, focused comparison test in `squidrun-app.test.js`. | Removing breaks external chat, fallback safety, route protections, or the first direct-channel proof. | Review live route-owner switch, separate New Mira channel, or local UI product promotion only as later James-visible gates. |
