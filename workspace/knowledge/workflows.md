...
- **Unified trading risk + Polymarket loop:** `ui/modules/trading/orchestrator.js` now uses `portfolio-tracker.getPortfolioSnapshot()` for kill-switch/daily-pause checks so total equity spans Alpaca, Polymarket, and other tracked markets. For prediction-market runs, call the orchestrator with `broker: 'polymarket'` or `assetClass: 'prediction_market'`; it will scan markets, build agent consensus, size with `polymarket-sizer`, and execute through `executor.submitOrder()` -> `polymarket-client.createOrder()`. Smart-money convergence inputs should be passed via `smartMoneySignals` or `smartMoneyScanner`, which auto-promote symbols into the dynamic watchlist with source `smart_money`.
