...
- **Unified trading risk:** `ui/modules/trading/orchestrator.js` uses `portfolio-tracker.getPortfolioSnapshot()` for kill-switch/daily-pause checks so total equity spans Alpaca, DeFi, and other tracked markets. Smart-money convergence inputs should be passed via `smartMoneySignals` or `smartMoneyScanner`, which auto-promote symbols into the dynamic watchlist with source `smart_money`.
